...
// link-preview runs the fetched URL through the SSRF guard, which resolves the
// hostname for real. The suites run without DNS, so every preview would be
// refused as "URL not allowed" (400) long before the stubbed fetch was reached,
// and each attempt would stall on a resolver timeout. The guard's own behaviour
// is covered by its unit suite; here it has to stay out of the way.
vi.mock('../../src/utils/ssrfGuard', () => ({
  checkSsrf: async () => ({ allowed: true, isPrivate: false, resolvedIp: '93.184.216.34' }),
  createPinnedDispatcher: () => ({ close: async () => {} }),
  safeFetchFollow: async (url: string) => ({ url }),
