...
export function createMcpTestRegistry(): McpRegistry {
  ...
  const immichService = new ImmichService(
    ...
    new MemoriesAccessService(dbService),
    generalStorage,
  );
  ...
  const captureBackfill = new PhotoCaptureBackfillService(
    new PhotoResolverService(
      trekPhotos,
      new ThumbnailService(addonsService, generalStorage, dbService),
      new TrekPhotoCacheService(dbService, generalStorage),
      new PhotoProviderRegistry([new ImmichPhotoProvider(immichService), new SynologyPhotoProvider(synologyService)]),
      generalStorage,
    ),
    trekPhotos,
    generalStorage,
  ...
  return createTestRegistry(
    [
      ...
      new TripInviteMcp(
        new TripInviteService(dbService, permissionsService, new TripMembershipService(dbService)),
        dbService,
        new RuntimeEnvService(),
        guards,
        new AuditService(dbService),
      ...
      new CollectionsMcp(
        new CollectionsService(dbService, permissionsService, realtimeService, notificationsStub(), generalStorage),
        dbService,
        authService,
        addonsService,
      ),
      ...
      new JourneyMcp(
        journeyDomain,
        new JourneyShareService(dbService, journeyDomain, new SettingsService(dbService)),
        addonsService,
        authService,
        captureBackfill,
      ),
      ...
      new ReservationImportMcp(
        new AirtrailImportService(
          dbService,
          realtimeService,
          reservationsService,
          new AirtrailClient(),
          new AirtrailService(dbService, new AuditService(dbService), new AirtrailClient()),
        ),
        dbService,
        authService,
        guards,
