...
function ctrl() {
  const svc = {
    ...
    updateUserConfig: vi.fn((_id: string, _uid: number, patch: Record<string, unknown>) => ({
      ...patch,
      apiKey: '••••••••',
    })),
...
describe('PluginUserSettingsController', () => {
  ...
  it('POST delegates the patch for a bound user; empty when gated', () => {
    const { c, svc } = ctrl();
    expect(c.update('p', { config: { units: 'metric' } }, req(5))).toEqual({
