...
const PLUGIN_HOST_VERSION = process.env.TREK_PLUGIN_HOST_VERSION;
afterEach(() => {
  if (APP_VERSION === undefined) delete process.env.APP_VERSION;
  else process.env.APP_VERSION = APP_VERSION;
  if (PLUGIN_HOST_VERSION === undefined) delete process.env.TREK_PLUGIN_HOST_VERSION;
  else process.env.TREK_PLUGIN_HOST_VERSION = PLUGIN_HOST_VERSION;
...
describe('hostSatisfies', () => {
  ...
  it('uses the independent plugin compatibility baseline before the display version', () => {
    process.env.APP_VERSION = '0.1.0';
    process.env.TREK_PLUGIN_HOST_VERSION = '4.2.1';
    expect(hostVersion()).toBe('4.2.1');
    expect(hostSatisfies('>=4.0.0 <5.0.0')).toBe(true);
    expect(hostSatisfies('>=4.3.0')).toBe(false);
  });
  it('uses the compatibility baseline for all TT 0.x display versions', () => {
    process.env.APP_VERSION = '0.2.0';
    delete process.env.TREK_PLUGIN_HOST_VERSION;
    expect(hostVersion()).toBe('4.2.1');
    expect(hostSatisfies('>=4.0.0 <5.0.0')).toBe(true);
  });
