...
function build(
  ...
  const registry = createTestPluginRegistry([
    new CollabRpc(
      {
        createNote: vi.fn(() => ({})),
        createPoll: vi.fn(() => ({})),
        votePoll: vi.fn(() => ({ error: null, poll: {} })),
        createMessage: vi.fn(() => ({ error: null, message: {} })),
        listNotes: vi.fn(() => []),
        listPolls: vi.fn(() => []),
        listMessages: vi.fn(() => []),
        ...overrides.collab,
    ...
    new AtlasRpc(
      {
        listVisitedCountries: vi.fn(() => []),
        listManuallyVisitedRegions: vi.fn(() => []),
        bucketList: vi.fn(() => []),
        markCountry: vi.fn(),
        unmarkCountry: vi.fn(),
        markRegion: vi.fn(),
    ...
    new CollectionsRpc(
      {
        ...
        savePlace: vi.fn(() => ({})),
        copyToTrip: vi.fn(() => ({})),
        deletePlace: vi.fn(() => Promise.resolve(undefined)),
...
const USERLESS: Array<[string, Record<string, unknown>, string]> = [
  ...
  [
    'journal.createEntry',
    { journeyId: 7, input: { entry_date: '2027-01-01' } },
  ...
  [
    ...
    { entryId: 1, input: { name: 'a.jpg', content_base64: 'eA==' } },
    'journal writes require an authenticated user context',
  ],
  ...
  [
    'atlas.markRegion',
    { regionCode: 'JP-13', countryCode: 'JP' },
    'atlas writes require an authenticated user context',
  ],
  ...
  [
    ...
    { input: { collection_id: 1, name: 'P', lat: 1, lng: 2 } },
    'collection writes require an authenticated user context',
  ],
  [
    'collections.copyToTrip',
    { input: { trip_id: 1, place_ids: [1] } },
    'collection writes require an authenticated user context',
  ],
  ['collections.deletePlace', { placeId: 1 }, 'collection writes require an authenticated user context'],
  [
    'collab.createNote',
    { tripId: 1, input: { title: 'n' } },
    'collab note writes require an authenticated user context',
  ],
  [
    'collab.createPoll',
    { tripId: 1, input: { question: 'q', options: ['a', 'b'] } },
    'collab poll writes require an authenticated user context',
  ],
  [
    'collab.votePoll',
    { tripId: 1, pollId: 1, optionIndex: 0 },
    'collab poll writes require an authenticated user context',
  ],
...
describe('addon-gated domains validate their input', () => {
  it('ADDONERR-001 collab refuses a blank note title, a thin poll and an oversized message', async () => {
    const host = build();
    expect(err(await host.dispatch(req('collab.createNote', { tripId: 1, input: { title: '  ' } }), 42)).message).toBe(
      'note title is required',
    );
    expect(
      err(
        await host.dispatch(
          req('collab.createPoll', { tripId: 1, input: { question: '  ', options: ['a', 'b'] } }),
          42,
        ),
      ).message,
    ).toBe('poll question is required');
    expect(
      err(await host.dispatch(req('collab.createPoll', { tripId: 1, input: { question: 'q', options: ['a'] } }), 42))
        .message,
    ).toBe('a poll needs at least two options');
    expect(
      err(await host.dispatch(req('collab.createMessage', { tripId: 1, text: 'x'.repeat(4001) }), 42)).message,
    ).toBe('message text is required (max 4000 chars)');
    expect(err(await host.dispatch(req('collab.createMessage', { tripId: 1, text: '  ' }), 42)).message).toBe(
      'message text is required (max 4000 chars)',
    );
  });
  ...
  it('ADDONERR-002 the collab service reports its own failures as BAD_PARAMS', async () => {
    const host = build({
      collab: {
        votePoll: vi.fn(() => ({ error: 'poll is closed' })),
        createMessage: vi.fn(() => ({ error: 'reply target is gone' })),
      },
    });
    expect(
      err(await host.dispatch(req('collab.votePoll', { tripId: 1, pollId: 1, optionIndex: 0 }), 42)),
    ).toMatchObject({ code: 'BAD_PARAMS', message: 'poll is closed' });
    expect(err(await host.dispatch(req('collab.createMessage', { tripId: 1, text: 'hi' }), 42))).toMatchObject({
      code: 'BAD_PARAMS',
      message: 'reply target is gone',
    });
  });

  it('ADDONERR-003 atlas rejects a code that is empty or too long', async () => {
    const host = build();
    expect(err(await host.dispatch(req('atlas.markCountry', { code: '' }), 42)).message).toBe(
      'code must be a short code',
    );
    expect(err(await host.dispatch(req('atlas.markCountry', { code: 'TOOLONGCODE' }), 42)).message).toBe(
      'code must be a short code',
    );
    expect(
      err(await host.dispatch(req('atlas.markRegion', { regionCode: 'JP-13', countryCode: '' }), 42)).message,
    ).toBe('countryCode must be a short code');
    expect(err(await host.dispatch(req('atlas.createBucketItem', { input: { name: ' ' } }), 42)).message).toBe(
      'bucket item name is required',
    );
  });

  it('ADDONERR-004 a missing bucket item is refused, naming it', async () => {
    const host = build({ atlas: { deleteBucketItem: vi.fn(() => false) } });
    expect(err(await host.dispatch(req('atlas.deleteBucketItem', { itemId: 404 }), 42)).message).toBe(
      'no bucket item 404 for this user',
    );
  });
  ...
  it("ADDONERR-004b a duplicate bucket item is the caller's mistake, not an internal error (#1898)", async () => {
    ...
    expect(err(await host.dispatch(req('atlas.createBucketItem', { input: { name: 'Kyoto' } }), 42))).toMatchObject({
      code: 'BAD_PARAMS',
      message: 'bucket item already exists',
    });
  });

  it('ADDONERR-005 vacay insists on an ISO date', async () => {
    const host = build();
    expect(err(await host.dispatch(req('vacay.toggleEntry', { date: '01.01.2027' }), 42)).message).toBe(
      'date must be YYYY-MM-DD',
    );
    expect(err(await host.dispatch(req('vacay.toggleCompanyHoliday', { date: 42 }), 42)).message).toBe(
      'date must be YYYY-MM-DD',
    );
  });
  ...
  it('ADDONERR-006 journal needs an entry_date and a journal title', async () => {
    const host = build();
    expect(err(await host.dispatch(req('journal.createEntry', { journeyId: 7, input: {} }), 42)).message).toBe(
      'entry_date is required',
    );
    expect(err(await host.dispatch(req('journal.createJourney', { input: { title: '  ' } }), 42)).message).toBe(
  ...
  it('ADDONERR-007 an unreadable or uneditable journey is RESOURCE_FORBIDDEN', async () => {
    ...
    expect(err(await host.dispatch(req('journal.getEntries', { journeyId: 7 }), 42)).message).toBe(
      'no access to journey 7',
    );
    expect(
      err(await host.dispatch(req('journal.createEntry', { journeyId: 7, input: { entry_date: '2027-01-01' } }), 42))
        .message,
    ).toBe('no editable journey 7 for this user');
    expect(err(await host.dispatch(req('journal.updateEntry', { entryId: 3, input: {} }), 42)).message).toBe(
      'no editable journal entry 3 for this user',
    );
    expect(err(await host.dispatch(req('journal.deleteEntry', { entryId: 3 }), 42)).message).toBe(
      'no editable journal entry 3 for this user',
    );
    expect(err(await host.dispatch(req('journal.deleteJourney', { journeyId: 7 }), 42)).message).toBe(
      'no deletable journal 7 for this user',
    );
  });
  ...
  it('ADDONERR-008 collections map the service status onto the RPC error taxonomy', async () => {
    ...
    const forbidden = build({
      collections: {
        createCollection: vi.fn(() => {
          throw statusError(403, 'not your collection');
        }),
      },
    });
    expect(err(await forbidden.dispatch(req('collections.create', { input: { name: 'C' } }), 42))).toMatchObject({
    ...
    const missing = build({
      collections: {
        updateCollection: vi.fn(() => {
          throw statusError(404, 'no such collection');
        }),
      },
    });
    expect(err(await missing.dispatch(req('collections.update', { id: 1, input: { name: 'C' } }), 42))).toMatchObject({
      code: 'RESOURCE_FORBIDDEN',
    });
    ...
    const conflict = build({
      collections: {
        savePlace: vi.fn(() => {
          throw statusError(409, 'already saved');
        }),
      },
    });
    expect(
      err(
        await conflict.dispatch(
          req('collections.savePlace', { input: { collection_id: 1, name: 'P', lat: 1, lng: 2 } }),
          42,
        ),
      ),
    ).toMatchObject({ code: 'BAD_PARAMS', message: 'already saved' });

    const badRequest = build({
      collections: {
        copyToTrip: vi.fn(() => {
          throw statusError(400, 'nothing to copy');
        }),
      },
    });
    expect(
      err(await badRequest.dispatch(req('collections.copyToTrip', { input: { trip_id: 1, place_ids: [1] } }), 42)),
    ).toMatchObject({ code: 'BAD_PARAMS' });
  });
  ...
  it('ADDONERR-009 an untagged collections failure stays a HOST_ERROR', async () => {
    ...
    expect(err(await host.dispatch(req('collections.deletePlace', { placeId: 1 }), 42))).toMatchObject({
      code: 'HOST_ERROR',
  ...
  it('ADDONERR-009b a refused collections delete is ForbiddenResource, not {deleted: true}', async () => {
    ...
    const host = build({
      collections: { deletePlace: vi.fn(() => Promise.reject(statusError(403, 'not your collection'))) },
    });
  ...
  it('ADDONERR-010 collections reject a payload the shared schema refuses', async () => {
    ...
    expect(err(await host.dispatch(req('collections.update', { id: 1, input: { name: 42 } }), 42)).code).toBe(
      'BAD_PARAMS',
    );
    expect(err(await host.dispatch(req('collections.savePlace', { input: {} }), 42)).code).toBe('BAD_PARAMS');
    expect(
      err(await host.dispatch(req('collections.copyToTrip', { input: { trip_id: 1, place_ids: [] } }), 42)).code,
    ).toBe('BAD_PARAMS');
  });
