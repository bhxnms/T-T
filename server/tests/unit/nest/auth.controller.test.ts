...
describe('AuthPublicController', () => {
  ...
  it('reset-password: error audits a fail, mfa branch, success', () => {
    ...
    expect(
      apc(asvc({ resetPassword: vi.fn().mockReturnValue({ userId: 1 }) } as Partial<AuthService>), rl()).resetPassword(
        anyBody(),
        req,
      ),
  ...
  it('reset-password 429 once the dedicated reset bucket is exhausted', () => {
    ...
    expect(thrown(() => c.resetPassword(anyBody(), req))).toEqual({
      status: 429,
  ...
  it('demo-login + register + invite throw 429 when the login bucket is exhausted', () => {
    ...
    expect(thrown(() => c.register(anyBody(), req, res))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
...
describe('AuthController (authenticated)', () => {
  it('GET /me 404 when missing, else returns the loaded user', () => {
    ...
    expect(
      ac(asvc({ getCurrentUser: vi.fn().mockReturnValue({ id: 1 }) } as Partial<AuthService>), rl()).me(user),
    ).toEqual({ user: { id: 1 } });
  });
  ...
  it('change-password maps error, else audits', () => {
    ...
    expect(
      ac(asvc({ changePassword: vi.fn().mockReturnValue({}) } as Partial<AuthService>), rl()).changePassword(
        user,
        anyBody(),
        req,
        res,
  ...
  it('resource-token 503 when unavailable, else returns the token payload', () => {
    expect(
      thrown(() => ac(asvc({}), rl(), { createResourceToken: vi.fn().mockReturnValue(null) }).resourceToken(user, {})),
    ).toEqual({ status: 503, body: { error: 'Service unavailable' } });
    expect(
      ac(asvc({}), rl(), { createResourceToken: vi.fn().mockReturnValue({ token: 'rt' }) }).resourceToken(user, {
        purpose: 'download',
      }),
    ).toEqual({ token: 'rt' });
  });
  ...
  it('ws/resource tokens throttle on their own buckets, so a mint loop cannot drain the store or block login', () => {
    ...
    expect(
      thrown(() => ac(asvc({}), s, { createWsToken: vi.fn().mockReturnValue({ token: 'ws' }) }).wsToken(user)),
  ...
  it('the ws/resource ceilings are per account, not per address: one heavy user cannot 429 the office', () => {
    ...
    expect(ac(asvc({}), s, { createWsToken: vi.fn().mockReturnValue({ token: 'ws2' }) }).wsToken(other)).toEqual({
      token: 'ws2',
    });
    expect(
      ac(asvc({}), s, { createResourceToken: vi.fn().mockReturnValue({ token: 'rt2' }) }).resourceToken(other, {}),
    ).toEqual({ token: 'rt2' });
  });
  ...
  it('rate-limited account ops throw 429 once the bucket is exhausted', () => {
    ...
    expect(thrown(() => c.changePassword(user, anyBody(), req, res))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
  });
  ...
  it('delete-account maps error, else audits and succeeds', () => {
    ...
    expect(
      ac(asvc({ deleteAccount: vi.fn().mockReturnValue({}) } as Partial<AuthService>), rl()).deleteAccount(user, req),
    ).toEqual({ success: true });
  ...
  it('maps-key + api-keys pass straight through to the service', () => {
    ...
    expect(ac(asvc({}), rl(), {}, { updateMapsKey }).mapsKey(user, { maps_api_key: 'k' }, req)).toEqual({
      success: true,
    });
  ...
  it('api-keys audits one settings.api_keys_update with the changed names only (#1939)', () => {
    ...
    expect(c.apiKeys(user, anyBody({ maps_api_key: 'AIza-super-secret' } as never), req)).toEqual({
      success: true,
      user: { id: 1 },
    });
  ...
  it('maps-key and settings audit the same action; an unchanged save writes nothing', () => {
    ...
    writeAudit.mockClear();
    ac(asvc({}), rl(), {}, { updateApiKeys: vi.fn().mockReturnValue({ success: true, changedKeys: [] }) }).apiKeys(
      user,
      anyBody({ maps_api_key: 'same' } as never),
      req,
    );
    expect(writeAudit).not.toHaveBeenCalled();
  });
  ...
  it('update-settings + get-settings map errors, else return their payloads', () => {
    expect(
      thrown(() =>
        ac(
          ...
          rl(),
          {},
          { updateSettings: vi.fn().mockReturnValue({ error: 'Bad', status: 400 }) },
    ...
    expect(
      thrown(() =>
        ac(asvc({}), rl(), {}, { getSettings: vi.fn().mockReturnValue({ error: 'Nope', status: 404 }) }).getSettings(
          user,
        ),
      ),
    ).toEqual({ status: 404, body: { error: 'Nope' } });
    expect(
      ac(asvc({}), rl(), {}, { getSettings: vi.fn().mockReturnValue({ settings: { theme: 'dark' } }) }).getSettings(
        user,
      ),
    ...
    expect(
      await thrownAsync(() =>
        ac(
          ...
          rl(),
          {},
          { validateKeys: vi.fn().mockResolvedValue({ error: 'fail', status: 502 }) },
        ).validateKeys(user),
      ),
  ...
  it('mcp-tokens list + create error + delete error/success', () => {
    expect(ac(asvc({}), rl(), { listMcpTokens: vi.fn().mockReturnValue([{ id: 't' }]) }).listMcpTokens(user)).toEqual({
      tokens: [{ id: 't' }],
    });
    expect(
      thrown(() =>
    ...
    expect(ac(asvc({}), rl(), { deleteMcpToken: vi.fn().mockReturnValue({}) }).deleteMcpToken(user, 'tid')).toEqual({
      success: true,
    });
  });
  ...
  it('api-tokens list + create success/error + delete error/success', () => {
    expect(ac(asvc({}), rl(), { listApiTokens: vi.fn().mockReturnValue([{ id: 'a' }]) }).listApiTokens(user)).toEqual({
      tokens: [{ id: 'a' }],
    });
    expect(
      ac(asvc({}), rl(), { createApiToken: vi.fn().mockReturnValue({ token: 'trek_x' }) }).createApiToken(
        user,
        { name: 'Homepage' },
        req,
      ),
    ...
    expect(
      thrown(() =>
        ac(asvc({}), rl(), {
          createApiToken: vi.fn().mockReturnValue({ error: 'Name taken', status: 409 }),
    ...
    expect(
      thrown(() =>
        ac(asvc({}), rl(), {
          deleteApiToken: vi.fn().mockReturnValue({ error: 'Not found', status: 404 }),
        }).deleteApiToken(user, 'tid'),
      ),
    ).toEqual({ status: 404, body: { error: 'Not found' } });
    expect(ac(asvc({}), rl(), { deleteApiToken: vi.fn().mockReturnValue({}) }).deleteApiToken(user, 'tid')).toEqual({
      success: true,
    });
  });
  ...
  it('ws-token maps error, else returns the token', () => {
    expect(
      thrown(() =>
        ac(asvc({}), rl(), { createWsToken: vi.fn().mockReturnValue({ error: 'down', status: 503 }) }).wsToken(user),
      ),
    ).toEqual({ status: 503, body: { error: 'down' } });
    expect(ac(asvc({}), rl(), { createWsToken: vi.fn().mockReturnValue({ token: 'ws' }) }).wsToken(user)).toEqual({
      token: 'ws',
    });
  });
