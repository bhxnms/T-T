...
const adminCtl = (
  ...
  addons: AddonsService = addonsStub(),
...
describe('AdminController system-info', () => {
  it('ADM-SYSINFO-001: hands the extractor description through unchanged', () => {
    const describeFn = vi.fn(() => ({
      available: true,
      path: '/usr/local/bin/kitinerary-extractor',
      version: '6.3.3',
      configuredPath: null,
    }));
    const res = adminCtl(
...
describe('AdminController users', () => {
  it('lists, creates (201 + audit), maps an error', () => {
    expect(
      adminCtl(svc({ listUsers: vi.fn().mockReturnValue([{ id: 1 }]) } as Partial<AdminService>)).listUsers(),
    ).toEqual({ users: [{ id: 1 }] });
    expect(
      thrown(() =>
        adminCtl(
  ...
  it('update + delete audit and map errors', () => {
    ...
    expect(
      adminCtl(svc({ deleteUser: vi.fn().mockReturnValue({ email: 'a@b.c' }) } as Partial<AdminService>)).deleteUser(
        user,
        '2',
        req,
      ),
    ...
    expect(
      thrown(() =>
        adminCtl(
          ...
          undefined,
          {},
          { deleteInvite: vi.fn().mockReturnValue({ error: 'not found', status: 404 }) },
...
describe('AdminController addons + sessions + jwt + defaults', () => {
  it('addon update audits + invalidates MCP sessions only when the MCP surface changed (#1414)', async () => {
    ...
    const c = adminCtl(
      svc({
        updateAddon: vi
          .fn()
          .mockReturnValue({ addon: { id: 'mcp', enabled: true }, mcpAffected: true, auditDetails: {} }),
    ...
    const noop = adminCtl(
      svc({
        updateAddon: vi
          .fn()
          .mockReturnValue({ addon: { id: 'llm_parsing', enabled: true }, mcpAffected: false, auditDetails: {} }),
        invalidateMcpSessions: noopInvalidate,
      } as Partial<AdminService>),
      runtime,
    );
    expect(await noop.updateAddon(user, 'llm_parsing', { config: { model: 'x' } }, req)).toEqual({
      addon: { id: 'llm_parsing', enabled: true },
    });
    expect(noopInvalidate).not.toHaveBeenCalled();

    // Disabling an addon cascades to the plugin runtime.
    const runtime2 = {
      deactivateForDisabledAddon: vi.fn().mockResolvedValue(['p1']),
    } as unknown as PluginRuntimeService;
    const disable = adminCtl(
      svc({
        updateAddon: vi
          .fn()
          .mockReturnValue({ addon: { id: 'budget', enabled: false }, mcpAffected: true, auditDetails: {} }),
  ...
  it('addon update maps the journey-dependency refusal to its status, before any audit', async () => {
    const c = adminCtl(
      svc({
        updateAddon: vi.fn().mockReturnValue({ error: 'Enable the Journey addon first', status: 409 }),
      } as Partial<AdminService>),
    );
  ...
  it('oauth-sessions revoke audits; rotate-jwt maps error', () => {
    expect(
      adminCtl(
        ...
        {},
        {},
        { adminRevokeOAuthSession: vi.fn().mockReturnValue({}) },
    ...
    expect(
      adminCtl(svc({ rotateJwtSecret: vi.fn().mockReturnValue({}) } as Partial<AdminService>)).rotateJwtSecret(
        user,
        req,
      ),
    ).toEqual({ success: true });
...
describe('AdminController error envelope fallbacks', () => {
  it('ok() defaults to 400 when the error envelope omits a status', () => {
    expect(
      thrown(() =>
        adminCtl(svc({ createUser: vi.fn().mockReturnValue({ error: 'boom' }) } as Partial<AdminService>)).createUser(
          user,
          {},
          req,
        ),
...
describe('AdminController read-only getters', () => {
  it('return service values verbatim', async () => {
    ...
    expect(adminCtl(svc({ getStats: vi.fn().mockReturnValue({ users: 3 }) } as Partial<AdminService>)).stats()).toEqual(
      { users: 3 },
    );
    expect(
      adminCtl(svc({ getPermissions: vi.fn().mockReturnValue({ a: 1 }) } as Partial<AdminService>)).permissions(),
    ).toEqual({ a: 1 });
    expect(
      adminCtl(svc({ getAuditLog: vi.fn().mockReturnValue({ entries: [] }) } as Partial<AdminService>)).auditLog({}),
    ).toEqual({ entries: [] });
    await expect(
      adminCtl(
    ...
    expect(
      adminCtl(svc(), undefined, undefined, {}, { listInvites: vi.fn().mockReturnValue([{ id: 1 }]) }).listInvites(),
    ).toEqual({ invites: [{ id: 1 }] });
    expect(
      adminCtl(svc({ listAddons: vi.fn().mockReturnValue([{ id: 'mcp' }]) } as Partial<AdminService>)).listAddons(),
    ).toEqual({ addons: [{ id: 'mcp' }] });
    expect(
      adminCtl(svc(), undefined, undefined, { listAllMcpTokens: vi.fn().mockReturnValue([{ id: 1 }]) }).listMcpTokens(),
    ).toEqual({ tokens: [{ id: 1 }] });
    expect(
      adminCtl(
        svc(),
        ...
        {},
        {},
        { listAllOAuthSessions: vi.fn().mockReturnValue([{ id: 1 }]) },
      ).listOAuthSessions(),
...
describe('AdminController tokens + sessions', () => {
  it('mcp token + oauth session deletes return success and map errors', () => {
    expect(
      adminCtl(svc(), undefined, undefined, { adminDeleteMcpToken: vi.fn().mockReturnValue({}) }).deleteMcpToken(
        user,
        '2',
        req,
      ),
    ...
    expect(
      thrown(() =>
        adminCtl(svc(), undefined, undefined, {
          adminDeleteMcpToken: vi.fn().mockReturnValue({ error: 'no token', status: 404 }),
        }).deleteMcpToken(user, '9', req),
      ),
    ...
    expect(
      thrown(() =>
        adminCtl(
          ...
          {},
          {},
          { adminRevokeOAuthSession: vi.fn().mockReturnValue({ error: 'no session', status: 404 }) },
...
describe('AdminController dev test-notification', () => {
  ...
  it('maps an Error from the notification service to 400', async () => {
    ...
    await expect(adminCtl(svc()).devTestNotification(user, { event: 'trip_reminder' })).rejects.toMatchObject({
...
describe('AdminController feature toggles', () => {
  it('ADMIN-TOGGLE-001 each toggle forwards to AddonsService and writes its own audit action', () => {
    ...
    const cases: Array<[() => unknown, string, keyof AddonsService]> = [
      ...
      [
        () => c.updatePlacesAutocomplete(user, { enabled: true }, req),
  ...
  it('ADMIN-TOGGLE-003 collab-features invalidates MCP sessions only when a flag flipped (#1414)', () => {
    ...
    const noopInvalidate = vi.fn();
    const noop = adminCtl(svc({ invalidateMcpSessions: noopInvalidate } as Partial<AdminService>), undefined, {
      ...addonsStub(),
      updateCollabFeatures: vi.fn(() => ({ features: { chat: true }, changed: false })),
    } as unknown as AddonsService);
