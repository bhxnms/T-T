...
describe('findMatchingPlaceId', () => {
  ...
  it('PLACES-SVC-009 — matches on a provider id even after the place was renamed (#1550)', () => {
    ...
    expect(svc.findMatchingPlaceId(String(trip.id), { name: 'Renamed By User', google_place_id: 'ChIJ_abc' })).toBe(
      place.id,
    );
  });
  ...
  it('PLACES-SVC-010 — does NOT match a NAMED candidate to a different place at the same coordinates', () => {
    ...
    expect(svc.findMatchingPlaceId(String(trip.id), { name: 'Rooftop Bar', lat: 52.52, lng: 13.405 })).toBeNull();
  });
  ...
  it('PLACES-SVC-014 — a provider id still wins over a name that points elsewhere', () => {
    ...
    expect(svc.findMatchingPlaceId(String(trip.id), { name: 'Trattoria da Enzo', google_ftid: '0x1:0x2' })).toBe(
      renamed.id,
    );
  });
