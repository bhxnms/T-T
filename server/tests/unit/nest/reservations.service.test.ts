...
const bridge = {
  ...
  createReservation: (tripId: string | number, data: Parameters<ReservationsService['create']>[1]) =>
    svc.create(tripId, data),
...
describe('ReservationsService (DI-native, real SQL)', () => {
  ...
  describe('create', () => {
    ...
    it('RESV-SVC-003: derives day_id from reservation_time for non-hotel bookings', () => {
      ...
      const day2 = testDb.prepare('SELECT id FROM days WHERE trip_id = ? AND date = ?').get(trip.id, '2030-05-02') as {
        id: number;
      };
      const { reservation } = svc.create(String(trip.id), {
        title: 'Tour',
        type: 'tour',
        reservation_time: '2030-05-02T10:00:00',
      });
      expect(reservation.day_id).toBe(day2.id);
      // Out-of-range time clamps to the nearest day (create path clamps).
      const day3 = testDb.prepare('SELECT id FROM days WHERE trip_id = ? AND date = ?').get(trip.id, '2030-05-03') as {
        id: number;
      };
      const { reservation: clamped } = svc.create(String(trip.id), {
        title: 'Late',
        type: 'tour',
        reservation_time: '2030-06-20T10:00:00',
      });
    ...
    it('RESV-SVC-004: auto-creates the accommodation for a hotel with create_accommodation', () => {
      ...
      const { reservation, accommodationCreated } = svc.create(String(trip.id), {
        ...
        create_accommodation: {
          ...
          end_day_id: days[2].id,
          check_in: '15:00',
        },
        confirmation_number: 'ABC123',
      });
      expect(accommodationCreated).toBe(true);
      const acc = testDb.prepare('SELECT * FROM day_accommodations WHERE trip_id = ?').get(trip.id) as Record<
        string,
        unknown
      >;
    ...
    it('RESV-SVC-005: saves endpoints, skipping null-coordinate rows; sequence defaults to the post-filter index', () => {
      ...
      const rows = testDb
        .prepare('SELECT name, sequence FROM reservation_endpoints WHERE reservation_id = ? ORDER BY sequence')
        .all(reservation.id) as { name: string; sequence: number }[];
    ...
    it('RESV-SVC-006 (quirk fixed): metadata check-in sync keys off the resolved id, so an auto-created accommodation gets its times too', () => {
      ...
      const acc = testDb.prepare('SELECT check_in FROM day_accommodations WHERE trip_id = ?').get(trip.id) as {
  ...
  describe('update', () => {
    ...
    it('RESV-SVC-033: a metadata payload without `price` keeps the linked expense price (and currency) from the stored metadata', () => {
      ...
      const { reservation } = svc.update(
        String(res.id),
        String(trip.id),
        { metadata: { airline: 'CZ', seat: '12A' } },
        current,
      );
      expect(JSON.parse(reservation.metadata as string)).toEqual({
        airline: 'CZ',
        seat: '12A',
        price: '2040',
        priceCurrency: 'CNY',
    ...
    it('RESV-SVC-034: a payload that names `price` is taken at face value, set or cleared', () => {
      ...
      ({ reservation } = svc.update(
        String(res.id),
        String(trip.id),
        { metadata: { price: null, seat: '1A' } },
        current,
      ));
    ...
    it('RESV-SVC-010: endpoints [] wipes stored endpoints; an absent field leaves them alone', () => {
      ...
      testDb
        .prepare(
          'INSERT INTO reservation_endpoints (reservation_id, role, sequence, name, lat, lng) VALUES (?, ?, 0, ?, 1, 2)',
        )
        .run(res.id, 'from', 'A');
      let current = svc.getReservation(String(res.id), String(trip.id))!;
      svc.update(String(res.id), String(trip.id), { notes: 'no endpoints field' }, current);
      expect(
        testDb.prepare('SELECT COUNT(*) as c FROM reservation_endpoints WHERE reservation_id = ?').get(res.id),
      ).toEqual({ c: 1 });
      current = svc.getReservation(String(res.id), String(trip.id))!;
  ...
  describe('remove', () => {
    ...
    it('RESV-SVC-012: returns the empty shape when the reservation is missing', () => {
      const { trip } = ownerTrip();
      expect(svc.remove('999', String(trip.id))).toEqual({
        deleted: undefined,
  ...
  describe('updatePositions', () => {
    it('RESV-SVC-013: without dayId updates the global day_plan_position (legacy branch)', () => {
      ...
      svc.updatePositions(String(trip.id), [
        { id: r2.id, day_plan_position: 0 },
        { id: r1.id, day_plan_position: 1 },
      ]);
      expect(testDb.prepare('SELECT day_plan_position FROM reservations WHERE id = ?').get(r2.id)).toEqual({
        day_plan_position: 0,
      });
      expect(testDb.prepare('SELECT day_plan_position FROM reservations WHERE id = ?').get(r1.id)).toEqual({
        day_plan_position: 1,
      });
    });
    ...
    it('RESV-SVC-014: with dayId upserts per-day positions into reservation_day_positions', () => {
      ...
      expect(
        testDb
          .prepare('SELECT position FROM reservation_day_positions WHERE reservation_id = ? AND day_id = ?')
          .get(r1.id, day.id),
      ).toEqual({ position: 5 });
      expect(testDb.prepare('SELECT day_plan_position FROM reservations WHERE id = ?').get(r1.id)).toEqual({
  ...
  describe('listUpcoming', () => {
    it('RESV-SVC-017: returns future reservations across owned + member trips, skipping cancelled', () => {
      ...
      testDb
        ...
        .run('2999-01-02T10:00:00', cancelled.id);
    ...
    it('RESV-SVC-017d: a reservation with no type at all still shows', () => {
      ...
      testDb
        ...
        .run('2999-01-01T10:00:00', res.id);
    ...
    it('RESV-SVC-017j: a nameless stay falls back to its linked booking', () => {
      ...
      testDb
        ...
        .run(String(acc.lastInsertRowid), booking.id);
  ...
  describe('resyncReservationDays', () => {
    it('RESV-SVC-018: re-anchors a dated booking to the day matching its time; out-of-range stays untouched', () => {
      const { trip } = ownerTrip({ start_date: '2030-05-01', end_date: '2030-05-03' });
      const days = testDb.prepare('SELECT id, date FROM days WHERE trip_id = ? ORDER BY day_number').all(trip.id) as {
        id: number;
        date: string;
      }[];
      ...
      testDb
        ...
        .run('2031-01-01T09:00:00', outside.id);

      svc.resyncReservationDays(String(trip.id));
      expect(testDb.prepare('SELECT day_id FROM reservations WHERE id = ?').get(res.id)).toEqual({
        day_id: days[2].id,
      });
      expect(testDb.prepare('SELECT day_id FROM reservations WHERE id = ?').get(outside.id)).toEqual({
        day_id: days[0].id,
      });
    });
  });
  ...
  describe('syncBudgetOnUpdate', () => {
    ...
    it('syncs the linked expense category when the booking type changes', () => {
      ...
      expect(broadcast).toHaveBeenCalledWith(
        String(trip.id),
        'budget:updated',
        { item: { id: item.id, category: 'flights' } },
        'sock',
      );
    });
    ...
    it('updates an existing linked item when a price is provided', () => {
      ...
      expect(budget.updateBudgetItem).toHaveBeenCalledWith(item.id, String(trip.id), {
        name: 'New',
        category: 'lodging',
        total_price: 80,
      });
    ...
    it('creates + links a new item when none exists, using the current title fallback', () => {
      ...
      expect(budget.createBudgetItem).toHaveBeenCalledWith(String(trip.id), {
        name: 'Old title',
        category: 'flight',
        total_price: 120,
      });
      // The service back-links the created item to the reservation itself.
      expect(testDb.prepare('SELECT reservation_id FROM budget_items WHERE id = ?').get(created.id)).toEqual({
        reservation_id: res.id,
      });
      expect(broadcast).toHaveBeenCalledWith(
        String(trip.id),
        'budget:created',
        { item: { id: created.id, reservation_id: res.id } },
        'sock',
      );
    });
  });
...
describe('ReservationsService — legacy branch parity (coverage of the folded conditionals)', () => {
  it('RESV-SVC-019: create with a linked accommodation_id syncs metadata times + confirmation onto it', () => {
    ...
    const row = testDb
      .prepare('SELECT check_in, check_out, confirmation FROM day_accommodations WHERE id = ?')
      .get(acc.id);
  ...
  it('RESV-SVC-021: hotel update with create_accommodation updates the linked accommodation in place', () => {
    ...
    testDb.prepare('UPDATE reservations SET accommodation_id = ? WHERE id = ?').run(String(acc.id), res.id);
    const current = svc.getReservation(String(res.id), String(trip.id))!;
    const { accommodationChanged } = svc.update(
      String(res.id),
      String(trip.id),
      {
        type: 'hotel',
    ...
    const row = testDb
      .prepare('SELECT place_id, start_day_id, end_day_id, check_in, confirmation FROM day_accommodations WHERE id = ?')
      .get(acc.id);
    expect(row).toEqual({
      place_id: place2.id,
      start_day_id: days[1].id,
  ...
  it('RESV-SVC-022: hotel update inserts a new accommodation when none is linked and a place is given', () => {
    ...
    const res = createReservation(testDb, trip.id, { title: 'Hotel', type: 'hotel' });
    const current = svc.getReservation(String(res.id), String(trip.id))!;
    const { reservation, accommodationChanged } = svc.update(
      String(res.id),
      String(trip.id),
      {
        type: 'hotel',
    ...
    expect(testDb.prepare('SELECT COUNT(*) as c FROM day_accommodations WHERE trip_id = ?').get(trip.id)).toEqual({
      c: 1,
    });
  });
  ...
  it('RESV-SVC-023: explicit update fields bind their values; empty strings null; absent fields keep current', () => {
    ...
    testDb
      .prepare(
        "UPDATE reservations SET location = 'Loc', notes = 'N', url = 'U', confirmation_number = 'C' WHERE id = ?",
      )
      .run(res.id);
    let current = svc.getReservation(String(res.id), String(trip.id))!;
    let { reservation } = svc.update(
      String(res.id),
      String(trip.id),
      {
        location: 'NewLoc',
  ...
  it('RESV-SVC-024: explicit day_id / end_day_id updates win over derivation; explicit null end clears', () => {
    ...
    let { reservation } = svc.update(
      String(res.id),
      String(trip.id),
      { day_id: days[1].id, end_day_id: days[2].id },
      current,
    );
    ...
    ({ reservation } = svc.update(
      String(res.id),
      String(trip.id),
      { end_day_id: null, reservation_end_time: '2030-05-02T18:00:00' },
      current,
    ));
    ...
    ({ reservation } = svc.update(
      String(res.id),
      String(trip.id),
      { reservation_end_time: '2030-05-02T18:00:00' },
      current,
    ));
  ...
  it('RESV-SVC-025: with metadata absent, the stored metadata JSON drives the accommodation sync (confirmation falls back to current)', () => {
    ...
    const res = createReservation(testDb, trip.id, { title: 'Hotel', type: 'hotel' });
    testDb
      .prepare("UPDATE reservations SET accommodation_id = ?, metadata = ?, confirmation_number = 'KEEP' WHERE id = ?")
  ...
  it('RESV-SVC-026: resync derives the end day too, keeping the stored one when the end time is out of range', () => {
    ...
    const res = createReservation(testDb, trip.id, { title: 'T', type: 'tour', day_id: days[0].id });
    testDb
      .prepare('UPDATE reservations SET reservation_time = ?, reservation_end_time = ?, end_day_id = ? WHERE id = ?')
    ...
    expect(testDb.prepare('SELECT day_id, end_day_id FROM reservations WHERE id = ?').get(res.id)).toEqual({
      day_id: days[1].id,
      end_day_id: days[0].id,
    });
...
describe('ReservationsService — quirk fixes (post-fold)', () => {
  ...
  it('RESV-FIX-002: update is atomic — a failing endpoint save rolls back the field update', () => {
    ...
    expect(() =>
      svc.update(String(res.id), String(trip.id), { title: 'New', endpoints: badEndpoints } as never, current),
    ).toThrow();
...
describe('ReservationsService — referenced ids stay inside the trip', () => {
  ...
  it('RESV-SCOPE-001: names every foreign id in the body', () => {
    ...
    const foreignDay = testDb.prepare('SELECT id FROM days WHERE trip_id = ? ORDER BY day_number').get(theirs.id) as {
      id: number;
    };
    const foreignAcc = createDayAccommodation(testDb, theirs.id, foreignPlace.id, foreignDay.id, foreignDay.id);

    expect(
      svc.referencesOutsideTrip(String(mine.id), {
        title: 'x',
        day_id: foreignDay.id,
        place_id: foreignPlace.id,
        accommodation_id: foreignAcc.id,
      }),
  ...
  it('RESV-SCOPE-005: a dangling id is not an offender, so the booking stays editable', () => {
    ...
    const day = testDb.prepare('SELECT id FROM days WHERE trip_id = ? ORDER BY day_number').get(mine.id) as {
      id: number;
    };
    ...
    expect(testDb.prepare('SELECT title, accommodation_id FROM reservations WHERE id = ?').get(res.id)).toEqual({
      title: 'Hotel renamed',
      accommodation_id: null,
    });
  });
  ...
  it('RESV-SCOPE-004: an update carrying a foreign accommodation_id does not write through to it', () => {
    ...
    svc.update(
      ...
      {
        accommodation_id: foreignAcc.id,
        metadata: { check_in_time: '23:00' },
      } as never,
      current,
    );

    expect(testDb.prepare('SELECT check_in FROM day_accommodations WHERE id = ?').get(foreignAcc.id)).toEqual({
      check_in: '14:00',
    });
    expect(testDb.prepare('SELECT accommodation_id FROM reservations WHERE id = ?').get(res.id)).toEqual({
      accommodation_id: null,
    });
  });
});
