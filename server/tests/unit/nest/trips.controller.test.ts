...
import type { TrekImportService } from '../../../src/nest/trips/trek-import.service';
...
const trekImportStub = {} as TrekImportService;
...
describe('TripsController (parity with the legacy /api/trips route)', () => {
  ...
  describe('POST / (create)', () => {
    it('403 without trip_create; a missing title 400s in the ZodValidationPipe', () => {
      expect(thrown(() => tc(svc({ can: vi.fn().mockReturnValue(false) })).create(user, { title: 'T' }, req))).toEqual({
        status: 403,
        body: { error: 'No permission to create trips' },
      });
    ...
    it('400 when end_date precedes start_date', () => {
      expect(
        thrown(() => tc(svc()).create(user, { title: 'T', start_date: '2026-07-10', end_date: '2026-07-01' }, req)),
      ).toEqual({
        status: 400,
    ...
    it('logs the reminder when reminderDays is set', () => {
      ...
      expect(tc(svc({ create } as Partial<TripsService>)).create(user, { title: 'T' }, req)).toEqual({
        trip: { id: 9 },
      });
    });
  });
  ...
  it('GET /:id 404 when missing', () => {
    expect(
      thrown(() => tc(svc({ get: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).get(user, '9')),
  ...
  describe('PUT /:id', () => {
    it('404 when no access; 403 on archive without trip_archive', async () => {
      expect(
        await thrownAsync(() =>
          tc(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).update(user, '9', {}, req),
        ),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
  ...
  describe('POST /:id/copy', () => {
    it('403 without trip_create, 404 without access', () => {
      expect(thrown(() => tc(svc({ can: vi.fn().mockReturnValue(false) })).copy(user, '9', {}, req))).toEqual({
        status: 403,
        body: { error: 'No permission to create trips' },
      });
      expect(
        thrown(() => tc(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).copy(user, '9', {}, req)),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
    });
  ...
  describe('DELETE /:id', () => {
    it('404 when no owner, 403 without trip_delete', () => {
      ...
      expect(thrown(() => tc(s).remove(user, '9', req))).toEqual({
        status: 403,
    ...
    it('404s for someone with no access, so the 403 is not an existence oracle', () => {
      const s = svc({
        getOwner: vi.fn().mockReturnValue({ user_id: 2 }),
        canAccessTrip: vi.fn().mockReturnValue(null),
        remove: vi.fn(),
      } as Partial<TripsService>);
  ...
  it('GET /:id/bundle 404 then aggregates', () => {
    expect(
      thrown(() => tc(svc({ get: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).bundle(user, '9')),
    ).toEqual({ status: 404, body: { error: 'Trip not found' } });
  ...
  describe('POST /:id/cover', () => {
    ...
    it('404 without access, 403 without permission, 404 raw trip, 400 no file, else commits + returns url', async () => {
      expect(
        await thrownAsync(() => tc(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).cover(user, '9', file)),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
  ...
  describe('GET /:id/export.ics', () => {
    ...
    it('404 without access, else sends the calendar with headers', () => {
      expect(
        thrown(() => tc(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).exportIcs(user, '9', makeRes())),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
    ...
    it('maps a NotFoundError from the export to 404 and re-throws others', () => {
      ...
      expect(thrown(() => tc(svc(), nf).exportIcs(user, '9', makeRes()))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const other = {
  ...
  it('POST /:id/copy maps a copy failure to 500', () => {
    ...
    expect(thrown(() => tc(s).copy(user, '9', {}, req))).toEqual({
      status: 500,
