...
describe('DayAssignmentsController (parity with the legacy day-assignments routes)', () => {
  ...
  it('404 day on GET', () => {
    ...
    expect(thrown(() => new DayAssignmentsController(s).list(user, '5', '9'))).toEqual({
      status: 404,
  ...
  it('PUT /reorder 404 day, else reorders + broadcasts', () => {
    expect(
      thrown(() =>
        ...
        ).reorder(user, '5', '3', { orderedIds: [1, 2] }),
      ),
...
describe('AssignmentOpsController (parity with the per-assignment op routes)', () => {
  it('PUT /:id/move 404 assignment, 404 target day, else moves', () => {
    ...
    const moveAssignment = vi.fn().mockReturnValue({ assignment: { id: 9 }, oldDayId: 3 });
    const broadcast = vi.fn();
    const reconcile = vi.fn();
    const s = svc({
      getAssignmentForTrip: vi.fn().mockReturnValue({ day_id: 3 }),
  ...
  it('GET /:id/participants 404 when the assignment is on another trip, else returns participants (access-only)', () => {
    expect(
      thrown(() =>
        new AssignmentOpsController(
          svc({ getAssignmentForTrip: vi.fn().mockReturnValue(undefined) } as Partial<AssignmentsService>),
        ).participants(user, '5', '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Assignment not found' } });
    const s = svc({
      getAssignmentForTrip: vi.fn().mockReturnValue({ id: 9 }),
      getParticipants: vi.fn().mockReturnValue([{ user_id: 2 }]),
    } as Partial<AssignmentsService>);
  ...
  it('PUT /:id/notes 404 missing, else updates + broadcasts (#2163)', () => {
    expect(
      thrown(() =>
        new AssignmentOpsController(
          svc({ getAssignmentForTrip: vi.fn().mockReturnValue(undefined) } as Partial<AssignmentsService>),
        ).notes(user, '5', '9', { notes: 'n' }),
      ),
    ).toEqual({ status: 404, body: { error: 'Assignment not found' } });
    const updateNotes = vi.fn().mockReturnValue({ id: 9, notes: 'Book the 10:00 entry' });
    const broadcast = vi.fn();
    const reconcile = vi.fn();
    const s = svc({
      getAssignmentForTrip: vi.fn().mockReturnValue({ id: 9 }),
    ...
    expect(new AssignmentOpsController(s).notes(user, '5', '9', { notes: 'Book the 10:00 entry' }, 'sock')).toEqual({
      assignment: { id: 9, notes: 'Book the 10:00 entry' },
    });
  ...
  it("PUT /:id/participants 404 on a foreign assignment, else sets + broadcasts (non-array bodies are the Zod pipe's 400, covered in e2e)", () => {
    ...
    expect(
      thrown(() =>
        new AssignmentOpsController(
          svc({
            getAssignmentForTrip: vi.fn().mockReturnValue(undefined),
            setParticipants,
          } as Partial<AssignmentsService>),
        ).setParticipants(user, '5', '9', { user_ids: [2] }),
      ),
    ).toEqual({ status: 404, body: { error: 'Assignment not found' } });
    ...
    expect(new AssignmentOpsController(s).setParticipants(user, '5', '9', { user_ids: [2] }, 'sock')).toEqual({
