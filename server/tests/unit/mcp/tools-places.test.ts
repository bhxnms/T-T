...
describe('Tool: search_place', () => {
  ...
  it('forwards locationBias and lang to the provider', async () => {
    ...
    await withHarness(user.id, async (h) => {
      ...
      expect(searchPlacesMock).toHaveBeenCalledWith(user.id, 'Central Station', 'ja', {
        lat: 35.6812,
        lng: 139.7671,
        radius: 8000,
      });
  ...
  it('accepts a bias without an explicit radius', async () => {
    ...
    await withHarness(user.id, async (h) => {
      ...
      expect(searchPlacesMock).toHaveBeenCalledWith(user.id, 'Museum of Modern Art', undefined, {
        lat: 40.7614,
        lng: -73.9776,
      });
    });
