...
describe('Tool: create_packing_bag', () => {
  ...
  it('creates a bag with a weight limit (#2154)', async () => {
    ...
    await withHarness(user.id, async (h) => {
      ...
      const row = testDb.prepare('SELECT weight_limit_grams FROM packing_bags WHERE id = ?').get(data.bag.id) as {
...
describe('Tool: delete_packing_bag', () => {
  ...
  it('pings the room to re-read the bag weights (#2191)', async () => {
    ...
    const r = testDb
      .prepare('INSERT INTO packing_bags (trip_id, name, color) VALUES (?, ?, ?)')
      .run(trip.id, 'Carry-on', '#000000');
    const bagId = r.lastInsertRowid as number;
    testDb
      .prepare(
        'INSERT INTO packing_items (trip_id, name, category, checked, bag_id, weight_grams) VALUES (?, ?, ?, 0, ?, ?)',
      )
    ...
    await withHarness(user.id, async (h) => {
      ...
      expect(testDb.prepare('SELECT bag_id FROM packing_items WHERE trip_id = ?').get(trip.id)).toEqual({
        bag_id: null,
      });
    });
  });
...
describe('Tool: apply_packing_template', () => {
  it('applies a template and pings the bag weights once (#2191)', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    const template = testDb
      .prepare("INSERT INTO packing_templates (name, created_by) VALUES ('Hiking', ?)")
      .run(user.id);
    const category = testDb
      .prepare("INSERT INTO packing_template_categories (template_id, name, sort_order) VALUES (?, 'Gear', 0)")
      .run(template.lastInsertRowid);
    testDb
      .prepare("INSERT INTO packing_template_items (category_id, name, sort_order) VALUES (?, 'Boots', 0)")
