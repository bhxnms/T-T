...
function insertExistingUser(db: Database.Database): void {
  db.prepare(
    "INSERT INTO users (username, email, password_hash, role) VALUES ('admin', 'admin@tt.local', 'x', 'admin')",
  ).run();
}
...
describe('seedAdminAccount — first-run admin', () => {
  ...
  it('warns about a partial config and falls back to a generated password', () => {
    ...
    expect(db.prepare('SELECT 1 FROM users WHERE email = ?').get('admin@tt.local')).toBeDefined();
