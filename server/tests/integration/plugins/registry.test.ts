...
beforeEach(() => {
  ...
  vi.stubGlobal(
    'fetch',
    vi.fn(async () => ({ ok: true, json: async () => REGISTRY }) as unknown as Response),
  );
...
describe('PluginRegistryService', () => {
  ...
  describe('recomputeUpdateHold', () => {
    ...
    const seedRow = (hold = 0) =>
      testDb
        .prepare("INSERT INTO plugins (id, status, enabled, update_hold) VALUES ('flight-tracker','inactive',0,?)")
        .run(hold);
    const holdInDb = () =>
      (testDb.prepare("SELECT update_hold FROM plugins WHERE id='flight-tracker'").get() as { update_hold: number })
        .update_hold;

    beforeEach(() => {
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => ({ ok: true, json: async () => twoVersions }) as unknown as Response),
      );
      __clearRegistryCacheForTests();
    });
    ...
    it('does not hold when a newer version exists but this TREK cannot run it', async () => {
      ...
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => ({ ok: true, json: async () => withIncompatibleNewest }) as unknown as Response),
      );
      __clearRegistryCacheForTests();
      try {
        seedRow(1);
  ...
  it('installs a signed plugin and pins the author key (TOFU)', async () => {
    ...
    await expect(svc.install('flight-tracker')).resolves.toEqual({
      id: 'flight-tracker',
      version: '1.0.0',
      trekRangeBypassed: null,
    });
  ...
  describe('range bypass (TREK_PLUGINS_IGNORE_TREK_RANGE)', () => {
    ...
    it('install-latest fetches the newest PUBLISHED version and reports the ignored range', async () => {
      ...
      await expect(svc.install('multi')).resolves.toEqual({
        id: 'multi',
        version: '2.1.0',
        trekRangeBypassed: { trekRange: '>=3.4.0 <4.0.0', hostVersion: '3.3.0' },
      });
    ...
    it("re-gate on the ARTIFACT's range warns instead of refusing, with the artifact's range", async () => {
      ...
      await expect(svc.install('flight-tracker')).resolves.toMatchObject({
        id: 'flight-tracker',
        trekRangeBypassed: { trekRange: '>=3.2.0 <3.4.0', hostVersion: '3.9.0' },
      });
    ...
    it('sideload: stages an incompatible or rangeless archive and reports it', () => {
      process.env.APP_VERSION = '3.9.0';
      const old = svc.stageUpload(
        makeArtifact({ id: 'my-upload', name: 'Up', version: '1.0.0', type: 'integration', trek: '>=3.0.0 <3.5.0' }),
      );
      expect(old).toMatchObject({
        id: 'my-upload',
        trekRangeBypassed: { trekRange: '>=3.0.0 <3.5.0', hostVersion: '3.9.0' },
      });
      fs.rmSync(old.stagingDir, { recursive: true, force: true });
      const rangeless = svc.stageUpload(
        makeArtifact({ id: 'my-upload', name: 'Up', version: '1.0.0', type: 'integration', trek: undefined }),
      );
      ...
      const fits = svc.stageUpload(
        makeArtifact({ id: 'my-upload', name: 'Up', version: '1.0.0', type: 'integration' }),
      );
