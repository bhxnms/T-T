...
function seedAdminAccount(db: Database.Database): void {
  try {
    ...
    // Demo mode seeds its own separate demo admin account right after this.
    ...
    if (env_admin_email && env_admin_pw) {
      ...
      if (adminEnvProvided) {
        console.warn('[admin] Only one of ADMIN_EMAIL/ADMIN_PASSWORD is set — both are required for a custom admin. Falling back to admin@tt.local with a generated password (shown below).');
      }
    ...
    console.log('');
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║  TT — First Run: Admin Account Created       ║');
    console.log('╠══════════════════════════════════════════════╣');
    console.log(`║  Email:    ${email.padEnd(33)}║`);
    console.log(`║  Password: ${password.padEnd(33)}║`);
    console.log('╠══════════════════════════════════════════════╣');
    console.log('║  本项目基于 TREK 开源项目改进而来           ║');
    console.log('║  TREK: github.com/Travels-Desk/trek          ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
  } catch (err: unknown) {
