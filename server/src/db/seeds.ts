...
function seedAdminAccount(db: Database.Database): void {
  try {
    ...
    if (env_admin_email && env_admin_pw) {
      ...
      if (adminEnvProvided) {
        console.warn(
          '[admin] Only one of ADMIN_EMAIL/ADMIN_PASSWORD is set — both are required for a custom admin. Falling back to admin@tt.local with a generated password (shown below).',
        );
      }
