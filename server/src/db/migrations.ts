...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Migration[] = [
    ...
    () => {
      ...
      if (!hasColumn('collections', 'links')) db.exec('ALTER TABLE collections ADD COLUMN links TEXT');
      if (!hasColumn('collection_places', 'links')) db.exec('ALTER TABLE collection_places ADD COLUMN links TEXT');
    },
    ...
    () => {
      const hasRole = db.prepare("SELECT 1 FROM pragma_table_info('collection_members') WHERE name = 'role'").get();
      if (!hasRole) db.exec("ALTER TABLE collection_members ADD COLUMN role TEXT NOT NULL DEFAULT 'editor'");
    },
    ...
    /**
     * TT: Activities table for unified day planning (#task-3)
     *
     * Creates a new `activities` table to unify day planning with places and reservations.
     * This enables bidirectional sync between the day planner and places sidebar.
     *
     * Structure:
     * - Combines places and reservations into a single activity stream per day
     * - Supports drag-and-drop ordering via order_index
     * - Links to either a place_id or reservation_id (one must be set)
     * - Tracks start_time and duration for timeline visualization
     *
     * Migration strategy:
     * - Existing day_assignments are migrated to activities (linked to places)
     * - Existing reservations can be optionally linked to days via activities
     */
    () => {
      // Check if activities table already exists
      const tableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='activities'").get();
      ...
      if (!tableExists) {
        db.exec(`
          CREATE TABLE activities (
            ...
            updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            CHECK (
              (place_id IS NOT NULL AND reservation_id IS NULL) OR
              (place_id IS NULL AND reservation_id IS NOT NULL)
            )
          );
          ...
          CREATE INDEX idx_activities_reservation ON activities(reservation_id);
        `);

        // Migrate existing day_assignments to activities
        db.exec(`
          INSERT INTO activities (day_id, trip_id, place_id, order_index, start_time, duration_minutes, created_at)
          SELECT
            da.day_id,
            d.trip_id,
            da.place_id,
          ...
          LEFT JOIN places p ON da.place_id = p.id
