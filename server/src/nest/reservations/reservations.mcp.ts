...
const legsSchema = transportLegsInputSchema
  .optional()
  .describe(
...
function applyLegs(plan: LegPlan): LegOutcome | { error: string } {
  ...
  for (let i = 0; i < legs.length; i++) {
    for (const field of ['dep_day_id', 'arr_day_id'] as const) {
      ...
      if (dayId != null && !lookupDay(dayId)) return { error: `legs[${i}].${field} does not belong to this trip.` };
    }
  }
...
export class ReservationsMcp {
  ...
  async createReservation(
    ...
    if (day_id && !this.days.getDay(day_id, tripId)) return errorResult('day_id does not belong to this trip.');
  ...
  async updateReservation(
    ...
    const { reservation } = this.reservations.update(
      reservationId,
      tripId,
      {
        title,
  ...
  async reorderReservations(
    ...
    if (dayId && !this.days.getDay(dayId, tripId)) return errorResult('dayId does not belong to this trip.');
  ...
  async linkHotelAccommodation(
    ...
    if (!this.assignments.placeExists(place_id, tripId)) return errorResult('place_id does not belong to this trip.');
    if (!this.days.getDay(start_day_id, tripId)) return errorResult('start_day_id does not belong to this trip.');
    if (!this.days.getDay(end_day_id, tripId)) return errorResult('end_day_id does not belong to this trip.');
  ...
  async updateTransport(
    ...
    if (legs !== undefined) {
      ...
      const applied = applyLegs({
        ...
        endpoints:
          resolvedEndpoints ??
          (this.reservations.getReservationWithJoins(reservationId)?.endpoints ?? []).map((e) => ({
            role: e.role,
            sequence: e.sequence,
            name: e.name,
            code: e.code,
