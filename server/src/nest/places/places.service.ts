...
export class PlacesService {
  ...
  async importGoogleList(
    ...
    // Only Google Maps links are accepted by this importer.
    let parsedUrl: URL;
    try {
    ...
    const isGoogleHost =
      host === 'google.com' || host.endsWith('.google.com') || host === 'goo.gl' || host.endsWith('.goo.gl');
    if (!isGoogleHost) return { error: 'URL is not allowed', status: 400 };
  ...
  async importNaverList(
    ...
    // Only Naver Maps links are accepted by this importer.
    let initialUrl: URL;
    try {
    ...
    if (!(initialHost === 'naver.com' || initialHost.endsWith('.naver.com') || initialHost === 'naver.me')) {
      return { error: 'URL is not allowed', status: 400 };
    }
