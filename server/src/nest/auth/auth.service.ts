...
export class AuthService {
  ...
  getAppConfig(authenticatedUser: User | undefined | null) {
    ...
    const placesEnrichSetting = this.db.get<{ value: string }>(
    ...
    return {
      ...
      // AMap (高德) place search: only *usable* when both the instance switch
      // is on and a key is stored. The client renders the provider toggle from
      // this pair, and route profile 'amap' is offered when `has_amap_key`
      // holds even if the search switch is off — routing and search are
      // independent features.
      has_amap_key: !!getAmapKey(this.db),
      // The JS SDK key is necessarily public in the browser. Prefer the
      // operator's restricted AMAP_JS_API_KEY; fall back to the existing
      // instance key for self-hosted installs that use one key for both APIs.
      amap_js_key: readEnv().maps.amapJsKey || undefined,
      amap_search_enabled:
        this.db.get<{ value: string }>("SELECT value FROM app_settings WHERE key = 'amap_search_enabled'")?.value ===
        'true',
