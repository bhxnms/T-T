...
export class UserProfileService {
  ...
  private storedKeyPlaintext(
    ...
    // AMap has no users column at all: the instance value is the only value, so
    // it is also what a save is compared against.
    if (name === 'amap_api_key') return readInstanceApiKey(this.db, 'amap_api_key') ?? '';
  ...
  updateApiKeys(userId: number, rawBody: unknown) {
    ...
    this.db.transaction(() => {
      ...
      // AMap is instance-only — there is no per-user column for it. An admin
      // save lands in app_settings through the same writeInstanceApiKey path
      // the two mirrored names use; a non-admin's amap_api_key is ignored.
      if (isAdmin && body.amap_api_key !== undefined) {
        writeInstanceApiKey(this.db, 'amap_api_key' as InstanceApiKeyName, body.amap_api_key);
      }
    });
  ...
  getSettings(userId: number): { error?: string; status?: number; settings?: Record<string, unknown> } {
    ...
    // AMap is instance-only, so it reads straight from app_settings.
    return {
      settings: {
