...
export class TodoService {
  ...
  createItem(
    ...
    if (data.assigned_user_id != null && !this.db.rosterUserIds(tripId).has(data.assigned_user_id)) {
      throw new Error('Assigned user is not a member of this trip');
    }
  ...
  updateItem(
    ...
    if (
      bodyKeys.includes('assigned_user_id') &&
      data.assigned_user_id != null &&
      !this.db.rosterUserIds(tripId).has(data.assigned_user_id)
    ) {
      throw new Error('Assigned user is not a member of this trip');
    }
