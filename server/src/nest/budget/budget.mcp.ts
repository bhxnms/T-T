...
export class BudgetMcp {
  ...
  async createBudgetItem(
    ...
    if (place_id != null && !this.placeOnTrip(tripId, place_id))
      return errorResult('place_id does not belong to this trip.');
    if (members !== undefined) {
      const refusal = this.splitRefusal(
        tripId,
        members,
        this.settledTotalCents(tripId, payers, total_price, toCents(total_price)),
        payers,
      );
  ...
  async updateBudgetItem(
    ...
    if (members !== undefined) {
      ...
      const refusal = this.splitRefusal(
        tripId,
        members,
        this.settledTotalCents(tripId, payers, total_price, toCents(existing.total_price)),
        payers,
      );
      if (refusal) return errorResult(refusal);
    }
  ...
  async createSettlement(
    {
      ...
      amount,
      currency,
    }: { tripId: number; from_user_id: number; to_user_id: number; amount: number; currency?: string | null },
    ctx: McpContext,
  ) {
    if (this.isDemoUser(ctx.userId)) return demoDenied();
    if (!this.budget.verifyTripAccess(tripId, ctx.userId)) return noAccess();
