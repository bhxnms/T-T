...
export class JourneyShareService {
  ...
  createOrUpdateJourneyShareLink(
    ...
    const existing = this.db
      ...
      .get(journeyId) as
      | { token: string; share_timeline: number; share_gallery: number; share_map: number; newest_first: number }
      | undefined;
    ...
    if (existing) {
      ...
      this.db
        .prepare(
          'UPDATE journey_share_tokens SET share_timeline = ?, share_gallery = ?, share_map = ?, newest_first = ? WHERE journey_id = ?',
        )
    ...
    this.db
      .prepare(
        'INSERT INTO journey_share_tokens (journey_id, token, created_by, share_timeline, share_gallery, share_map, newest_first) VALUES (?, ?, ?, ?, ?, ?, ?)',
      )
      .run(
        journeyId,
        token,
