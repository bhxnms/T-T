...
export class UnifiedMemoriesService {
  ...
  private _providers(): Array<{ id: string; enabled: boolean }> {
    ...
    const rows = this.db.prepare('SELECT id, enabled FROM photo_providers').all() as Array<{
      id: string;
      enabled: number;
    }>;
  ...
  listTripPhotos(tripId: string, userId: number): ServiceResult<any[]> {
    ...
    try {
      const enabledProviders = this._providers()
        .filter((p) => p.enabled)
        .map((p) => p.id);
  ...
  async addTripPhotos(
    ...
    for (const selection of selections) {
      ...
      for (const raw of selection.asset_ids) {
        ...
        if (!assetId) continue;
        const result = this._addTripPhoto(
          tripId,
          userId,
          selection.provider,
          assetId,
  ...
  removeTripPhoto(tripId: string, userId: number, photoId: number, sid?: string): ServiceResult<true> {
    const access = this.db.canAccessTrip(tripId, userId);
    if (!access) {
      return fail('Trip not found or access denied', 404);
    }
  ...
  removeAlbumLink(tripId: string, linkId: string, userId: number): ServiceResult<true> {
    ...
    try {
      const linkedPhotos = this.db
        .prepare('SELECT photo_id FROM trip_photos WHERE trip_id = ? AND album_link_id = ?')
        .all(tripId, linkId) as Array<{ photo_id: number }>;

      this.db.transaction(() => {
        this.db.prepare('DELETE FROM trip_photos WHERE trip_id = ? AND album_link_id = ?').run(tripId, linkId);
        this.db
          .prepare('DELETE FROM trip_album_links WHERE id = ? AND trip_id = ? AND user_id = ?')
  ...
  private async _notifySharedTripPhotos(
    ...
    try {
      const actorRow = this.db.prepare('SELECT username, email FROM users WHERE id = ?').get(actorUserId) as {
