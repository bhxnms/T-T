...
function assertValidRedirectUris(uris: string[]): void {
  for (const u of uris) {
    ...
    if (verdict === 'malformed') throw new InvalidClientMetadataError(`Invalid redirect URI: ${u}`);
    if (verdict === 'dangerous') throw new InvalidClientMetadataError(`Dangerous redirect URI scheme: ${u}`);
