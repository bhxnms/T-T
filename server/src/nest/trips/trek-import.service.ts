...
export class TrekImportService implements OnModuleDestroy {
  ...
  async previewFromZip(zipPath: string): Promise<{ token: string; trips: TrekTripSummary[] }> {
    ...
    try {
      ...
      const tables = new Set(
        (src.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as { name: string }[]).map((t) => t.name),
      );
      for (const required of ['trips', 'days', 'places']) {
        if (!tables.has(required)) throw new Error(`Invalid backup: missing table ${required}`);
      }
      const trips = src.prepare('SELECT * FROM trips ORDER BY start_date DESC, id DESC').all() as Record<string, unknown>[];
      const summaries = trips.map((t) => {
        ...
        return {
          ...
          budget_count: tables.has('budget_items') ? count('SELECT COUNT(*) AS n FROM budget_items WHERE trip_id = ?') : 0,
        };
      });
  ...
  importTrips(userId: number, token: string, tripIds: number[]): { imported: { id: number; title: string }[] } {
    ...
    if (!entry || entry.expiresAt < Date.now()) {
      if (entry) { this.previews.delete(token); this.rmDir(entry.dir); }
    ...
    try {
      ...
      for (const oldTripId of tripIds) {
        const newId = this.db.transaction(() =>
          this.importOneTrip(userId, src, srcTables, Number(oldTripId)),
        );
  ...
  private importOneTrip(userId: number, src: InstanceType<typeof Database>, srcTables: Set<string>, oldTripId: number): number {
    const srcTrip = src.prepare('SELECT * FROM trips WHERE id = ?').get(oldTripId) as Record<string, unknown> | undefined;
    ...
    if (tripCols.includes('feed_token') && srcTrip.feed_token != null) tripRow.feed_token = crypto.randomBytes(16).toString('hex');
    ...
    const reservationMap = this.copyScoped(src, srcTables, 'reservations', {
      trip_id: tripOf,
      day_id: dayOf,
      end_day_id: dayOf,
      place_id: placeOf,
    ...
    this.copyScoped(src, srcTables, 'day_notes', { trip_id: tripOf, day_id: dayOf }, `WHERE trip_id = ${Number(oldTripId)}`);
    ...
    this.copyScoped(src, srcTables, 'photos', { trip_id: tripOf, day_id: dayOf, place_id: placeOf }, `WHERE trip_id = ${Number(oldTripId)}`);
    this.copyScoped(src, srcTables, 'trip_files', { trip_id: tripOf, place_id: placeOf, reservation_id: reservationOf }, `WHERE trip_id = ${Number(oldTripId)}`);
    this.copyScoped(src, srcTables, 'day_accommodations', { trip_id: tripOf, place_id: placeOf, start_day_id: dayOf, end_day_id: dayOf }, `WHERE trip_id = ${Number(oldTripId)}`);
    ...
    if (srcTables.has('place_ratings') && placeMap.size > 0) {
      ...
      for (const r of src.prepare(`SELECT * FROM place_ratings WHERE place_id IN (${ids})`).all() as Record<string, unknown>[]) {
    ...
    if (srcTables.has('place_tags') && placeMap.size > 0) {
      ...
      const ids = [...placeMap.keys()].join(',');
      const rels = src.prepare(`SELECT * FROM place_tags WHERE place_id IN (${ids})`).all() as Record<string, unknown>[];
      for (const rel of rels) {
        ...
        if (tagId === undefined) {
          const srcTag = src.prepare('SELECT * FROM tags WHERE id = ?').get(oldTagId) as Record<string, unknown> | undefined;
          const existing = srcTag
            ? this.db.get<{ id: number }>('SELECT id FROM tags WHERE user_id = ? AND name = ?', userId, String(srcTag.name ?? ''))
            : undefined;
          const tagCols = this.commonCols('tags', src, 'tags', ['id', 'user_id']);
          tagId = existing?.id
            ?? this.insertRow('tags', [...tagCols, 'user_id'], { ...srcTag, user_id: userId });
  ...
  private commonCols(ttTable: string, src: InstanceType<typeof Database>, srcTable: string, skip: string[]): string[] {
    const srcCols = new Set((src.prepare(`PRAGMA table_info('${srcTable}')`).all() as { name: string }[]).map((c) => c.name));
  ...
  private tableInfo(table: string) {
    if (!this.pragmaCache.has(table)) {
      this.pragmaCache.set(
        table,
        this.db.prepare(`PRAGMA table_info('${table}')`).all() as { name: string; type: string; notnull: number; dflt_value: string | null }[],
      );
    }
  ...
  private mapCategory(userId: number, src: InstanceType<typeof Database>, oldCatId: number): number {
    const srcCat = src.prepare('SELECT * FROM categories WHERE id = ?').get(oldCatId) as Record<string, unknown> | undefined;
    if (srcCat) {
      const existing = this.db.get('SELECT id FROM categories WHERE user_id = ? AND name = ?', userId, String(srcCat.name ?? ''));
    ...
    return this.insertRow('categories', ['name', 'color', 'icon', 'user_id'], { name: 'Imported', color: '#6366f1', icon: '📍', user_id: userId });
  }
  ...
  private async extractZip(zipPath: string, outDir: string): Promise<void> {
    ...
    for (const entry of archive.files) {
      ...
      if (rel.startsWith('..') || path.isAbsolute(rel)) throw new Error('Invalid backup: entry path escapes the archive root');
