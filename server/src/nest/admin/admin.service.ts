...
export class AdminService {
  ...
  async getGithubReleases(perPage: string = '10', page: string = '1') {
    ...
    const data = await this.fetchGithub(`https://api.github.com/repos/bhxnms/T-T/releases?${qs}`);
    return Array.isArray(data) ? data : [];
  }

  async checkVersion(): Promise<VersionInfo> {
    const currentVersion: string = readEnv().app.appVersion || '0.6.0';
    ...
    const data = await this.fetchGithub('https://api.github.com/repos/bhxnms/T-T/releases/latest');
    type Release = { tag_name?: unknown; html_url?: unknown; prerelease?: unknown; draft?: unknown };
    const latest = data && !Array.isArray(data) ? (data as Release) : null;
    ...
    if (
      ...
      !/^v?\d+(?:\.\d+){1,2}$/.test(latestTag)
    ) {
      writeVersionCache(fallback);
      return fallback;
    }
    ...
    const result: VersionInfo = {
      ...
      update_available: compareVersions(latestVersion, currentVersion.replace(/^v/, '')) > 0,
      release_url: typeof latest.html_url === 'string' ? latest.html_url : undefined,
      is_docker: isDocker,
      is_prerelease: isPrerelease,
    };
