...
export class AdminService {
  ...
  async checkVersion(): Promise<VersionInfo> {
    ...
    // TT is a fork, not a TREK instance: the upstream release channel
    // (liketrek/TREK on GitHub) must never surface "update available" here —
    // neither in the admin banner nor as a notification. Answered inline with
    // "current is latest" so the admin UI keeps its data shape and shows
    // nothing. Plugin host-compat keeps its own TREK baseline on purpose; that
    // is a capability shim, not an update channel.
    return {
