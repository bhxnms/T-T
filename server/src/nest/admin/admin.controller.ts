...
export class AdminController {
  ...
  @Get('invites')
  listInvites() {
    return { invites: this.invites.listInvites() };
  }

  // Trips an admin can optionally bind a registration invite to (#1402).
  @Get('invites/trips')
  listInviteTrips() {
    return { trips: this.invites.listTripsForInvite() };
  }
  ...
  @Get('amap-search')
  getAmapSearch() {
    return this.addons.getAmapSearch();
  }

  @Put('amap-search')
  updateAmapSearch(@CurrentUser() user: User, @Body() body: AdminFeatureToggleDto, @Req() req: Request) {
    const result = this.addons.updateAmapSearch(body.enabled);
    this.audit.writeAudit({
      userId: user.id,
      action: 'admin.amap_search',
      ip: getClientIp(req),
      details: { enabled: result.enabled },
    });
    return result;
  ...
  @Get('mcp-tokens')
  listMcpTokens() {
    return { tokens: this.tokens.listAllMcpTokens() };
  }
  ...
  @Get('oauth-sessions')
  listOAuthSessions() {
    return { sessions: this.oauth.listAllOAuthSessions() };
  }
