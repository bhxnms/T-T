...
// The range is split into independent escapes so the class cannot be interpreted as a combined character sequence.
const EMOJI_RE =
  /\p{Emoji_Presentation}|\p{Emoji_Modifier}|\p{Regional_Indicator}|\u200D|\uFE00-\uFE0F|\u20E3|[\u{E0020}-\u{E007F}]/gu;

/** Remove emojis from a display string and tidy the whitespace they leave behind. */
export function stripEmoji(s: string): string {
  const stripped = s.replace(EMOJI_RE, '').replace(/[\uFE00-\uFE0F\u20E3]/g, '');
