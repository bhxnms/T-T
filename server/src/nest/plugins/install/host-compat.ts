...
const DEFAULT_PLUGIN_HOST_VERSION = '4.2.1';

function isTourismTeamVersion(version: string): boolean {
  return /^0\.\d+\.\d+(?:[-+].*)?$/.test(version);
}

/** The running TREK version used by plugin compatibility checks. */
export function hostVersion(): string {
  const configured = readEnv().plugins.pluginHostVersion?.trim();
  const appVersion = readEnv().app.appVersion || (require('../../../../package.json') as { version: string }).version;
  // Tourism-Team keeps its public 0.x version independent from the TREK plugin
  // host contract. Explicit configuration always wins; all TT 0.x builds use the
  // compatibility baseline unless an operator pins a different host surface.
  return configured || (isTourismTeamVersion(appVersion) ? DEFAULT_PLUGIN_HOST_VERSION : appVersion);
}
