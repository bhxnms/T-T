...
// google_place_id, AMap POI ids ("amap:B0FF…" — resolved against restapi.amap.com,
// never Google), the raw photo URLs legacy rows keep in image_url, and photo
...
const NON_GOOGLE_PLACE_ID = /^(?:coords|node|way|relation|amap):|^https?:\/\/|^-?\d+(?:\.\d+)?,\s*-?\d+(?:\.\d+)?$|~p\d+$/i;
