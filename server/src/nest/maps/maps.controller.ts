...
import { MapsSearchDto, MapsAutocompleteDto, MapsResolveUrlDto, MapsAmapRouteDto } from './maps.dto';
...
export class MapsController {
  ...
  async amapRoute(
    @CurrentUser() user: User,
    @Body() body: MapsAmapRouteDto,
  ) {
    const profile = body.profile === 'walking' || body.profile === 'cycling' ? body.profile : 'driving';
    try {
      return await this.maps.amapRoute(profile, body.waypoints);
    } catch (err: unknown) {
      throw toHttpException(err, 'Route error', 502);
    }
  }

  @Post('autocomplete')
  @HttpCode(200)
  async autocomplete(    @CurrentUser() user: User,
    ...
    try {
      return await this.maps.autocomplete(user.id, body.input, body.lang, body.locationBias, body.sessionToken, body.provider);
    } catch (err: unknown) {
