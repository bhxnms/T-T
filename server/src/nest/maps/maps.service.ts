...
export class MapsService {
  ...
  /**
   * AMap routing for the day's route line: real traffic-aware durations fetched
   * server-side, so the browser never needs a restapi.amap.com CSP allowance or
   * sees the key. Coordinates in and out are WGS-84; conversion lives in the
  ...
  async amapRoute(profile: 'driving' | 'walking' | 'cycling', waypoints: { lat: number; lng: number }[]) {
    ...
    return amapRoute(this.database, profile, waypoints);
  }
  ...
  async searchNominatim(query: string, lang?: string, lane: GeoLane = 'interactive') {
    try {
      return await this.searchNominatimOnce(query, lang, lane);
    } catch (err) {
      // Nominatim unreachable (a "fetch failed" before any HTTP status — DNS
      // pollution/blocking kills the host outright in some networks) or erroring:
      // degrade to Photon, which serves the same OSM data from a different
      // endpoint. A Photon miss rethrows the original error; an empty answer is
      // a real "no results".
      const places = await photonSearch(query, lang);
      if (!places) throw err;
      return places.map((item) => {
        ...
        return {
          ...
          google_ftid: null,
          osm_id: item.osm_id ? `${item.osm_type}:${item.osm_id}` : null,
  ...
  private async searchNominatimOnce(query: string, lang?: string, lane: GeoLane = 'interactive') {
  ...
  async searchPlaces(
    ...
    // AMap (高德): an explicit 'amap' asks for it, an omitted provider follows
    // the instance switch; 'native' always forces the native stack. Both AMap
    // branches still require a stored key — a stale client toggle (or the
    // switch left on after the key was removed) degrades to the native
    ...
    if (
      ...
      try {
        ...
        console.error('[Maps] AMap search failed, falling back:', err instanceof Error ? err.message : err);
      }
    }
  ...
  async autocompletePlaces(
    ...
    if (provider === 'amap' || (provider === undefined && isAmapSearchEnabled(this.database))) {
      if (getAmapKey(this.database)) {
        try {
          ...
          console.error('[Maps] AMap autocomplete failed, falling back:', err instanceof Error ? err.message : err);
        }
      }
    }
