...
export class VacayService {
  ...
  getYearSettings(userId: number): VacayUserSettings {
    return (
      this.getUserYearSettings(userId) ?? {
        user_id: userId,
        year_type: 'calendar',
        year_start_month: 1,
        year_start_day: 1,
  ...
  getOwnPlan(userId: number): VacayPlan {
    ...
    if (!plan) {
      ...
      this.db.run(
        'INSERT OR IGNORE INTO vacay_user_colors (user_id, plan_id, color) VALUES (?, ?, ?)',
        userId,
        plan.id,
        '#6366f1',
      );
  ...
  shiftOwnerEntriesForTripWindow(ownerId: number, oldStart: string, oldEnd: string, newStart: string): void {
  ...
  async applyHolidayCalendars(planId: number): Promise<void> {
    const plan = this.db.get<{ holidays_enabled: number }>(
      'SELECT holidays_enabled FROM vacay_plans WHERE id = ?',
      planId,
    );
    if (!plan?.holidays_enabled) return;
    const calendars = this.db.all<VacayHolidayCalendar>(
      "SELECT * FROM vacay_holiday_calendars WHERE plan_id = ? AND type = 'public_holiday' ORDER BY sort_order, id",
      planId,
    );
  ...
  async updatePlan(planId: number, body: UpdatePlanBody, socketId: string | undefined) {
    ...
    if (school_holidays_enabled !== undefined) {
      updates.push('school_holidays_enabled = ?');
      params.push(school_holidays_enabled ? 1 : 0);
    }
    ...
    if (company_holidays_enabled === true) {
      const companyDates = this.db.all<{ date: string }>(
        'SELECT date FROM vacay_company_holidays WHERE plan_id = ?',
        planId,
      );
    ...
    // Turning public holidays on with no calendar yet seeds a China calendar:
    // the default audience expects 国庆/春节/中秋 to simply be there rather
    // than having to know that calendars are a separate add-step. A calendar
    // the user added themselves (any region) means a deliberate choice — never
    // seeded over. Runs server-side so desktop and mobile pick it up alike.
    if (holidays_enabled === true) {
      const hasPublicCalendar = this.db.get(
        "SELECT id FROM vacay_holiday_calendars WHERE plan_id = ? AND (type = 'public_holiday' OR type IS NULL)",
        planId,
      );
      if (!hasPublicCalendar) {
        this.db.run(
          "INSERT INTO vacay_holiday_calendars (plan_id, type, region, label, color, sort_order) VALUES (?, 'public_holiday', 'CN', NULL, '#fecaca', 0)",
          planId,
        );
      }
    }
    ...
    if (carry_over_enabled === true) {
      ...
      this.db.transaction(() => {
        const years = this.db.all<{ year: number }>(
        ...
        for (let i = 0; i < years.length - 1; i++) {
          ...
          for (const u of users) {
            ...
            const config = this.db.get<VacayUserYear>(
              'SELECT * FROM vacay_user_years WHERE user_id = ? AND plan_id = ? AND year = ?',
              u.id,
              planId,
              yr,
            );
    ...
    const updatedCalendars = this.db.all<VacayHolidayCalendar>(
      'SELECT * FROM vacay_holiday_calendars WHERE plan_id = ? ORDER BY sort_order, id',
      planId,
    );
    return {
      plan: {
  ...
  addHolidayCalendar(
    ...
    const cal = this.db.get<VacayHolidayCalendar>(
      'SELECT * FROM vacay_holiday_calendars WHERE id = ?',
  ...
  updateHolidayCalendar(
    ...
    const cal = this.db.get<VacayHolidayCalendar>(
      'SELECT * FROM vacay_holiday_calendars WHERE id = ? AND plan_id = ?',
      calId,
      planId,
    );
    if (!cal) return null;
  ...
  sendInvite(
    ...
    const targetUser = this.db.get(
      'SELECT id, username FROM users WHERE id = ? AND COALESCE(is_guest, 0) = 0',
      targetUserId,
    );
    if (!targetUser) return { error: 'User not found', status: 404 };

    const existing = this.db.get<{ id: number; status: string }>(
      'SELECT id, status FROM vacay_plan_members WHERE plan_id = ? AND user_id = ?',
      planId,
      targetUserId,
    );
    if (existing) {
    ...
    const targetFusion = this.db.get(
      "SELECT id FROM vacay_plan_members WHERE user_id = ? AND status = 'accepted'",
      targetUserId,
    );
  ...
  acceptInvite(userId: number, planId: number, socketId: string | undefined): { error?: string; status?: number } {
    ...
    const result = this.db.transaction((): { error?: string; status?: number } => {
      const invite = this.db.get<VacayPlanMember>(
        "SELECT * FROM vacay_plan_members WHERE plan_id = ? AND user_id = ? AND status = 'pending'",
        planId,
        userId,
      );
      ...
      if (ownPlan && ownPlan.id !== planId) {
        ...
        const ownYears = this.db.all<VacayUserYear>(
          'SELECT * FROM vacay_user_years WHERE user_id = ? AND plan_id = ?',
          userId,
          ownPlan.id,
        );
        ...
        const colorRow = this.db.get<{ color: string }>(
          'SELECT color FROM vacay_user_colors WHERE user_id = ? AND plan_id = ?',
          userId,
          ownPlan.id,
        );
        if (colorRow) {
      ...
      const existingColors = this.db
        .all<{
          color: string;
        }>('SELECT color FROM vacay_user_colors WHERE plan_id = ? AND user_id != ?', planId, userId)
        .map((r) => r.color);
      const myColor = this.db.get<{ color: string }>(
        'SELECT color FROM vacay_user_colors WHERE user_id = ? AND plan_id = ?',
        userId,
        planId,
      );
  ...
  dissolvePlan(userId: number, socketId: string | undefined): void {
    ...
    const allUserIds = this.db.transaction(() => {
      ...
      const companyHolidays = this.db.all<{ date: string; note: string }>(
        'SELECT date, note FROM vacay_company_holidays WHERE plan_id = ?',
        plan.id,
      );
  ...
  `,
    ...
    const incoming = incomingRows
      .filter((s) => !coMembers.has(s.owner_id))
      .map((s) => ({
        id: s.id,
        owner_id: s.owner_id,
        username: s.username,
  ...
  shareCalendar(
    ...
    const existing = this.db.get(
      'SELECT id FROM vacay_shares WHERE owner_id = ? AND user_id = ?',
      ownerId,
      targetUserId,
    );
    ...
    return shares
      ...
      .map((s) => {
        ...
        const companyHolidays = plan.company_holidays_enabled
          ? this.db.all(
              'SELECT date FROM vacay_company_holidays WHERE plan_id = ? AND date >= ? AND date < ? ORDER BY date',
              plan.id,
              start,
              end,
            )
    ...
    if (!exists) {
      this.db.transaction(() => {
        ...
        for (const u of users) {
          let carriedOver = 0;
          if (carryOverEnabled) {
            const prevConfig = this.db.get<VacayUserYear>(
              'SELECT * FROM vacay_user_years WHERE user_id = ? AND plan_id = ? AND year = ?',
              u.id,
              planId,
              year - 1,
  ...
  deleteYear(planId: number, year: number, socketId: string | undefined): number[] {
    ...
    this.db.transaction(() => {
      ...
      const authors = this.db.all<{ user_id: number }>(
        'SELECT DISTINCT user_id FROM vacay_entries WHERE plan_id = ?',
        planId,
      );
      for (const { user_id } of authors) {
        const { start, end } = this.resolveYearWindow(user_id, year);
        this.db.run(
          'DELETE FROM vacay_entries WHERE plan_id = ? AND user_id = ? AND date >= ? AND date < ?',
          planId,
          user_id,
          start,
          end,
      ...
      const members = this.getPlanUsers(planId);
      const windows = (members.length > 0 ? members.map((m) => m.id) : [owner?.owner_id ?? -1]).map((id) =>
        this.resolveYearWindow(id, year),
      );
      ...
      if (holidayStart < holidayEnd) {
        this.db.run(
          'DELETE FROM vacay_company_holidays WHERE plan_id = ? AND date >= ? AND date < ?',
          planId,
          holidayStart,
          holidayEnd,
        );
      ...
      if (nextYearExists) {
        ...
        const carryOverEnabled = plan ? !!plan.carry_over_enabled : true;
        const users = this.getPlanUsers(planId);
        const prevYear = this.db.get<{ year: number }>(
          'SELECT year FROM vacay_years WHERE plan_id = ? AND year < ? ORDER BY year DESC LIMIT 1',
          planId,
          year + 1,
        );
        ...
        for (const u of users) {
          ...
          this.db.run(
            'UPDATE vacay_user_years SET carried_over = ? WHERE user_id = ? AND plan_id = ? AND year = ?',
            carry,
            u.id,
            planId,
            year + 1,
  ...
  `,
    ...
    const companyHolidays = this.db.all(
      'SELECT * FROM vacay_company_holidays WHERE plan_id = ? AND date >= ? AND date < ?',
      planId,
      start,
      end,
    );
  ...
  toggleEntry(
    ...
    kind?: unknown,
    socketId?: string,
  ): { action?: string; fraction?: number; kind?: string; error?: string } {
    ...
    const existing = this.db.get<{ id: number; fraction: number; kind: string | null }>(
      'SELECT id, fraction, kind FROM vacay_entries WHERE user_id = ? AND date = ? AND plan_id = ?',
      userId,
      date,
      planId,
    );
    ...
    if (weekendBlocked) return { error: 'weekend_blocked' };
    this.db.run(
      'INSERT INTO vacay_entries (plan_id, user_id, date, note, fraction, kind) VALUES (?, ?, ?, ?, ?, ?)',
      planId,
      userId,
      date,
      '',
  ...
  `,
    ...
    const holidayCalendars = this.db.all<VacayHolidayCalendar>(
      'SELECT * FROM vacay_holiday_calendars WHERE plan_id = ? ORDER BY sort_order, id',
      activePlanId,
    );
