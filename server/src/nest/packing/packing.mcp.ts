...
export class PackingMcp {
  ...
  @Tool({
    ...
    description:
      'Change a packing item: rename it, recategorise it, move it into a bag, set how many are needed, record its weight, or flip it between the common list and your own. Ticking it off is toggle_packing_item; choosing who a private item is shared with is set_packing_item_sharing.',
    inputSchema: {
      tripId: z.number().int().positive(),
      itemId: z.number().int().positive(),
  ...
  private broadcastItemUpdate(
    tripId: number,
    itemId: number,
    item: { is_private?: number; owner_id?: number | null; recipients?: { user_id: number }[] },
    wasPrivate: boolean,
  ) {
