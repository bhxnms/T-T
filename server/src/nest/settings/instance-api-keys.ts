...
// literal allow-list. Partial because `amap_api_key` is instance-only — it has
// no users column, so it has no row statement and resolveApiKey never reaches
// one for it.
...
export function resolveApiKey(
  ...
  // Instance-only names (amap_api_key) have no per-user column; the chain ends
  // at the instance for them.
