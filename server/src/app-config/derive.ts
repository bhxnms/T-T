...
export function deriveMaps(raw: RawEnv) {
  return {
    ...
    /** CARTO basemaps watermark keyless tiles; the key is appended as ?key= (#2054). Public too. */
    cartoKey: raw.CARTO_API_KEY || undefined,
    /** Public browser key for the restricted AMap JS SDK; never use a Web-Service secret here. */
    amapJsKey: raw.AMAP_JS_API_KEY || undefined,
  };
}
...
export function derivePlugins(raw: RawEnv) {
  return {
    /** Compatibility baseline for plugins; lets a branded fork expose its own app version. */
    pluginHostVersion: raw.TREK_PLUGIN_HOST_VERSION,
