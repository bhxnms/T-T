...
describe('MPlanTimeline', () => {
  ...
  describe('timeline header', () => {
    ...
    it('FE-MOB-PLTL-006: a stay chip opens the stay editor for members who may edit days (#2210)', () => {
      const { shell } = renderTimeline({
        hotelChips: [
          { key: 'in-3', variant: 'checkin', name: 'Capsule Tokyo', time: '15:00:00', accId: 3, placeId: 301 },
        ],
      });
    ...
    it('FE-MOB-PLTL-006c: a stay without a place still leads a read-only member to the day sheet', () => {
      const { planner, shell } = renderTimeline(
        {
          hotelChips: [
            { key: 'out-3', variant: 'checkout', name: 'Capsule Tokyo', time: '11:00', accId: 3, placeId: null },
          ],
        },
        { can: vi.fn(() => false) }
      );
  ...
  describe('rows', () => {
    ...
    it('FE-MOB-PLTL-011: a place row without a place still reports the assignment', () => {
      ...
      const rows: PlanRow[] = [
        { key: 'pl-13', kind: 'place', item: M_PLACE, assignment: orphan, linkedReservations: [] },
      ];
  ...
  describe('plugin day schedule', () => {
    it('FE-MOB-PLTL-021: slots contributions under their anchors and at the day edges', () => {
      mocks.schedule = {
        ...
        byReservation: {
          2: {
            21: [
              { pluginId: 'ev', id: 'r1', dayId: 2, reservationId: 21, minutes: 45, label: 'Security', tone: 'warn' },
            ],
    ...
    it('FE-MOB-PLTL-022: contributes nothing while no day is selected', () => {
      mocks.schedule = {
        ...
        byPosition: {
          2: {
            start: [{ pluginId: 'ev', id: 's1', dayId: 2, position: 'start', label: 'Morning prep', tone: 'default' }],
            end: [],
          },
        },
        minutesByDay: { 2: 35 },
  ...
  describe('empty day', () => {
    it('FE-MOB-PLTL-023: shows the mascot empty state in go mode', () => {
      ...
      expect(container.querySelector('.tt-mascot--guide')).toBeTruthy();
    });
  ...
  describe('edit mode', () => {
    ...
    it('FE-MOB-PLTL-032: undo is disabled until there is something to undo', () => {
      ...
      const { planner } = renderTimeline({}, { canUndo: true, lastActionLabel: 'undo.reorder' }, { mode: 'edit' });
  ...
  describe('day swipe (#2051)', () => {
    const DAYS = [{ id: 1, trip_id: 1, day_number: 1 }, DAY, { id: 3, trip_id: 1, day_number: 3 }] as unknown as Day[];
    ...
    it('FE-MOB-PLTL-042: a committed swipe announces the day it reached', async () => {
      ...
      await waitFor(() =>
        expect(container.querySelector('[role="status"]')).toHaveTextContent('mobileTrip.dayAnnounce:3,3')
      );
    });
