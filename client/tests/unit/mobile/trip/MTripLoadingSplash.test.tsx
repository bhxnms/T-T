...
describe('MTripLoadingSplash', () => {
  ...
  it('FE-MOB-SPLASH-001: opens on the packing beat with the trip title', () => {
    ...
    expect(container.querySelector('.tt-mascot--packing')).toBeTruthy()
  })

  it('FE-MOB-SPLASH-002: falls back to the TT wordmark when the trip has no title', () => {
    render(<MTripLoadingSplash title="" />)

    expect(screen.getByText((_, el) => el?.tagName === 'DIV' && el.children.length === 0 && el.textContent === 'TT')).toBeInTheDocument()
  })
  ...
  it('FE-MOB-SPLASH-003: walks the four beats in order, one every 1400ms', () => {
    ...
    expect(container.querySelector('.tt-mascot--transport')).toBeTruthy()

    tick(STEP_MS)
    expect(screen.getByText('Loading place photos...')).toBeInTheDocument()
    expect(container.querySelector('.tt-mascot--dashboard')).toBeTruthy()

    tick(STEP_MS)
    expect(screen.getByText('Almost there...')).toBeInTheDocument()
    expect(container.querySelector('.tt-mascot--collections')).toBeTruthy()
  })
  ...
  it('FE-MOB-SPLASH-007: animates the mascot stage between beats', () => {
    ...
    const stage = container.querySelector('.tt-mascot--packing')?.parentElement
  ...
  it('FE-MOB-SPLASH-009: reduced motion drops the beat animation', () => {
    ...
    const stage = container.querySelector('.tt-mascot--dashboard')?.parentElement
