...
/**
 * Monotonic id for the journey load in flight. A slow response for journey A must
 * not overwrite the journey B the user already navigated to, and a stale 404 must
 * not blank the current one — both write only while they are still the newest load.
 */
let journeyLoadRequestId = 0;
...
export const useJourneyStore = create<JourneyState>((set, get) => ({
  ...
  loadJourney: async (id) => {
    const requestId = ++journeyLoadRequestId;
    const cold = get().current?.id !== id;
    if (cold) set({ loading: true, notFound: false });
    try {
      const data = await journeyApi.get(id);
      if (requestId === journeyLoadRequestId) set({ current: data, notFound: false });
    } catch (err: any) {
      if (requestId === journeyLoadRequestId && err?.response?.status === 404) {
        set({ current: null, notFound: true });
      }
      throw err;
    } finally {
      if (requestId === journeyLoadRequestId && cold) set({ loading: false });
    }
  },
