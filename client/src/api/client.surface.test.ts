...
describe('client > endpoint wiring', () => {
  ...
  it('FE-APISURF-010: adminApi maps user, addon and settings endpoints', async () => {
    await assertCalls([
      ...
      { n: 'getTunnel', r: () => adminApi.getTunnel(), e: 'GET /api/admin/tunnel' },
      { n: 'updateTunnel', r: () => adminApi.updateTunnel({ enabled: true }), e: 'PUT /api/admin/tunnel' },
      { n: 'testTunnel', r: () => adminApi.testTunnel({}), e: 'POST /api/admin/tunnel/test' },
      { n: 'getTunnelConnector', r: () => adminApi.getTunnelConnector(), e: 'GET /api/admin/tunnel/connector' },
