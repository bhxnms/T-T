...
export interface TrekImportTripSummary {
  id: number
  title: string
...
export const tripsApi = {
  ...
  // Import from a TREK instance's backup zip: preview lists the trips inside
  // the archive (token-addressed snapshot), importTrek copies the picked ones.
  previewTrekImport: (formData: FormData) => postMultipart<{ token: string; trips: TrekImportTripSummary[] }>('/trips/import-trek/preview', formData),
  importTrek: (token: string, tripIds: number[]) => apiClient.post('/trips/import-trek', { token, trip_ids: tripIds }).then(r => r.data),
...
export const adminApi = {
  ...
  updateAmapSearch: (enabled: boolean) => apiClient.put('/admin/amap-search', { enabled }).then(r => r.data),
...
export const mapsApi = {
  ...
  // POI explore uses Overpass by default and can target AMap when the map
  // provider is 高德 and the instance has its Web-Service key.
  pois: (
    category: string,
    ...
    apiClient.get('/maps/pois', { params: { category, ...bbox, lang, provider }, signal, timeout: 20000 }).then(r => r.data as { pois: import('../components/Map/poiCategories').Poi[]; source: string; truncated: boolean; clamped?: boolean }),
}
...
// Activities API - unified day planning with places and reservations
export const activitiesApi = {
  getForDay: (dayId: number) =>
    apiClient.get(`/activities/day/${dayId}`).then(r => r.data),
  getForTrip: (tripId: number) =>
    apiClient.get(`/activities/trip/${tripId}`).then(r => r.data),
  getForPlace: (placeId: number) =>
    apiClient.get(`/activities/place/${placeId}`).then(r => r.data),
  create: (data: {
    day_id: number
    trip_id: number
    place_id?: number
  ...
  update: (id: number, data: {
    ...
    apiClient.put(`/activities/${id}`, data).then(r => r.data),
  delete: (id: number) =>
    apiClient.delete(`/activities/${id}`).then(r => r.data),
  reorder: (dayId: number, activityIds: number[]) =>
