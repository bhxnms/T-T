...
export default function MAtlas() {
  ...
  return (
    ...
    <div className="relative h-dvh overflow-hidden">
      ...
      {/* Header chips: bucket list + check-ins side by side. The bottom nav makes
          a back button redundant on this main-nav screen. */}
      <div className="absolute left-4 right-4 top-[var(--m-safe-top,12px)] z-[5] flex items-center gap-2">
        <button
          type="button"
          onClick={() => setBucketOpen(true)}
          className="flex h-[38px] flex-1 items-center justify-center gap-[6px] rounded-full border border-[color:var(--m-gbr)] bg-[color:var(--m-sheet)] px-4 text-[0.78125rem] font-bold text-m-ink shadow-[0_5px_12px_-8px_rgba(0,0,0,.18)]"
        >
          {t('atlas.bucketTab')}
        </button>
        <button
      ...
      <MAtlasCheckinSheet open={checkinOpen} onClose={() => setCheckinOpen(false)} />
      <MAtlasCountryPopup atlas={atlas} />

      {/* Landmark tap popup — the same overlay the desktop Atlas uses. The map
          layer (dots, zoom behaviour) is shared through useAtlas; without this
          the tap that sets selectedLandmark would do nothing visible here. */}
      {selectedLandmark && (
        <LandmarkPopup
