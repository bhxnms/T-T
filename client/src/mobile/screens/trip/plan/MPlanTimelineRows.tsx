...
const dragClass = (d?: RowDrag): string =>
  !d
    ? ''
    : `${d.dragging ? 'opacity-40' : ''} ${d.dropTarget && !d.dragging ? 'shadow-[inset_0_2px_0_0_var(--m-act)]' : ''}`;
...
const TIME_CHIP =
  'flex-none whitespace-nowrap rounded-[6px] bg-[color:var(--m-ic)] px-[6px] py-px font-geist text-[0.65625rem] font-semibold';
...
export function PlaceRow({
  ...
  const sub = bookingLines.length > 0 ? bookingLines[0].text : place?.address || place?.description || '';
...
export function TransportRow({
  ...
  const time = start ? `${fmtTime(start, chrome)}${phase === 'single' && end ? ` – ${fmtTime(end, chrome)}` : ''}` : '';
      ...
      <div className="min-w-0 flex-1 overflow-hidden rounded-[14px] border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)]">
        ...
        >
          <div className="flex items-center gap-[7px]">
            <span className="min-w-0 truncate text-[0.84375rem] font-bold">{to ? `${from} → ${to}` : res.title}</span>
            {start && (
              <span className="ml-auto flex-none whitespace-nowrap rounded-[6px] bg-[color:var(--m-ic)] px-[6px] py-px font-geist text-[0.6875rem] font-semibold tabular-nums">
                ...
                {fmtTime(start, chrome)}
                {end ? ` – ${fmtTime(end, chrome)}` : ''}
              </span>
            )}
