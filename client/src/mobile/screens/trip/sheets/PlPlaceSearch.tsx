 ...
 */
export default function PlPlaceSearch({ planner, locationBias, onPick, onResolvingChange, provider, onProviderChange }: PlPlaceSearchProps) {
  ...
  const fetchSuggestions = useCallback(
    async (input: string) => {
      ...
      try {
        const result = await mapsApi.autocomplete(input, language, locationBias, controller.signal, placesSessionRef.current.current(), provider)
    ...
    <div className="relative">
      ...
      {onProviderChange && (
        <div className="mt-1.5 flex items-center gap-1.5 text-[0.6875rem] text-m-muted">
          ...
          <button type="button" onClick={() => onProviderChange('amap')} className={`rounded-full px-2 py-[2px] ${provider === 'amap' ? 'bg-m-act text-m-actfg' : 'bg-[color:var(--m-ic)]'}`}>{t('places.searchProviderAmap')}</button>
          <button type="button" onClick={() => onProviderChange('native')} className={`rounded-full px-2 py-[2px] ${provider === 'native' ? 'bg-m-act text-m-actfg' : 'bg-[color:var(--m-ic)]'}`}>{t('places.searchProviderNative')}</button>
        </div>
      )}
