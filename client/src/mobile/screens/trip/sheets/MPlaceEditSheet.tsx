...
export default function MPlaceEditSheet({ planner, onOpenExpense }: MPlaceEditSheetProps) {
  ...
  // AMap/native provider toggle for the search row; only rendered when the
  // instance carries an AMap key (hasAmapKey from app-config).
  const hasAmapKey = useAuthStore(s => s.hasAmapKey)
  const amapSearchEnabled = useAuthStore(s => s.amapSearchEnabled)
  const [searchProvider, setSearchProvider] = useState<'amap' | 'native'>(amapSearchEnabled && hasAmapKey ? 'amap' : 'native')
  const providerTouchedRef = useRef(false)

  useEffect(() => {
    if (!hasAmapKey) {
      providerTouchedRef.current = false
      setSearchProvider('native')
    } else if (!providerTouchedRef.current && amapSearchEnabled) {
      setSearchProvider('amap')
    }
  }, [hasAmapKey, amapSearchEnabled])

  const handleProviderChange = (next: 'amap' | 'native') => {
  ...
  return (
    <MSheet open={showPlaceForm} onClose={handleClose} material="opaque" ariaLabel={sheetPlace ? t('places.editPlace') : t('places.addPlace')}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[18px] pb-[6px] pt-[2px]" onPaste={handlePaste}>
        <PlPlaceSearch planner={planner} locationBias={locationBias} onPick={applyPick} onResolvingChange={setResolvingPick} provider={searchProvider} onProviderChange={hasAmapKey ? handleProviderChange : undefined} />
