...
export default function MPlaceEditSheet({ planner, onOpenExpense }: MPlaceEditSheetProps) {
  ...
  const [searchProvider, setSearchProvider] = useState<'amap' | 'native'>(
    amapSearchEnabled && hasAmapKey ? 'amap' : 'native'
  );
    ...
    if (editingPlace) {
      const assignment = editingAssignmentId
        ? Object.values(assignments)
            .flat()
            .find((a) => a.id === editingAssignmentId)
        : null;
    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[18px] pb-[6px] pt-[2px]" onPaste={handlePaste}>
        <PlPlaceSearch
          ...
          onProviderChange={hasAmapKey ? handleProviderChange : undefined}
        />
