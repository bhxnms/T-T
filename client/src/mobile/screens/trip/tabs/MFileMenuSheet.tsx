...
export default function MFileMenuSheet({ planner, file, onClose, onOpenLinks }: MFileMenuSheetProps) {
  ...
  const commitNote = () => {
    ...
    setSaving(true);
    filesApi
      .update(tripId, shown.id, { description: value })
  ...
  const download = () => {
    downloadFile(shown.url, shown.original_name).catch(() => {});
  };
  ...
  const remove = () => {
    ...
    setDeleting(true);
    tripActions
      .deleteFile(tripId, shown.id)
      .then(() => {
        toast.success(t('files.toast.trashed'));
        onClose();
      })
      ...
      <MSheet open={open} onClose={onClose} variant="card" material="glass" ariaLabel={shown.original_name}>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-[18px] pb-[18px]">
          ...
          <div className="mt-3 flex flex-col gap-[2px]">
            ...
            {canEdit && <MListRow icon={Link2} label={t('files.link')} onClick={() => onOpenLinks(shown)} />}
            {canDelete && (
