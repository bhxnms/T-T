...
export default function MBookingsTab({ planner, shell }: MTabScreenProps) {
  ...
  const bookings =
    travelerFilter.size === 0
      ? allBookings
      : allBookings.filter((r) => (r.travelers || []).some((tv) => travelerFilter.has(tv.user_id)));
    ...
    <TabScroller>
      ...
      {sections.length === 0 ? (
        <div className="flex min-h-full flex-1 flex-col items-center justify-center px-8 py-10 text-center">
          <MDancingTT scene="bookings" className="mb-2" />
...
function BookingCard({
  ...
  const startDay =
    isHotel && res.accommodation_start_day_id
      ? days.find((d) => d.id === res.accommodation_start_day_id)
      : res.day_id != null
        ? days.find((d) => d.id === res.day_id)
        : undefined;
  const endDay =
    isHotel && res.accommodation_end_day_id
      ? days.find((d) => d.id === res.accommodation_end_day_id)
      : res.end_day_id != null
        ? days.find((d) => d.id === res.end_day_id)
        : undefined;
          ...
          <button type="button" onClick={openEdit} className="block w-full text-left">
            ...
            {res.notes && (
              <div className="mt-2 rounded-[10px] border border-[color:var(--m-rowbr)] bg-m-card px-[10px] py-2">
                <p className="whitespace-pre-wrap font-geist text-[0.6875rem] leading-[1.5] text-m-muted">
                  {res.notes}
                </p>
              </div>
            )}
