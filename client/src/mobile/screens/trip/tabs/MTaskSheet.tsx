 ...
 */
export default function MTaskSheet({
  planner,
  open,
  itemId,
  categories,
  ...
  return (
    <MSheet open={open} onClose={onClose} ariaLabel={item ? t('todo.detail.title') : t('todo.newItem')}>
      <FormSheetHeader
        title={item ? t('todo.detail.title') : t('todo.newItem')}
        onClose={onClose}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[18px] pb-[6px] pt-1">
          ...
          {PRIORITY_LEVELS.map((p) => {
            ...
            return (
              <button
                ...
                style={
                  active && color ? { background: `color-mix(in srgb, ${color} 16%, transparent)`, color } : undefined
                }
              >
                {p === 0 ? (
        ...
        <div className="flex flex-wrap gap-[6px]">
          ...
          {category && !categories.includes(category) && <span className={pillCls(true)}>{category}</span>}
          {!addingCategory ? (
            <button
              type="button"
