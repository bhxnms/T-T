  ...
  return (
    <>
            ...
            <div className="mt-3 flex flex-col gap-2">
              {files.map((file) => {
                ...
                return (
                  <div
                    key={file.id}
                    className="flex items-center gap-[10px] rounded-2xl border border-[color:var(--m-rowbr)] bg-m-sheetop px-3 py-[9px] opacity-80"
                  >
                    <span
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-[2px] font-geist text-[0.625rem] text-m-faint">
                        {[formatSize(file.file_size), formatFileDate(file.created_at, locale)]
                          .filter(Boolean)
                          .join(' · ')}
                      </div>
                    </div>
      ...
      <MConfirmSheet
        ...
        danger
        onConfirm={() => {
          if (confirmDeleteId != null) permanentDelete(confirmDeleteId);
        }}
      />
    </>
  );
