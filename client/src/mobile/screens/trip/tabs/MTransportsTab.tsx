...
import { TRANSPORT_TYPE_COLOR, groupTransports, orderedEndpoints, parseTransportMeta } from './transportsModel';
...
export default function MTransportsTab({ planner, shell }: MTabScreenProps) {
  ...
  const transports =
    travelerFilter.size === 0
      ? allTransports
      : allTransports.filter((r) => (r.travelers || []).some((tv) => travelerFilter.has(tv.user_id)));
  const groups = groupTransports(transports, days);
  const canEdit = planner.can('day_edit', planner.trip);
  const showTravelerFilter =
    planner.tripMembers.length > 1 && allTransports.some((r) => (r.travelers || []).length > 0);
    ...
    <TabScroller>
      ...
      {sections.length === 0 ? (
        <div className="flex min-h-full flex-col items-center justify-center px-8 py-10 text-center">
          <MDancingTT scene="transport" className="mb-2" />
...
function TransportCard({
  ...
  if (!hasEndpoints && meta.departure_airport)
    metaCells.push({ label: t('reservations.meta.from'), value: meta.departure_airport });
  if (!hasEndpoints && meta.arrival_airport)
    metaCells.push({ label: t('reservations.meta.to'), value: meta.arrival_airport });
  ...
  if (meta.seat)
    metaCells.push({ label: t('reservations.meta.seat'), value: meta.seat + (meta.class ? ` · ${meta.class}` : '') });
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-2xl border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)]">
      {/* Header */}
      <div
        className="flex items-center gap-[7px] border-b border-[color:var(--m-rowbr)] px-3 py-[10px]"
      ...
      >
        ...
        <button type="button" onClick={openDetail} className="flex min-w-0 flex-1 items-center gap-[7px] text-left">
              ...
              <div className="mt-2 flex flex-wrap items-center justify-center gap-2 rounded-[10px] border border-[color:var(--m-rowbr)] bg-m-card px-[10px] py-2 text-[0.71875rem] font-semibold text-m-ink">
                {eps.map((ep, i) => (
                  <span key={i} className="inline-flex min-w-0 items-center gap-2">
                    {i > 0 && (
                      <TypeIcon size={12} strokeWidth={2.2} className="flex-none" style={{ color: typeColor }} />
                    )}
            ...
            {metaCells.length > 0 && (
              <div className="mt-2 flex gap-2">
                {metaCells.slice(0, 3).map((c, i) => (
                  <Field key={i} label={c.label} className="flex-1">
                    {c.value}
                  </Field>
                ))}
              </div>
                ...
                <div className="flex flex-col gap-1">
                  {files.map((f) => (
                    <span
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        openFile(f.url, f.original_name);
                      }}
                      onKeyDown={(e) => {
                    ...
                    >
                      ...
                      <span className="truncate font-geist text-[0.65625rem] font-semibold text-m-muted">
                        {f.original_name}
                      </span>
                    </span>
                  ))}
