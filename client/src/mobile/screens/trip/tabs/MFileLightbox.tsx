...
export default function MFileLightbox({ files, index, onIndexChange, onClose, t }: MFileLightboxProps) {
  ...
  useEffect(() => {
    ...
    if (fileUrl && !isVideo(fileMimeType)) {
      getAuthUrl(fileUrl, 'download').then((url) => {
  ...
  return createPortal(
    ...
    >
      ...
      <div
        role="presentation"
        className="flex flex-none items-center justify-between px-4 py-[10px]"
      ...
      >
        ...
        <div className="flex flex-none items-center gap-1">
          <button
            type="button"
            onClick={() => {
              openFile(file.url, file.original_name).catch(() => {});
            }}
          ...
          <button
            type="button"
            onClick={() => {
              downloadFile(file.url, file.original_name).catch(() => {});
            }}
