            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                {member.email && !pending && (
                  <div className="truncate font-geist text-[0.625rem] text-m-muted">{member.email}</div>
                )}
              </div>
              {member.is_owner ? (
                ...
                >
                  {busy ? (
                    ...
                    t(`collections.role.${member.role ?? 'editor'}`).toUpperCase()
                  )}
                  <ChevronDown size={11} strokeWidth={2.4} className="text-m-faint" />
                </button>
              ) : (
                <span className={`${badge} text-m-muted`}>
                  {t(`collections.role.${member.role ?? 'editor'}`).toUpperCase()}
                </span>
              )}
        ...
        {isOwner ? (
          <>
            ...
            {availableUsers.length === 0 ? (
              <div className="py-3 text-center font-geist text-[0.71875rem] text-m-faint">
          ...
          <div className="mt-[14px]">
            <div className="mb-3 font-geist text-[0.71875rem] leading-[1.5] text-m-muted">
              {t('collections.share.memberHint')}
            </div>
            {confirmLeave ? (
              <>
                <div className="mb-2 text-[0.8125rem] text-m-ink">{t('collections.share.leaveConfirm')}</div>
                <div className="flex items-center gap-2">
                  <CancelPill className="ml-auto" onClick={() => setConfirmLeave(false)}>
                    {t('common.cancel')}
                  </CancelPill>
                  <PrimaryPill
                    onClick={handleLeave}
                    disabled={leaving}
                    className="!bg-[color:var(--m-st-danger)] !text-white"
                  >
