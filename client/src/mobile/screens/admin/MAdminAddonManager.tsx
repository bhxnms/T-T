...
export default function MAdminAddonManager({
  bagTrackingEnabled,
  onToggleBagTracking,
  collabFeatures,
  ...
  return (
    <div className="space-y-3">
      ...
      {addons.length === 0 ? (
        <MAdminCard>
          <p className="py-6 text-center font-geist text-[0.6875rem] text-m-faint">{t('admin.addons.noAddons')}</p>
        </MAdminCard>
      ) : (
        <>
          {/* Trip Addons */}
          ...
          {integrationAddons.length > 0 && (
            <MAdminCard>
              <MGroupHead
                icon={Link2}
                label={`${t('admin.addons.type.integration')} — ${t('admin.addons.integrationHint')}`}
              />
  ...
  const save = async () => {
    ...
    try {
      ...
      await adminApi.updateAddon(addon.id, {
        config: { provider, model: model.trim(), baseUrl: baseUrl.trim(), apiKey, multimodal: cfg.multimodal === true },
      });
      toast.success('Saved');
    } catch {
                  ...
                  <button
                    ...
                    className={`max-w-full truncate rounded-full border px-[10px] py-[5px] text-[0.6875rem] ${
                      model === name
                        ...
                        : 'border-[color:var(--m-rowbr)] text-m-ink'
                    }`}
                  >
                    {name}
                  </button>
                        ...
                        {isPulling && (
                          <div className="mt-1.5">
                            <div className="h-1.5 w-full overflow-hidden rounded-full bg-[color:var(--m-ic)]">
                              <div
                                className="h-full bg-m-act transition-[width] duration-200"
                                style={{ width: `${pullPct}%` }}
                              />
                            </div>
                            <div className="mt-0.5 font-geist text-[0.5625rem] text-m-faint">
                              {pullStatus}
                              {pullPct ? ` · ${pullPct}%` : ''}
                            </div>
                          </div>
