import { MOBILE_DASH_TOKENS, normalizeAppearance, type Collection, type MobileDashToken } from '@trek/shared';
import {
  ArrowRight,
  ArrowRightLeft,
  Bookmark,
...
export function resolveMobileDashOrder(stored: string[] | undefined): MobileDashToken[] {
  ...
  for (const tok of stored ?? []) {
    if (valid.has(tok) && !seen.has(tok)) {
      seen.add(tok);
      out.push(tok as MobileDashToken);
    }
  }
...
export function MobileDashWidget({
  id,
  upcoming,
}: {
  id: MobileDashToken;
  ...
  switch (id) {
    case 'currency':
      return <MCurrencyWidget />;
    case 'collections':
      return <MCollectionsWidget />;
    case 'timezones':
      return <MTimezonesWidget />;
    case 'upcomingReservations':
      return <MUpcomingWidget items={upcoming} />;
    default:
      return null;
  }
}
  ...
  return (
    <WidgetPanel
      ...
      action={
        <button
          type="button"
          aria-label={t('dashboard.aria.refreshRates')}
          onClick={fetchRates}
          className="flex text-m-faint"
        >
    ...
    >
      <div className="mt-[11px] flex items-center gap-2">
        <div className="min-w-0 flex-1 rounded-[14px] border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)] p-[11px_12px]">
          <div className="font-geist text-[0.5625rem] font-bold uppercase tracking-[.1em] text-m-faint">
            {t('dashboard.fx.from')}
          </div>
          <input
            value={amount}
        ...
        <div className="min-w-0 flex-1 rounded-[14px] border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)] p-[11px_12px]">
          ...
          <div className="truncate pt-[2px] text-[1.5rem] font-bold tabular-nums">
  ...
  return (
    <WidgetPanel
      ...
      action={
        <button
          type="button"
          aria-label={t('collections.widget.title')}
      ...
      {adding && (
        ...
        >
          <option value="" disabled>
            {t('dashboard.tz.searchPlaceholder')}
          </option>
          {allZones
