...
import { type DashboardTrip, MS_PER_DAY, daysUntil, getTripStatus } from '../../../pages/dashboard/dashboardModel';
...
import { MobileDashWidget, useMobileDashOrder, useMobileDashVisibility } from './MDashWidgets';
...
export default function MDashboard(): React.ReactElement {
  ...
  const badgesFor = useTripCardBadges(
    gridTrips.map((trip) => trip.id),
    anyPluginActive
  );
  ...
  const openEdit = (trip: DashboardTrip) => {
    setEditingTrip(trip);
    setShowForm(true);
  };
      ...
      return [
        {
          ...
          onClick: () => setCopyTrip(trip),
        },
        {
          key: 'restore',
          label: t('dashboard.restore'),
          icon: <ArchiveRestore size={15} strokeWidth={2.1} />,
      ...
      <>
        ...
        {showEmpty && (
          <div className="mt-[10px] flex flex-col items-center rounded-[20px] border border-[color:var(--m-gbr)] bg-[color:var(--m-glass)] px-4 py-8 text-center">
            <MDancingTT scene="dashboard" size={96} className="mb-2" />
    ...
    <>
      ...
      <div className="px-4 pb-[calc(var(--bottom-nav-h,84px)+32px)] pt-[calc(var(--m-safe-top,12px)+82px)]">
        {loadError && (
          <div
            role="alert"
            className="mb-3 flex items-center gap-3 rounded-[20px] border border-[color:var(--m-gbr)] bg-[color:var(--m-glass)] p-[14px]"
          >
            <span className="min-w-0 flex-1 text-[0.8125rem] font-medium text-m-ink">
...
function MSpotlightCard({
  ...
  const primaryBadge = ongoing
    ? t('dashboard.status.ongoing')
    : status === 'past'
      ? t('dashboard.hero.badgeRecent')
      : t('dashboard.hero.badgeNext');
  const secondaryBadge = ongoing
    ? t('dashboard.mobile.spotlightDayOf', { day: dayOfTrip, total: totalDays })
    : status === 'today'
      ...
      <div className="absolute bottom-[10px] left-[10px] right-[10px] rounded-[18px] border border-white/[.16] bg-[rgba(14,14,17,.52)] p-[12px_14px] text-white backdrop-blur-[22px] backdrop-saturate-[1.6]">
        ...
        <div className={`flex gap-[6px] ${ongoing ? '' : 'mt-[9px]'}`}>
          <SpotlightPill
            ...
            label={
              days === 1
                ? t('dashboard.mobile.spotlightDayOne', { count: days })
                : t('dashboard.mobile.spotlightDaysMany', { count: days })
            }
          />
          <SpotlightPill
            ...
            label={
              places === 1
                ? t('dashboard.hero.destinationOne', { count: places })
                : t('dashboard.hero.destinationMany', { count: places })
            }
          />
          <SpotlightPill
            ...
            label={
              people === 1
                ? t('dashboard.hero.travelerOne', { count: people })
                : t('dashboard.hero.travelerMany', { count: people })
            }
          />
        </div>
      </div>
...
function MTripBadges({ items }: { items: TripCardBadge[] }): React.ReactElement | null {
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-[6px]">
      {items.map((b) => {
        const cls =
          'inline-flex max-w-full items-center gap-1 rounded-full border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)] px-2 py-[3px] font-geist text-[0.625rem]';
        const inner = (
          <>
        ...
        return b.url ? (
          ...
          <span key={b.pluginId + b.id} className={cls}>
            {inner}
          </span>
        );
      })}
    ...
    >
      ...
      <div className="p-[14px_16px_16px]">
        ...
        <div className="flex text-center">
          ...
          <ListStat
            value={trip.shared_count ?? 0}
            label={trip.shared_count === 1 ? t('dashboard.card.buddyOne') : t('dashboard.members')}
          />
        </div>
        <MTripBadges items={pluginBadges} />
      </div>
