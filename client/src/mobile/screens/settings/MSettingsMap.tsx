...
import { MSetButton, MSetCard, MSetEyebrow, MSetHint, MSetInput, MSetRow, MSetSelectRow } from './MSettingsUi';
...
export default function MSettingsMap() {
  ...
  const providers: { id: Provider; name: string; sub: string; icon: typeof Layers }[] = [
    ...
    { id: 'amap', name: '高德地图', sub: '简体中文 · 在线地图', icon: Map },
  ];
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className={`block font-geist text-[0.625rem] ${active ? 'opacity-70' : 'text-m-muted'}`}>
                  {p.sub}
                </span>
              </span>
      ...
      {(provider === 'mapbox-gl' || provider === 'maplibre-gl') && (
        <>
          ...
          <MSetInput
            ...
            placeholder={defaultStyleForProvider(provider as GlMapProvider)}
          />
          <MSetHint>
          ...
          {provider === 'mapbox-gl' && (
            <div className="mt-1">
              <MSetRow
                ...
                trailing={
                  <MToggle checked={mapbox3d} onChange={setMapbox3d} ariaLabel={t('settings.map3dBuildings')} />
                }
              />
              <MSetRow
                ...
                trailing={
                  <MToggle
                    checked={mapboxQuality}
                    onChange={setMapboxQuality}
                    ariaLabel={t('settings.mapHighQuality')}
                  />
                }
              />
      ...
      <div className="relative mt-3 h-[200px] w-full overflow-hidden rounded-xl">
        {provider === 'mapbox-gl' || provider === 'maplibre-gl' ? (
          ...
          <ErrorBoundary
            ...
            resetKeys={[provider]}
            fallback={<div className="h-full w-full bg-m-card" />}
          >
            <Suspense fallback={<div className="h-full w-full animate-pulse bg-m-card" />}>
              <GlMapPreview
                provider={provider}
                token={mapboxToken}
                style={mapboxStyle}
          ...
          label: (
            <span className="flex flex-wrap items-center gap-1">
              ...
              {(p.tags || []).map((tag) => (
                <span
                  key={tag}
                  className="rounded bg-[color:var(--m-ic)] px-[5px] py-[2px] font-geist text-[0.5625rem] font-bold uppercase tracking-wide text-m-muted"
                >
                  {tag}
                </span>
              ))}
