    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-[18px] pb-[18px] pt-2">
        ...
        <div className="flex items-start gap-[11px] py-3">
          ...
          <MToggle
            checked={plan.block_weekends !== false}
            onChange={() => updatePlan({ block_weekends: !plan.block_weekends })}
            ariaLabel={t('vacay.blockWeekends')}
          />
        </div>
        ...
        <div className="flex items-start gap-[11px] py-3">
          ...
          <MToggle
            checked={plan.carry_over_enabled}
            onChange={() => updatePlan({ carry_over_enabled: !plan.carry_over_enabled })}
            ariaLabel={t('vacay.carryOver')}
          />
        </div>
        <div className="my-2 h-px bg-[color:var(--m-rowbr)]" />
        ...
        <div className="flex items-start gap-[11px] py-3">
          ...
          <div className="flex-1">
            ...
            <div className="font-geist text-[0.65625rem] text-m-muted">
              {t('vacay.companyHolidaysHint')} · {t('vacay.companyHolidaysNoDeduct')}
            </div>
          </div>
          <MToggle
            checked={plan.company_holidays_enabled}
            onChange={() => updatePlan({ company_holidays_enabled: !plan.company_holidays_enabled })}
            ariaLabel={t('vacay.companyHolidays')}
          />
        </div>
        <div className="my-2 h-px bg-[color:var(--m-rowbr)]" />
        ...
        <div className="flex items-start gap-[11px] py-3">
          ...
          <MToggle
            checked={plan.holidays_enabled}
            onChange={() => updatePlan({ holidays_enabled: !plan.holidays_enabled })}
            ariaLabel={t('vacay.publicHolidays')}
          />
        </div>
        ...
        <div className="flex items-start gap-[11px] py-3">
          ...
          <MToggle
            checked={plan.school_holidays_enabled}
            onChange={() => updatePlan({ school_holidays_enabled: !plan.school_holidays_enabled })}
            ariaLabel={t('vacay.schoolHolidays')}
          />
        </div>
          ...
          <div className="mt-[14px] rounded-2xl border border-[rgba(214,39,59,.16)] bg-[rgba(214,39,59,.07)] p-[13px]">
            ...
            <div className="mt-[9px] flex flex-wrap gap-[6px]">
              {users.map((u) => (
                <span
                  key={u.id}
                  className="inline-flex items-center gap-[5px] rounded-full bg-[color:var(--m-sheetop)] px-[10px] py-[3px]"
                >
        ...
        <div className="ml-7 flex gap-2">
          ...
          <span className="w-[86px] flex-none">
            <SelectRow
              ...
              options={Array.from({ length: monthDayCap(yearSettings.year_start_month) }, (_, i) => ({
                value: String(i + 1),
                label: String(i + 1),
              }))}
...
function SelectRow({
  ...
  return (
    <span className="relative mt-[7px] flex items-center justify-between rounded-[10px] border border-[color:var(--m-rowbr)] bg-[color:var(--m-sheetop)] px-[11px] py-[9px] text-[0.78125rem] font-semibold">
      <span className={`min-w-0 truncate ${selected ? '' : 'text-m-muted'}`}>
    ...
    <div className="mt-2 rounded-[14px] border border-[color:var(--m-rowbr)] bg-[color:var(--m-ic)] p-3">
      <div className="flex items-center gap-[9px]">
        ...
        <MIconBtn
          ...
          className="border border-[color:var(--m-rowbr)] !bg-[color:var(--m-sheetop)]"
        >
