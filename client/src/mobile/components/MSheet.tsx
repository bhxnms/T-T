...
const SHAPE: Record<MSheetVariant, string> = {
  ...
  bottom:
    'rounded-[26px] shadow-[0_-8px_40px_-14px_rgba(0,0,0,.45)] max-h-[calc(100dvh-var(--bottom-nav-h,84px)-var(--m-safe-top,12px)-72px)]',
...
const MATERIAL: Record<MSheetMaterial, string> = {
  glass: 'bg-[color:var(--m-sheet)] backdrop-blur-[40px] backdrop-saturate-[1.8] border border-[color:var(--m-shbr)]',
  'bar-glass':
    ...
    >
      <div className={POSITION[variant]}>
        <div
          ...
          tabIndex={-1}
          className={`relative flex flex-col overflow-hidden text-m-ink outline-none ${SHAPE[variant]} ${MATERIAL[resolvedMaterial]} ${animation} ${className}`}
