...
import { Briefcase, CheckCircle2, Globe, MapPin, Plus, Search, Star, Trash2, X } from 'lucide-react';
...
function AtlasPageDesktop(): React.ReactElement {
  ...
  const noTransparency =
    typeof document !== 'undefined' && document.documentElement.hasAttribute('data-no-transparency');
  ...
  return (
    <div className="h-screen overflow-hidden bg-surface">
        ...
        >
          ...
          >
            {confirmAction.code.length === 2 ? (
              ...
              <div style={{ fontSize: 'calc(36px * var(--fs-scale-title, 1))', marginBottom: 12 }}>
                {countryCodeToFlag(confirmAction.code)}
              </div>
            )}
            <h3
              className="text-content"
              style={{ margin: '0 0 16px', fontSize: 'calc(16px * var(--fs-scale-subtitle, 1))', fontWeight: 700 }}
            >
              {confirmAction.name}
            </h3>
                    ...
                    >
                      ...
                      <div>
                        <div
                          ...
                          style={{ fontSize: 'calc(13px * var(--fs-scale-body, 1))', fontWeight: 600 }}
                        >
                          {t('atlas.removeFromBucket')}
                        </div>
                        <div
                          className="text-content-muted"
                          style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', marginTop: 1 }}
                        >
                          {t('atlas.removeFromBucketHint')}
                        </div>
                      </div>
                    </button>
                  );
            ...
            {confirmAction.type === 'choose-region' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {confirmAction.countryName && (
                  <p
                    className="text-content-muted"
                    style={{ margin: '-8px 0 8px', fontSize: 'calc(12px * var(--fs-scale-body, 1))' }}
                  >
                    {confirmAction.countryName}
                  </p>
                )}
            ...
            {confirmAction.type === 'unmark' && (
              <>
                <p
                  className="text-content-muted"
                  style={{ margin: '0 0 20px', fontSize: 'calc(13px * var(--fs-scale-body, 1))' }}
                >
            ...
            {confirmAction.type === 'bucket' && (
              <>
                <p
                  className="text-content-muted"
                  style={{ margin: '0 0 14px', fontSize: 'calc(13px * var(--fs-scale-body, 1))' }}
                >
                  {t('atlas.bucketWhen')}
                </p>
      ...
      {/* Landmark Popup — must live in THIS component: selectedLandmark state
...
interface SidebarContentProps {
  ...
  bucketList: any[];
  bucketTab: 'stats' | 'bucket' | 'checkins';
  setBucketTab: (tab: 'stats' | 'bucket' | 'checkins') => void;
...
function SidebarContent({
  ...
  // ── 打卡点 tab ──
  // Both check-in sources live client-side (landmarkStorage + checkinStorage).
  // Re-read whenever a tab writes a change ('tt-checkins-changed') or another
  // browser tab syncs ('storage').
  const [checkinsVersion, setCheckinsVersion] = useState(0);
  useEffect(() => {
    const bump = () => setCheckinsVersion((v) => v + 1);
  ...
  const visitedLandmarkList = React.useMemo(() => {
    void checkinsVersion;
    const visited = getVisitedLandmarks();
    return getAllLandmarks().filter((l) => visited.has(`${l.provinceCode}:${l.name}`));
  }, [checkinsVersion]);
  const checkedPlaceList = React.useMemo(() => {
    void checkinsVersion;
    return getCheckedPlaces();
  }, [checkinsVersion]);
  const checkinTotal = visitedLandmarkList.length + checkedPlaceList.length;
  ...
  const tabBar = (
    <div style={{ display: 'flex', gap: 4, padding: '12px 16px 0', marginBottom: 4 }}>
      {[
        ...
        { id: 'checkins', label: t('atlas.checkinTab'), icon: CheckCircle2 },
      ].map((tab) => (
        <button
          type="button"
          key={tab.id}
  ...
  if (countries.length === 0 && !lastTrip && bucketTab === 'stats') {
    return (
      <>
        {tabBar}
      ...
      >
        ...
        {bucketList.length === 0 && !showBucketAdd && (
          <div
            ...
            style={{ color: tf, fontSize: 'calc(12px * var(--fs-scale-body, 1))' }}
          >
  ...
  // ── 打卡点 tab content: total count + the two source lists ──
  const checkinContent = (
    <div style={{ padding: '4px 16px 10px', overflowY: 'auto' }}>
      <div
        className="mx-1 my-2 flex flex-col items-center justify-center rounded-xl py-3"
        style={{ background: bg(0.08) }}
      >
        <span className="text-4xl font-black tabular-nums leading-none" style={{ color: tp }}>
          {checkinTotal}
        </span>
        <span className="mt-1 text-[9px] font-semibold uppercase tracking-wide" style={{ color: tf }}>
          {t('atlas.checkinTotal')}
        </span>
      </div>
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid' }}>
        <div style={bucketTab === 'stats' ? { gridArea: '1/1' } : { visibility: 'hidden' as const, gridArea: '1/1' }}>
          <div ref={statsContentRef} className="flex items-stretch justify-center">
            {/* ═══ SECTION 1: Numbers ═══ */}
            {/* Countries hero */}
            <div
              className="mx-2 my-2 flex flex-col justify-center rounded-xl px-5 py-4"
              style={{ background: bg(0.08) }}
            >
            ...
            always known AQ, but a permanent zero column would be dead space for everyone else. */}
              {[
                ...
                'Africa',
                'Oceania',
                ...((continents?.['Antarctica'] || 0) > 0 ? ['Antarctica'] : []),
              ].map((cont) => {
                ...
                <div className="flex items-center gap-3 px-6 py-4">
                  ...
                  <div>
                    ...
                    <p className="mb-1 text-[10px]" style={{ color: tf }}>
                      {countryDetail.places.length} {t('atlas.places')} · {countryDetail.trips.length}{' '}
                      {t('atlas.tripPlural')}
                    </p>
