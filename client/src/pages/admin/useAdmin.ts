...
export function useAdmin() {
  ...
  // AMap (高德) Web-Service key + instance-wide default search switch.
  const [amapKey, setAmapKey] = useState<string>('')
  const [amapSearchEnabledState, setAmapSearchEnabledState] = useState<boolean>(false)
  ...
  const loadApiKeys = async () => {
    try {
      ...
      try {
        const amap = await adminApi.getAmapSearch()
        setAmapSearchEnabledState(!!amap.enabled)
      } catch {
        // Non-fatal: the toggle falls back to off.
      }
    } catch (err: unknown) {
      // ignore
    }
