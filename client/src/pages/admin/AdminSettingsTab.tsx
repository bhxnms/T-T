...
export default function AdminSettingsTab({ admin, t }: AdminSettingsTabProps): React.ReactElement {
  const {
    ...
    mapsKey, setMapsKey, unsplashKey, setUnsplashKey, amapKey, setAmapKey, amapSearchEnabledState, setAmapSearchEnabledState, showKeys, savingKeys, validating, validation,
    managed,
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full pr-10 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-transparent"
              />
              <button type="button" onClick={() => toggleKey('amap')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600">
          ...
          {!managed && (
            <div className="flex items-center justify-between gap-4 py-3 border-t border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">{t('admin.amapSearch.subtitle')}</p>
              </div>
              <ToggleSwitch
                ...
                onToggle={async () => {
                  ...
                  try { await adminApi.updateAmapSearch(next) } catch { setAmapSearchEnabledState(!next) }
                }}
              />
            </div>
          )}
