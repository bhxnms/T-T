...
export default function AdminNotificationsPanel({
  ...
  const isActive = (id: string) =>
    matrix.channels?.some((c: { id: string; active: boolean }) => c.id === id && c.active) ?? false;
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <div className="p-6">
          {saving && (
            <p
              className="text-content-faint"
              style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', marginBottom: 8 }}
            >
              Saving…
            </p>
          )}
                  ...
                  if (!implementedForEvent.includes(ch)) {
                    return (
                      <span
                        key={ch}
                        className="text-content-faint"
                        style={{ textAlign: 'center', fontSize: 'calc(14px * var(--fs-scale-body, 1))' }}
                      >
                        —
                      </span>
                    );
