...
interface MockMarker {
  ...
  /** Landmark markers are added to their layer group in createLandmarkLayer. */
...
const lf = vi.hoisted(() => ({
  ...
  mapsRemoved: 0,
  markers: 0,
  /** Markers NOT created in the landmark pane — i.e. bucket-list markers. */
  defaultPaneMarkers: 0,
...
vi.mock('leaflet', () => {
  ...
  const L = {
    ...
    svg: vi.fn(() => ({})),
    divIcon: vi.fn(() => ({})),
    marker: vi.fn((latlng: unknown, options?: { pane?: string }) => {
      lf.markers += 1;
      if (options?.pane !== 'landmarkPane') lf.defaultPaneMarkers += 1;
      const handlers: Record<string, ((e?: unknown) => void)[]> = {};
...
describe('useAtlas', () => {
  it('FE-HOOK-ATLAS-001: loads stats, bucket list and visited regions on mount', async () => {
    ...
    // A bucket item with coordinates becomes a map marker. The landmark layer
    // also spawns markers, so count only non-landmark (default pane) markers.
    await waitFor(() => expect(lf.defaultPaneMarkers).toBe(1));
  });
