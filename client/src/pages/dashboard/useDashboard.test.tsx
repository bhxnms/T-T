...
describe('useDashboard', () => {
  ...
  it('FE-HOOK-DASH-006: an unhealthy auth check also counts as a load error', async () => {
    ...
    act(() => {
      useAuthStore.setState({ authCheckFailed: true });
    });
  ...
  it('FE-HOOK-DASH-007: creating a trip prepends it and returns the payload', async () => {
    ...
    let payload: unknown;
    await act(async () => {
      payload = await result.current.handleCreate({ title: 'Iceland' });
    });
  ...
  it('FE-HOOK-DASH-009: updating replaces the edited trip', async () => {
    server.use(http.put('/api/trips/:id', () => HttpResponse.json({ trip: { ...PARIS, title: 'Paris Reloaded' } })));
    const { result } = await mountLoaded();

    act(() => {
      result.current.setEditingTrip(PARIS);
    });
    await act(async () => {
      await result.current.handleUpdate({ title: 'Paris Reloaded' });
    });
  ...
  it('FE-HOOK-DASH-010: updating without an edited trip is a no-op', async () => {
    ...
    await act(async () => {
      await result.current.handleUpdate({ title: 'nope' });
    });
  ...
  it('FE-HOOK-DASH-012: deleting drops the trip from both lists', async () => {
    ...
    act(() => {
      result.current.setDeleteTrip(PARIS);
    });
    await act(async () => {
  ...
  it('FE-HOOK-DASH-015: archiving moves the trip into the archive list', async () => {
    ...
    await act(async () => {
      await result.current.handleArchive(101);
    });
  ...
  it('FE-HOOK-DASH-017: restoring moves the trip back into the active list', async () => {
    ...
    await act(async () => {
      await result.current.handleUnarchive(103);
    });
  ...
  it('FE-HOOK-DASH-019: copying adds the duplicate and clears the dialog', async () => {
    server.use(
      http.post('/api/trips/:id/copy', async ({ request }) => {
    ...
    act(() => {
      result.current.setCopyTrip(PARIS);
    });
    await act(async () => {
      await result.current.confirmCopy();
    });
