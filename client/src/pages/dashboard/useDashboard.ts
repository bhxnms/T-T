...
import { authApi, reservationsApi, tripsApi } from '../../api/client';
...
export function useDashboard() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(
    () => (localStorage.getItem('trek_dashboard_view') as 'grid' | 'list') || 'grid'
  );
  ...
  useEffect(() => {
    ...
    authApi
      ...
      .catch((err) => {
        if (!cancelled) console.error('Failed to load travel stats:', err);
      });
    reservationsApi
      .upcoming()
      .then((r: { reservations: UpcomingReservation[] }) => {
        if (!cancelled) setUpcoming(r.reservations || []);
      })
      .catch((err) => {
        if (!cancelled) console.error('Failed to load upcoming reservations:', err);
      });
    return () => {
      cancelled = true;
    };
