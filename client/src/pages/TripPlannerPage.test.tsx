...
describe('TripPlannerPage', () => {
  ...
  describe('FE-PAGE-PLANNER-016: Left panel collapse toggle', () => {
    it('collapses the left sidebar when the collapse button is clicked', async () => {
      ...
      // The wrapper (DayPlanSidebarWithActivities) sits between the mock and the
      // collapsible panel: mock → content div → wrapper → panel. The panel's
      // previousElementSibling is the collapse button.
      const sidebarPanel = screen.getByTestId('day-plan-sidebar').parentElement!.parentElement!.parentElement!;
      const collapseButton = sidebarPanel.previousElementSibling as HTMLElement;

      fireEvent.click(collapseButton);

      await waitFor(() => {
        expect(sidebarPanel).toHaveStyle('opacity: 0');
      });
    });
  });
