...
import { ALL_SAVED, useCollectionStore } from '../../store/collectionStore';
...
export function useCollections() {
  ...
  const dark =
    dm === true ||
    dm === 'dark' ||
    (dm === 'auto' && typeof window !== 'undefined' && window.matchMedia('(prefers-color-scheme: dark)').matches);
  ...
  useEffect(() => {
    ...
    categoriesApi
      ...
      .catch((err) => {
        if (!cancelled) console.error('Failed to load collection categories:', err);
      });
    return () => {
      cancelled = true;
    };
  ...
  const handleWsMessage = useCallback(
    (msg: { type: string; collectionId?: number }) => {
  ...
  const visiblePlaces = useMemo(
    () =>
      sortPlaces(
        filterPlaces(places, statusFilter, search, categoryFilter, isAllSaved ? [] : labelFilter, ratingFilter),
        sortMode
      ),
  ...
  const handleStatusChange = useCallback(
    async (placeId: number, status: CollectionStatus) => {
      try {
  ...
  const handleRatePlace = useCallback(
    async (placeId: number, rating: number | null) => {
      try {
  ...
  const handleDuplicateToList = useCallback(
    async (targetId: number) => {
      if (selectedIds.length === 0) return;
      try {
  ...
  const handleUpdateLabel = useCallback(
    async (labelId: number, body: CollectionLabelUpdateRequest) => {
      try {
        await updateLabel(labelId, body);
      } catch (err) {
        toast.error(getApiErrorMessage(err, t('common.error')));
      }
    },
  ...
  const handleBulkAssignLabels = useCallback(
  ...
  const handleAssignPlaceLabels = useCallback(
    async (placeId: number, labelIds: number[]) => {
      try {
        await updatePlace(placeId, { label_ids: labelIds });
      } catch (err) {
        toast.error(getApiErrorMessage(err, t('common.error')));
      }
    },
  ...
  const handleCopyToTrip = useCallback(
    async (tripId: number) => {
      const ids = copyIds ?? [];
  ...
  return {
    ...
    listColRef: listCol.ref,
    listColRect: listCol.rect,
    categories,
    // store data
    collections,
    ownedLists,
    ...
    handleUpdateLabel,
    handleDeleteLabel,
    handleBulkAssignLabels,
    handleAssignPlaceLabels,
    // local UI state
    editorTarget,
    setEditorTarget,
    handleEditorCreated,
