...
function makeStore() {
  return {
    ...
    copyToTrip: vi.fn(async (_tripId: number, _ids: number[]) => ({
      copied: 2,
      skipped: [] as { id: number; name: string }[],
    })),
...
describe('useCollections — active list and permissions', () => {
  ...
  it('FE-PAGE-COLL-016: a non-member has no role at all', async () => {
    ...
    store.activeId = 2;
    store.members = [
      { user_id: 99, username: 'other', status: 'accepted', role: 'editor' } as unknown as CollectionMember,
    ];
    const { result } = await setup();
  ...
  it('FE-PAGE-COLL-018: splits the rail into owned and shared lists', async () => {
    store.collections = [
      collection({ id: 1 }),
      collection({ id: 2, is_owner: false }),
      collection({ id: 3, is_owner: undefined }),
    ];
    const { result } = await setup();
...
describe('useCollections — derived place lists', () => {
  beforeEach(() => {
    ...
    store.places = [
      place({
        ...
        lng: 20,
        category_id: 3,
        category: { id: 3, name: 'Food', color: '#f00', icon: 'utensils' } as unknown as CollectionPlace['category'],
        label_ids: [50],
        sort_order: 1,
      }),
  ...
  it('FE-PAGE-COLL-025: category and label options carry their counts', async () => {
    ...
    expect(result.current.categoryOptions).toEqual([
      { id: 3, name: 'Food', color: '#f00', icon: 'utensils', count: 1 },
    ]);
    expect(result.current.labelOptions.map((l) => [l.id, l.count])).toEqual([
      [50, 1],
      [51, 1],
    ]);
  });
  ...
  it('FE-PAGE-COLL-028: the detail place is shimmed with a trip id', async () => {
    store.places = [
      place({
        ...
        name: 'Museum',
        source_trip_id: 12,
        category: { id: 3, name: 'Food' } as unknown as CollectionPlace['category'],
      }),
    ];
...
describe('useCollections — place handlers', () => {
  it('FE-PAGE-COLL-039: status changes forward to the store', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleStatusChange(5, 'visited');
    });
  ...
  it('FE-PAGE-COLL-041: deleting a place forwards, and failures toast', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleDeletePlace(5);
    });
...
describe('useCollections — move and duplicate', () => {
  it('FE-PAGE-COLL-045: move and duplicate no-op on an empty selection', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleMoveToList(9);
    });
    await act(async () => {
      await result.current.handleDuplicateToList(9);
    });
...
describe('useCollections — labels', () => {
  it('FE-PAGE-COLL-050: create, update and delete forward to the store', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleCreateLabel('Food', '#f00');
    });
    await act(async () => {
      await result.current.handleUpdateLabel(3, { name: 'Drinks' });
    });
    await act(async () => {
      await result.current.handleDeleteLabel(3);
    });
  ...
  it('FE-PAGE-COLL-051: each label mutation surfaces its error', async () => {
    ...
    await act(async () => {
      await result.current.handleCreateLabel('Food');
    });
    expect(addToast).toHaveBeenLastCalledWith('dup name', 'error', undefined);

    await act(async () => {
      await result.current.handleUpdateLabel(3, { color: '#000' });
    });
  ...
  it('FE-PAGE-COLL-052: bulk assign needs both a selection and a label', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleBulkAssignLabels([1]);
    });
    expect(store.assignLabels).not.toHaveBeenCalled();

    store.selectedIds = [4];
    await act(async () => {
      await result.current.handleBulkAssignLabels([]);
    });
    expect(store.assignLabels).not.toHaveBeenCalled();
  });
  ...
  it('FE-PAGE-COLL-053: a successful bulk assign closes the picker', async () => {
    ...
    await act(async () => {
      await result.current.handleBulkAssignLabels([1, 2]);
    });
...
describe('useCollections — invites and leaving', () => {
  it('FE-PAGE-COLL-056: accepting an invite opens the list', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleAcceptInvite(12);
    });
  ...
  it('FE-PAGE-COLL-058: declining forwards, and failures toast', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleDeclineInvite(12);
    });
...
describe('useCollections — detail panel and copy to trip', () => {
  ...
  it('FE-PAGE-COLL-062: the detail status button sets the status of the open place', async () => {
    ...
    const { result } = await setup();
    await act(async () => {
      result.current.handleDetailStatus('want');
    });
  ...
  it('FE-PAGE-COLL-063: removing from the detail deletes and closes it', async () => {
    ...
    const { result } = await setup();
    await act(async () => {
      await result.current.handleDetailRemove();
    });
  ...
  it('FE-PAGE-COLL-069: copying returns the store result and clears select mode', async () => {
    ...
    await act(async () => {
      res = await result.current.handleCopyToTrip(20);
    });
  ...
  it('FE-PAGE-COLL-070: copying without an open modal sends an empty id list', async () => {
    const { result } = await setup();
    await act(async () => {
      await result.current.handleCopyToTrip(20);
    });
