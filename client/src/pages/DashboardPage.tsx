...
function DashboardPageDesktop(): React.ReactElement {
  ...
  const sidebarVisible =
    dashCfg.desktop.sidebar &&
    (showCurrency || showCollections || showTimezones || showUpcoming || widgetPlugins.length > 0);
                ...
                <div className="sec-head">
                  ...
                  <div className="sec-tools">
                    <div className="seg">
                      <button
                        type="button"
                        className={tripFilter === 'planned' ? 'on' : ''}
                      ...
                      <button
                        type="button"
                        className={tripFilter === 'archive' ? 'on' : ''}
                        onClick={() => setTripFilter('archive')}
                      >
        ...
        <div className="hero-top">
          ...
          <div className="hero-tools">
            ...
            <button
              ...
              title={t('dashboard.aria.duplicate')}
...
function AtlasStats({ stats }: { stats: TravelStats | null }): React.ReactElement | null {
  ...
  const atlasTemplate =
    [
      dash.desktop.atlas && '1.5fr',
      dash.desktop.tripsTotal && '1fr',
      dash.desktop.daysTraveled && '1fr',
      dash.desktop.distanceFlown && '1fr',
    ]
      .filter(Boolean)
      .join(' ') || '1fr';
  ...
  return (
    <section
      className="atlas"
      style={{ '--atlas-template': atlasTemplate, '--atlas-template-m': atlasTemplateM } as React.CSSProperties}
    >
      {showAtlas && (
      ...
      {showTrips && (
        <div className="atlas-card">
          ...
          <div className="value mono">
            {statsPending ? <Skeleton width={44} height={30} radius={6} /> : (stats.totalTrips ?? 0)}
          </div>
          <div className="delta">
            {statsPending ? (
  ...
  return (
    <div className="tool">
      <div className="tool-head">
        ...
        <button type="button" className="tool-action" aria-label={t('dashboard.aria.refreshRates')} onClick={fetchRate}>
          <RefreshCw size={14} />
        </button>
      </div>
      <div className="fx-input">
        ...
        <button type="button" className="fx-swap" aria-label={t('dashboard.aria.swapCurrencies')} onClick={swap}>
...
function UpcomingTool({
  ...
  return (
    <div className="tool">
      ...
      {items.length === 0 ? (
        <div style={{ fontSize: 'calc(13px * var(--fs-scale-body, 1))', color: 'var(--ink-3)' }}>
