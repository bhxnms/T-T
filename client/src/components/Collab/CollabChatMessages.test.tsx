...
function renderMessages(messages: ChatMsg[], overrides: Record<string, unknown> = {}): Handles {
...
describe('ChatMessages', () => {
  ...
  it('FE-W5CCM-010: the hover actions become visible for the hovered message only', () => {
    renderMessages([buildMsg({ id: 1, text: 'one' }), buildMsg({ id: 2, text: 'two' })], { hoveredId: 1 });
  ...
  it('FE-W5CCM-016: an own reply quote shows the quoted author and a truncated body', () => {
    ...
    renderMessages([buildMsg({ user_id: 1, username: 'me', reply_username: 'alice', reply_text: longQuote })]);
  ...
  it('FE-W5CCM-018: a resolved link preview scrolls down when the view is pinned to the bottom', async () => {
    const { scrollToBottom } = renderMessages([buildMsg({ text: 'look at https://example.com/a' })], {
  ...
  it('FE-W5CCM-019: a resolved link preview does not scroll when the user scrolled up', async () => {
    const { scrollToBottom } = renderMessages([buildMsg({ text: 'look at https://example.com/b' })], {
      isAtBottom: { current: false },
    });
