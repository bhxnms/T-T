import { ExternalLink, Pencil, Plus, Settings, StickyNote, X } from 'lucide-react';
...
function useCollabNotes({ tripId, currentUser }: CollabNotesProps) {
  ...
  const handleUpdateNote = useCallback(
    async (noteId, data, opts: { silent?: boolean } = {}) => {
      let result;
      try {
  ...
  const renameCategory = useCallback(
    async (oldName, newName) => {
      ...
      for (const n of toUpdate) {
        try {
          await handleUpdateNote(n.id, { category: newName }, { silent: true });
        } catch {
          failed++;
        }
      }
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
        {canEdit && (
          <button
            ...
            title={t('collab.notes.categorySettings')}
            style={{
              display: 'flex',
              alignItems: 'center',
