...
vi.mock('../../api/client', () => ({
  collabApi: { linkPreview: (tripId: number, url: string) => linkPreview(tripId, url) },
}));
...
describe('ReactionBadge', () => {
  ...
  it('FE-W4CCS-008: shows no tooltip when nobody is named', () => {
    render(
      <ReactionBadge reaction={reaction({ users: [] } as Partial<ChatReaction>)} currentUserId={1} onReact={() => {}} />
    );
...
describe('UserAvatar', () => {
  ...
  it('FE-W4CCS-010: renders the avatar image when one is set', () => {
    const { container } = render(
      <UserAvatar user={{ username: 'ada', avatar: '/uploads/avatars/ada.png' } as NoteAuthor} size={20} />
    );
    const img = container.querySelector('img') as HTMLImageElement;
...
describe('LinkPreview', () => {
  ...
  it('FE-W4CCS-019: a preview that resolves after the url changed is not shown', async () => {
    ...
    linkPreview.mockImplementationOnce(
      () =>
        new Promise((res) => {
          resolveFirst = res;
        })
