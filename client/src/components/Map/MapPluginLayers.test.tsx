...
vi.mock('react-leaflet', () => ({
  Polyline: ({ positions, pathOptions, children }: PathProps) => (
    <div data-testid="polyline" data-points={JSON.stringify(positions)} data-path-options={JSON.stringify(pathOptions)}>
      {children}
    </div>
  ),
  Polygon: ({ positions, pathOptions, children }: PathProps) => (
    <div data-testid="polygon" data-points={JSON.stringify(positions)} data-path-options={JSON.stringify(pathOptions)}>
      {children}
    </div>
  ),
...
const optionsOf = (el: HTMLElement) =>
  JSON.parse(el.getAttribute('data-path-options') || '{}') as Record<string, unknown>;
...
describe('PluginMapLayers', () => {
  it('FE-COMP-MAPPLUGINLAYERS-001: renders nothing without a trip', async () => {
    let requests = 0;
    server.use(
      http.get('/api/map-layers/:tripId', () => {
        requests += 1;
  ...
  it('FE-COMP-MAPPLUGINLAYERS-005: draws polylines, polygons and metric circles', async () => {
    ...
    expect(JSON.parse(screen.getByTestId('polygon').getAttribute('data-points') || '')).toEqual([
      [48, 2],
      [48, 3],
      [49, 3],
    ]);
  ...
  it('FE-COMP-MAPPLUGINLAYERS-007: only a labelled feature gets a tooltip and stays clickable', async () => {
    serveLayers([layer([feature({ label: 'Charging corridor' }), feature({ type: 'polygon' })])]);
