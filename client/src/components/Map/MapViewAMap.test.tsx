...
const mockAMap = () => {
  ...
  function MockMarker(opts: any) {
    const marker = {
      options: opts,
      setMap: vi.fn((m: any) => { if (m) overlays.push(marker); else overlays.splice(overlays.indexOf(marker), 1) }),
      on: vi.fn(),
      setPosition: vi.fn(),
      getElement: vi.fn(() => {
...
describe('MapViewAMap clustering', () => {
  ...
  it('FE-COMP-MAPVIEWAMAP-001: clusters nearby places at zoom 10', async () => {
    ...
    const { container } = render(<MapViewAMap
      places={[
    ...
    const clusterMarker = markerCalls.find((call: any) =>
      call[0]?.title?.includes('places') || call[0]?.content?.includes('places')
    )
  ...
  it('FE-COMP-MAPVIEWAMAP-002: dissolves clusters at zoom 11', async () => {
    const { loadAmap } = await import('./engines/amap')
    const { rerender } = render(<MapViewAMap
      places={[
        { id: 1, lat: 39.908, lng: 116.397, name: 'A' },
        { id: 2, lat: 39.9081, lng: 116.3971, name: 'B' },
      ]}
...
describe('MapViewAMap event priority', () => {
  ...
  it('FE-COMP-MAPVIEWAMAP-004: GPX track click suppresses map click', async () => {
    ...
    render(<MapViewAMap
      places={[{ id: 1, lat: 39.908, lng: 116.397, name: 'A', route_geometry: JSON.stringify([[39.908, 116.397], [39.909, 116.398]]) }]}
      zoom={12}
      center={[39.908, 116.397]}
      onMapClick={onMapClick}
      onMarkerClick={onMarkerClick}
