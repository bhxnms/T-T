...
function renderOverlay(props: Partial<React.ComponentProps<typeof ReservationOverlay>> = {}) {
  return render(<ReservationOverlay reservations={[booking()]} showConnections showStats={false} {...props} />);
}
...
describe('ReservationOverlay', () => {
  ...
  it('FE-COMP-RESOVERLAY-005: a train is drawn as the plain two-point line', () => {
    ...
    expect(JSON.parse(lines()[0].getAttribute('data-points') || '[]')).toEqual([
      [48, 16],
      [51.5, 16],
    ]);
  });
  ...
  it('FE-COMP-RESOVERLAY-012: each transport type gets its own declutter floor', () => {
    ...
    const short = (type: string, id: number) =>
      booking({
        id,
...
describe('ReservationOverlay declutter floors and road routes', () => {
  ...
  const hop = (type: string, id: number, dLat: number) =>
    booking({
      id,
      type: type as Reservation['type'],
      endpoints: [
...
describe('ReservationOverlay duration math', () => {
  const timed = (
    from: Partial<ReservationEndpoint>,
    to: Partial<ReservationEndpoint>,
  ...
  it('FE-COMP-RESOVERLAY-028: a time-only endpoint borrows the date from the other side', () => {
    renderOverlay({
      reservations: [
        timed({ local_date: '2024-01-15', local_time: '09:20' }, {}, {
  ...
  it('FE-COMP-RESOVERLAY-029: two date-less times and an implausible gap both render', () => {
    ...
    renderOverlay({
      reservations: [
        timed({}, {}, {
          id: 5,
          reservation_time: '2024-01-15T09:20',
          reservation_end_time: '2024-01-19T17:30',
        } as Partial<Reservation>),
      ],
    });
    expect(lines().length).toBeGreaterThan(0);
  ...
  it('FE-COMP-RESOVERLAY-040: a date-less departure borrows the date from the arrival', () => {
    renderOverlay({
      reservations: [
        timed({}, { local_date: '2024-01-16', local_time: '08:00' }, {
  ...
  it('FE-COMP-RESOVERLAY-041: a hop shorter than an hour renders', () => {
    renderOverlay({
      reservations: [
        timed({}, {}, {
          reservation_time: '2024-01-15T09:20',
          reservation_end_time: '2024-01-15T09:50',
        } as Partial<Reservation>),
      ],
    });
    expect(lines().length).toBeGreaterThan(0);
  ...
  it('FE-COMP-RESOVERLAY-042: a stored timestamp that carries no clock time renders', () => {
    renderOverlay({
      reservations: [
        timed({ timezone: 'Europe/Vienna' }, { timezone: 'Europe/London' }, {
          reservation_time: '2024-01-15T',
          reservation_end_time: '2024-01-15T18:00',
        } as Partial<Reservation>),
      ],
    });
    expect(lines().length).toBeGreaterThan(0);
