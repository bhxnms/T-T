...
import { Bike, Bus, Car, CarTaxiFront, Plane, Route, Sailboat, Ship, Train, TramFront } from 'lucide-react';
  ...
  const items = useMemo<TransportItem[]>(() => {
    ...
    for (const r of reservations) {
      ...
      out.push({
        ...
        type,
        arcs,
        transitSegs: type === 'transit' ? getTransitMapSegments(r) : [],
        mainLabel,
        subLabel,
      });
    }
              ...
              <Polyline
                ...
                pathOptions={
                  seg.walk
                    ...
                    : {
                        color: seg.color || TYPE_META.transit.color,
                        weight: 3.5,
                        opacity: 0.95,
                        lineCap: 'round',
                        lineJoin: 'round',
      ...
      {visibleItems.flatMap((item) =>
        item.waypoints.map((wp, wi) => (
          <Marker
            ...
            icon={endpointIcon(
              item.type,
              showEndpointLabels && labelVisibleIds.has(item.res.id) ? wp.code || cleanEndpointName(wp.name) : null
            )}
            pane={ENDPOINT_PANE}
            zIndexOffset={1000}
