...
function createPlaceIcon(place, orderNumbers, isSelected) {
  ...
  if (
    ...
    (place.image_url.startsWith('data:') ||
      place.image_url.startsWith('/api/maps/place-photo/') ||
      place.image_url.startsWith('/uploads/'))
  ) {
    const imgIcon = L.divIcon({
      ...
      ">
        ...
        ">
          <img src="${escapeHtml(place.image_url)}" alt="" aria-hidden="true" width="${size}" height="${size}" style="display:block;border-radius:50%;object-fit:cover;" />
        </div>
        ${badgeHtml}
      </div>`,
      iconSize: [size, size],
