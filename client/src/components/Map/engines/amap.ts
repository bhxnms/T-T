...
let loading: Promise<AMapModule> | null = null

export function loadAmap(): Promise<AMapModule> {
  if ((window as AMapWindow).AMap) return Promise.resolve((window as AMapWindow).AMap as AMapModule)
  ...
  loading = new Promise((resolve, reject) => {
    ...
    script.src = 'https://webapi.amap.com/maps?v=2.0&key=' + encodeURIComponent((window as any).__TREK_AMAP_JS_KEY__ || '') + '&lang=zh_cn'
    script.async = true
    script.onload = () => {
...
export function wgs84ToGcj02(lng: number, lat: number): { lng: number; lat: number } {
  ...
  return { lng: lng + ((tLng * 180) / (A / s * Math.cos(r) * PI)), lat: lat + ((tLat * 180) / ((A * (1 - EE)) / (m * s) * PI)) }
}
