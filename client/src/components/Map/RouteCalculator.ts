...
export async function calculateRouteWithLegs(
  ...
  // AMap profile (`amap:driving|walking|cycling`): routed server-side through
  // /api/maps/amap-route so the key never reaches the browser and every
  // polyline vertex arrives already converted GCJ-02→WGS-84. Real-traffic
  // durations come back per leg, formatted exactly like the OSRM branch below.
  if (profile.startsWith('amap:')) {
    const amapProfile = profile.slice('amap:'.length)
    if (amapProfile !== 'driving' && amapProfile !== 'walking' && amapProfile !== 'cycling') {
      throw new Error(`Unknown AMap profile: ${profile}`)
    }
    const route = await mapsApi.amapRoute(amapProfile, waypoints.map((p) => ({ lat: p.lat, lng: p.lng })), signal)
    const legs: RouteSegment[] = route.legs.map((leg, i): RouteSegment => {
      const from: [number, number] = [waypoints[i].lat, waypoints[i].lng]
      const to: [number, number] = [waypoints[i + 1].lat, waypoints[i + 1].lng]
      const mid: [number, number] = [(from[0] + to[0]) / 2, (from[1] + to[1]) / 2]
    ...
    const result: RouteWithLegs = { coordinates: route.coordinates, distance: route.distance, duration: route.duration, legs }
    routeCache.set(cacheKey, result)
    if (routeCache.size > ROUTE_CACHE_MAX) {
      const oldest = routeCache.keys().next().value
      if (oldest !== undefined) routeCache.delete(oldest)
