...
const mapMock = vi.hoisted(() => ({
  ...
  getPane: vi.fn(function (this: void, name: string) {
    return mapMock.panes.get(name);
  }),
  ...
  Marker: ({ children, eventHandlers, position, icon, zIndexOffset }: any) => (
    ...
    >
      <button
        ...
        onClick={(e: any) => {
          e.stopPropagation();
          eventHandlers?.mouseover?.({ originalEvent: { clientX: 100, clientY: 100 } });
        }}
      />
      <button
        ...
        onClick={(e: any) => {
          e.stopPropagation();
          eventHandlers?.mousemove?.({ originalEvent: { clientX: 150, clientY: 160 } });
        }}
      />
      <button
...
describe('MapView', () => {
  ...
  it('FE-COMP-MAPVIEW-020: a day fit expands to include the route once it arrives (#1128)', async () => {
    ...
    const dayPlaces = [buildMapPlace({ id: 1, lat: 48.0, lng: 2.0 }), buildMapPlace({ id: 2, lat: 48.1, lng: 2.1 })];
    ...
    const lastBounds = () => {
      const c = L.latLngBounds.mock.calls;
  ...
  it('FE-COMP-MAPVIEW-073: a category icon that throws leaves the marker circle intact', () => {
    ...
    CATEGORY_ICON_MAP[key] = (() => {
      throw new Error('icon blew up');
    }) as unknown as (typeof CATEGORY_ICON_MAP)['MapPin'];
    try {
...
const leafletMock = async () =>
  (await import('leaflet')).default as unknown as {
...
const markersWithZ = (z: string) =>
  screen.queryAllByTestId('marker').filter((m) => m.getAttribute('data-zindex') === z);
...
describe('MapView explore POIs', () => {
  ...
  it('FE-COMP-MAPVIEW-036: POIs of one category share a single cached icon', async () => {
    ...
    const poiIcons = L.divIcon.mock.calls.filter(
      (c) => JSON.stringify((c[0] as { iconSize: number[] }).iconSize) === '[26,26]'
    );
...
describe('MapView plugin route vias', () => {
  ...
  it('FE-COMP-MAPVIEW-039: vias of the same tone share one cached icon', async () => {
    ...
    const viaIcons = L.divIcon.mock.calls.filter(
      (c) => JSON.stringify((c[0] as { iconSize: number[] }).iconSize) === '[13,13]'
    );
...
describe('MapView — untrusted values in the marker HTML', () => {
  const iconHtml = () =>
    screen
      ...
      .map((el) => el.getAttribute('data-icon-html') || '')
      .join('');
      ...
      <MapView
        places={[
          buildMapPlace({
            lat: 48,
            lng: 2,
            category_color: 'red" onmouseover="alert(1)',
          }),
        ]}
      />
    );
