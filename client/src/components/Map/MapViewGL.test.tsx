...
describe('MapViewGL', () => {
  ...
  it('FE-COMP-MAPVIEWGL-005: adds the clustered place source + layers so markers group on zoom-out (#1385)', async () => {
    ...
    render(
      <MapViewGL places={[buildMapPlace({ id: 1, lat: 48.8584, lng: 2.2945 })]} fitKey={1} glProvider="maplibre-gl" />
    );
    await act(async () => {});
  ...
  it('FE-COMP-MAPVIEWGL-008: a second finger (pinch) cancels the long-press (#1398)', async () => {
    ...
    try {
      ...
      act(() => {
        glCanvasContainer.dispatchEvent(touchEvent('touchstart', [{ clientX: 30, clientY: 40 }]));
        glCanvasContainer.dispatchEvent(
  ...
  it('FE-COMP-MAPVIEWGL-012: a right-button rotate/pitch drag does not open Add-Place on release (#1398)', async () => {
    ...
    act(() => {
      ...
      handler({
        lngLat: { lat: 1, lng: 2 },
        originalEvent: new MouseEvent('contextmenu', { clientX: 140, clientY: 90 }),
      });
    });
    expect(onContext).not.toHaveBeenCalled();
    ...
    act(() => {
      ...
      handler({
        lngLat: { lat: 1, lng: 2 },
        originalEvent: new MouseEvent('contextmenu', { clientX: 11, clientY: 10 }),
      });
    });
    expect(onContext).toHaveBeenCalledTimes(1);
  });
  ...
  it('FE-COMP-MAPVIEWGL-011: clicking a marker clears the hover card; movestart clears + suppresses it (#1404)', async () => {
    ...
    const markerCall = (mapboxgl.Marker as unknown as ReturnType<typeof vi.fn>).mock.calls.find((c) => c[0]?.element);
  ...
  it('FE-COMP-MAPVIEWGL-014: fits bounds immediately even when MapLibre loaded() is false', async () => {
    ...
    const { rerender } = render(<MapViewGL places={places} dayPlaces={places} fitKey={1} glProvider="maplibre-gl" />);
    await act(async () => {});
  ...
  it('FE-COMP-MAPVIEWGL-015: fits MapLibre bounds to route geometry when it arrives after a day fit', async () => {
    ...
    const { rerender } = render(
      <MapViewGL places={dayPlaces} dayPlaces={dayPlaces} route={straightLines} fitKey={1} glProvider="maplibre-gl" />
    );
    await act(async () => {});

    // Pick a day: fits the markers, with only the straight-line route to go on so far.
    rerender(
      <MapViewGL places={dayPlaces} dayPlaces={dayPlaces} route={straightLines} fitKey={2} glProvider="maplibre-gl" />
    );
    await act(async () => {});
    ...
    rerender(
      <MapViewGL places={dayPlaces} dayPlaces={dayPlaces} route={roadGeometry} fitKey={2} glProvider="maplibre-gl" />
    );
    await act(async () => {});
  ...
  describe('opening camera', () => {
    ...
    it('FE-COMP-MAPVIEWGL-019: falls back to the world view when no place has coordinates', async () => {
      render(<MapViewGL places={[buildMapPlace({ id: 1, lat: null, lng: null })]} glProvider="maplibre-gl" />);
      await act(async () => {});
  ...
  it('FE-COMP-MAPVIEWGL-016: leaves the camera alone when a route appears long after the fit', async () => {
    ...
    const { rerender } = render(
      <MapViewGL places={dayPlaces} dayPlaces={dayPlaces} route={null} fitKey={1} glProvider="maplibre-gl" />
    );
    await act(async () => {});

    // Pick a day with the route toggle off: no route is pending for this fit.
    rerender(<MapViewGL places={dayPlaces} dayPlaces={dayPlaces} route={null} fitKey={2} glProvider="maplibre-gl" />);
    await act(async () => {});
    const afterDayFit = glMap.fitBounds.mock.calls.length;
    expect(afterDayFit).toBeGreaterThan(0);
      ...
      <MapViewGL
        places={[
          buildMapPlace({
            ...
            lat: 48.13,
            lng: 2.13,
            image_url: '/uploads/x" onerror="alert(1)" y="',
          }),
        ]}
        fitKey={1}
      />
  ...
  it('FE-COMP-MAPVIEWGL-024: the hover card shows name, category and address and follows the cursor', async () => {
    ...
    act(() => {
      el.dispatchEvent(new MouseEvent('mouseenter', { clientX: 40, clientY: 60 }));
    });
    ...
    act(() => {
      el.dispatchEvent(new MouseEvent('mousemove', { clientX: 100, clientY: 120 }));
    });
  ...
  it('FE-COMP-MAPVIEWGL-026: a place that disappears takes its marker and hover card with it', async () => {
    ...
    act(() => {
      droppedEl.dispatchEvent(new MouseEvent('mouseenter', { clientX: 10, clientY: 10 }));
    });
    expect(queryByTestId('tooltip')).toBeTruthy();
  ...
  it('FE-COMP-MAPVIEWGL-067: hovering does not rebuild the markers under the cursor (#1404)', async () => {
    loadOnAttach();
    const places = [buildMapPlace({ id: 41, lat: 48.41, lng: 2.41 }), buildMapPlace({ id: 42, lat: 48.42, lng: 2.42 })];
  ...
  it.each([
    ...
    'FE-COMP-MAPVIEWGL-068: the %s hover card clears its marker without a tail',
    async (provider, popupOf) => {
      loadOnAttach();
  ...
  it('FE-COMP-MAPVIEWGL-027: clicking a GPX track selects its place unless a marker or cluster is on top', async () => {
    ...
    act(() => {
      selectTrack(trackEvent(document.createElement('div')));
    });
    ...
    act(() => {
      selectTrack(trackEvent(markerEl));
    });
  ...
  it('FE-COMP-MAPVIEWGL-028: hovering a track turns the cursor into a pointer', async () => {
    ...
    act(() => {
      layerHandler('mouseenter', 'trip-gpx-hit')();
    });
    expect(canvas.style.cursor).toBe('pointer');
    act(() => {
      layerHandler('mouseleave', 'trip-gpx-hit')();
    });
    expect(canvas.style.cursor).toBe('');

    act(() => {
      layerHandler('mouseenter', 'trip-place-clusters-circle')();
    });
    expect(canvas.style.cursor).toBe('pointer');
    act(() => {
      layerHandler('mouseleave', 'trip-place-clusters-circle')();
    });
    expect(canvas.style.cursor).toBe('');
  });
  ...
  it('FE-COMP-MAPVIEWGL-029: clicking a cluster bubble eases in to its expansion zoom', async () => {
    ...
    const clusterSource: {
      ...
      getClusterExpansionZoom: vi.fn((_id: number, cb: (err: Error | null, zoom: number) => void) => {
        cb(null, 12);
        return undefined;
      }),
    };
  ...
  it('FE-COMP-MAPVIEWGL-030: a labelled plugin layer feature answers a click with a plain-text popup', async () => {
    ...
    act(() => {
      showLabel({ lngLat: { lng: 2, lat: 48 }, features: [{ properties: { label: 'Charging corridor' } }] });
    });
    ...
    act(() => {
      showLabel({ lngLat: { lng: 2, lat: 48 }, features: [{ properties: { label: '' } }] });
    });
  ...
  it('FE-COMP-MAPVIEWGL-031: a map click on a marker, a cluster or a track never drops a new place', async () => {
    ...
    act(() => {
      click({ point: { x: 1, y: 1 }, lngLat: { lat: 1, lng: 2 }, originalEvent: { target: markerEl } });
    });
    ...
    act(() => {
      click({ point: { x: 1, y: 1 }, lngLat: { lat: 1, lng: 2 }, originalEvent: { target: glCanvasContainer } });
    });
    expect(onMapClick).not.toHaveBeenCalled();
  ...
  it('FE-COMP-MAPVIEWGL-037: markers stay put while the terrain DEM is still streaming in', async () => {
    ...
    glMap.queryTerrainElevation.mockImplementation(() => {
  ...
  it('FE-COMP-MAPVIEWGL-043: plugin markers render as tone dots with a text-only popup', async () => {
    ...
    act(() => {
      glMarkers.created[0].element.dispatchEvent(new MouseEvent('click'));
    });
    ...
    act(() => {
      glMarkers.created[1].element.dispatchEvent(new MouseEvent('click'));
    });
  ...
  it('FE-COMP-MAPVIEWGL-044: plugin layer features become one geojson source with closed rings', async () => {
    ...
    const features: PluginMapLayerFeature[] = [
      ...
      },
      {
        ...
        center: [48, 2],
        radiusM: 1000,
        tone: 'nonsense' as PluginMapLayerFeature['tone'],
        width: 2,
        dash: 'dot',
        opacity: 0.4,
        fill: false,
  ...
  it('FE-COMP-MAPVIEWGL-046: plugin route via points show their label and dwell time on tap', async () => {
    ...
    act(() => {
      glMarkers.created[2].element.dispatchEvent(new MouseEvent('click'));
    });
  ...
  it('FE-COMP-MAPVIEWGL-052: a fit that throws before the style is ready is retried on load', async () => {
    const places = [buildMapPlace({ id: 82, lat: 48.1, lng: 2.1 }), buildMapPlace({ id: 83, lat: 48.2, lng: 2.2 })];
    const { rerender } = render(<MapViewGL places={places} fitKey={1} />);
    await act(async () => {});

    glMap.fitBounds.mockImplementationOnce(() => {
  ...
  it('FE-COMP-MAPVIEWGL-053: on a phone it shows the location FAB, uses mobile padding and hides the hover card', async () => {
    ...
    try {
      const places = [buildMapPlace({ id: 84, lat: 48.1, lng: 2.1 }), buildMapPlace({ id: 85, lat: 48.2, lng: 2.2 })];
      ...
      act(() => {
        glMarkers.created[0].element.dispatchEvent(new MouseEvent('mouseenter', { clientX: 5, clientY: 5 }));
      });
      expect(queryByTestId('tooltip')).toBeNull();
  ...
  it('FE-COMP-MAPVIEWGL-054: desktop padding leaves room for the side panels and the inspector', async () => {
    const places = [buildMapPlace({ id: 86, lat: 48.1, lng: 2.1 }), buildMapPlace({ id: 87, lat: 48.2, lng: 2.2 })];
  ...
  it('FE-COMP-MAPVIEWGL-056: an in-flight photo is not requested twice and the proxy url is used as the id', async () => {
    ...
    const places = [
      ...
      buildMapPlace({
        ...
        lng: 2.5,
        osm_id: 'osm-5',
        image_url: '/api/maps/place-photo/abc',
        name: 'Museum',
      }),
    ];
  ...
  it('FE-COMP-MAPVIEWGL-058: unmounting unsubscribes the thumb listeners and drops the map', async () => {
    ...
    glMap.remove.mockImplementation(() => {
      throw new Error('context already lost');
    });

    const { unmount } = render(
      <MapViewGL places={[buildMapPlace({ id: 97, lat: 48, lng: 2, osm_id: 'osm-7' })]} fitKey={1} />
    );
    await act(async () => {});
    // A thumb arriving just before unmount leaves a frame scheduled.
    act(() => {
      thumbListeners['osm-7']?.('data:image/png;base64,CCC');
    });
  ...
  it('FE-COMP-MAPVIEWGL-061: plugin markers are torn down when the trip loses them', async () => {
    loadOnAttach();
    server.use(
      http.get('/api/map-markers/:tripId', ({ params }) =>
        HttpResponse.json({
          markers:
            params.tripId === '4'
