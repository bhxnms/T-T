...
function renderView(entry: JourneyEntry, props: Partial<ViewProps> = {}) {
  ...
  const result = render(
    <MobileEntryView entry={entry} readOnly={props.readOnly} publicPhotoUrl={props.publicPhotoUrl} {...handlers} />
  );
...
describe('MobileEntryView', () => {
  ...
  it('FE-COMP-JENTRYVIEW-010: a public journey builds its own photo urls', () => {
    const { container } = renderView(buildEntry({ photos: [buildPhoto(4), buildPhoto(5)] }), {
