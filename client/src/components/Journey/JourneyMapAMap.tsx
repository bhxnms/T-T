...
function JourneyMapAMap({
  ...
  const setStyle = useCallback((id: string, highlighted: boolean) => {
    ...
    if (entry && marker?.setContent)
      marker.setContent(markerContent(entry.dayColor || '#52525B', entry.dayLabel ?? 1, highlighted));
  ...
  const popup = useCallback(
    (id: string | null) => {
      ...
      if (!popupRef.current)
        popupRef.current = new AMap.InfoWindow({ isCustom: true, offset: [0, -42], closeClick: false });
    ...
    loadAmap(settingsKey)
      .then((AMap) => {
        ...
        const map = new AMap.Map(hostRef.current, {
          zoom: valid.length ? 5 : 2,
          center: first ? gcj(first.lat, first.lng) : [0, 30],
          lang: 'zh_cn',
          resizeEnable: true,
        });
        mapRef.current = map;
        ...
        const addLine = (points: [number, number][], options: Record<string, unknown>) => {
          if (points.length > 1) {
            const line = new AMap.Polyline({ path: points, ...options });
            line.setMap(map);
            overlays.push(line);
            boundsOverlays.push(line);
          }
        };
        addLine(
          (trail || emptyTrail).map((p) => gcj(p.lat, p.lng)),
          { strokeColor: '#6366f1', strokeWeight: 3, strokeOpacity: 0.4, strokeStyle: 'dashed' }
        );
        for (const track of tracks || emptyTracks)
          addLine(
            track.points.map(([lat, lng]) => gcj(lat, lng)),
            { strokeColor: track.color || '#4f46e5', strokeWeight: 3.5, strokeOpacity: 0.95 }
          );
        valid.forEach((entry, index) => {
          const marker = new AMap.Marker({
            position: gcj(entry.lat, entry.lng),
            content: markerContent(entry.dayColor || '#52525B', entry.dayLabel ?? index + 1, false),
            anchor: 'bottom',
            title: entry.title || '',
          });
          marker.setMap(map);
      ...
      .catch(() => {
        /* ErrorBoundary handles SDK failures */
      });
    return () => {
      cancelled = true;
      popupRef.current?.close();
      if (mapRef.current) {
        mapRef.current.off?.('zoomend', drawPhotos);
        mapRef.current.off?.('moveend', drawPhotos);
      }
      clearPhotoOverlays();
  ...
  useEffect(() => {
    if (activeMarkerId) {
      const timer = setTimeout(() => focusMarker(activeMarkerId), 50);
      return () => clearTimeout(timer);
    }
  }, [activeMarkerId, focusMarker]);
  useEffect(() => {
    drawPhotos();
    return () => {
      clearPhotoOverlays();
    };
  }, [photos, drawPhotos, clearPhotoOverlays]);
