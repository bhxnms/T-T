...
const glModule = vi.hoisted(() => () => ({
  default: {
    ...
    Marker: vi.fn(function (options: { element?: HTMLElement }) {
      return gl.createMarker(options);
    }),
    Popup: vi.fn(function (options: Record<string, unknown>) {
      return gl.createPopup(options);
    }),
    LngLatBounds: vi.fn(function () {
      return gl.createBounds();
    }),
  },
}));
...
const JourneyMapGL = React.forwardRef(function JourneyMapGLShim(
  ...
  return (
    <JourneyMapGLWithEngine ref={ref} {...props} gl={props.glProvider === 'maplibre-gl' ? maplibregl : mapboxgl} />
  );
});
...
describe('JourneyMapGL', () => {
  ...
  it('FE-COMP-JMAPGL-007: two or more entries add the dashed route source and layer', () => {
    ...
    expect(gl.map.addLayer).toHaveBeenCalledWith(
  ...
  it('FE-COMP-JMAPGL-010: the camera fits entries and trail with the requested bottom padding', () => {
    withToken();
    render(
      <JourneyMapGL checkins={[]} entries={entries} trail={[{ lat: 50, lng: 8 }]} paddingBottom={200} fullScreen />
    );

    expect(gl.boundsList[0].points).toEqual([
      [2.3522, 48.8566],
      [13.405, 52.52],
      [8, 50],
    ]);
    expect(gl.map.fitBounds).toHaveBeenCalledWith(
      gl.boundsList[0],
      expect.objectContaining({
  ...
  it('FE-COMP-JMAPGL-012: clicking a pin reports the entry id and stops the map from handling it', () => {
    ...
    act(() => {
      gl.markers[1].element.dispatchEvent(ev);
    });
  ...
  it('FE-COMP-JMAPGL-015: an entry with neither title nor place still gets a heading', () => {
    withToken();
    const ref = React.createRef<JourneyMapGLHandle>();
    render(<JourneyMapGL ref={ref} checkins={[]} entries={[{ id: 'e1', lat: 1, lng: 2, entry_date: '' }]} />);
  ...
  it('FE-COMP-JMAPGL-020: highlightMarker(null) closes the popup', () => {
    ...
    act(() => {
      ref.current!.highlightMarker(null);
    });
  ...
  it('FE-COMP-JMAPGL-021: highlighting an unknown id changes nothing', () => {
    ...
    render(<JourneyMapGL ref={ref} checkins={[]} entries={entries} />);
    act(() => {
      ref.current!.highlightMarker('ghost');
    });
  ...
  it('FE-COMP-JMAPGL-025: focusMarker swallows sdk errors while the map is not ready', () => {
    ...
    const ref = React.createRef<JourneyMapGLHandle>();
    render(<JourneyMapGL ref={ref} checkins={[]} entries={entries} />);
    gl.map.getZoom.mockImplementationOnce(() => {
  ...
  it('FE-COMP-JMAPGL-032: sdk failures while configuring the basemap do not break the load', () => {
    withToken();
    vi.mocked(isStandardFamily).mockReturnValue(true);
    gl.map.setTerrain.mockImplementationOnce(() => {
      throw new Error('no terrain');
    });
    gl.map.setConfigProperty.mockImplementationOnce(() => {
  ...
  it('FE-COMP-JMAPGL-033: an empty bounds fit is swallowed', () => {
    withToken();
    gl.map.fitBounds.mockImplementationOnce(() => {
