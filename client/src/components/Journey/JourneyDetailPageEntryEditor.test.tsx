...
function mountEditor(entry: JourneyEntry, opts: { galleryPhotos?: GalleryPhoto[]; withProviderHook?: boolean } = {}) {
  const onClose = vi.fn();
  const onDone = vi.fn();
...
describe('EntryEditor', () => {
  ...
  it('FE-JRN-EDITOR-009: links gallery photos picked before the entry exists', async () => {
    const linked: string[] = [];
    server.use(
      http.post('/api/journeys/entries/55/link-photo', async ({ request }) => {
  ...
  it('FE-JRN-EDITOR-010: links a gallery photo straight away on an existing entry', async () => {
    let linked = false;
    server.use(
      http.post('/api/journeys/entries/10/link-photo', () => {
        linked = true;
  ...
  it('FE-JRN-EDITOR-011: unlinks a removed photo from an existing entry', async () => {
    let unlinked = false;
    server.use(
      http.delete('/api/journeys/entries/10/photos/100', () => {
        unlinked = true;
  ...
  it('FE-JRN-EDITOR-012: promoting a photo to first persists the new sort order', async () => {
    ...
    server.use(
      http.patch('/api/journeys/photos/:id', async ({ params, request }) => {
  ...
  it('FE-JRN-EDITOR-021: sends queued provider photos on save and keeps the failed ones', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Save' }));
    await waitFor(() => expect(onDone).toHaveBeenCalledTimes(1));
    expect(onAddProviderPhotos).toHaveBeenLastCalledWith(
      55,
      expect.objectContaining({
        provider: 'immich',
        assetIds: ['asset-1'],
  ...
  it('FE-JRN-EDITOR-024: falls back to today and an empty strip when the entry carries neither', () => {
    const { container } = mountEditor(
      buildEntry({
        entry_date: '',
        photos: undefined as unknown as JourneyPhoto[],
      })
    );
  ...
  it('FE-JRN-EDITOR-027: removes a con row again', async () => {
    ...
    const { onSave } = mountEditor(
      buildEntry({
        id: 10,
        pros_cons: { pros: ['Food'], cons: ['Queues', 'Heat'] },
      })
    );
  ...
  it('FE-JRN-EDITOR-029: rolls the order back when persisting it fails', async () => {
    let attempts = 0;
    server.use(
      http.patch('/api/journeys/photos/:id', () => {
        attempts += 1;
  ...
  it('FE-JRN-EDITOR-033: merges a second pick into the already queued group', async () => {
    ...
    expect(onAddProviderPhotos).toHaveBeenCalledWith(
      55,
      expect.objectContaining({
        provider: 'immich',
  ...
  it('FE-JRN-EDITOR-036: shows the upload progress while photos are in flight', async () => {
    ...
    onUploadPhotos.mockImplementationOnce(
      (_entryId, files, cbs) =>
