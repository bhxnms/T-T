...
function GalleryHarness({
  ...
  return (
    <>
      {providers.map((p) => (
        <button type="button" key={p.id} onClick={() => browseRef.current?.(p.id)}>
          {p.name}
        </button>
      ))}
      <GalleryView
...
describe('GalleryView', () => {
  it('FE-JRN-GALLERY-001: shows the mascot empty state when the journey has no photos', async () => {
    ...
    expect(container.querySelector('.tt-mascot--journey')).toBeInTheDocument();
  ...
  it('FE-JRN-GALLERY-007: removes a deleted photo from the store optimistically', async () => {
    let deleted = false;
    server.use(
      http.delete('/api/journeys/9/gallery/100', () => {
        deleted = true;
    ...
    useJourneyStore.setState({
      current: {
        ...
        entries: [
          {
            ...entries[0],
            photos: [{ id: 100, entry_id: 10, photo_id: 100, sort_order: 0, shared: 1, created_at: 0 }],
          },
        ],
        gallery: [photo],
        trips,
        contributors: [],
        stats: { entries: 1, photos: 1, places: 0 },
      },
    });
    const user = userEvent.setup();
  ...
  it('FE-JRN-GALLERY-008: reports a failed delete and asks the parent to refetch', async () => {
    ...
    useJourneyStore.setState({
      current: {
        ...
        trips,
        contributors: [],
        stats: { entries: 0, photos: 1, places: 0 },
      },
    });
    const user = userEvent.setup();
    const { container, onRefresh } = mountGallery([photo]);
  ...
  it('FE-JRN-GALLERY-012: adds picked provider photos to the gallery', async () => {
    ...
    server.use(
      http.post('/api/journeys/9/gallery/provider-photos', async ({ request }) => {
  ...
  it('FE-JRN-GALLERY-014: uploads picked files and refreshes on success', async () => {
    ...
    const { container, onRefresh } = mountGallery([], (fn) => {
      registered = fn;
    });
