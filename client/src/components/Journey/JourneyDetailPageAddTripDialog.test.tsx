...
function mountDialog(props: Partial<React.ComponentProps<typeof AddTripDialog>> = {}) {
  const onClose = vi.fn();
  const onAdded = vi.fn();
  render(<AddTripDialog journeyId={4} existingTripIds={[]} onClose={onClose} onAdded={onAdded} {...props} />);
...
describe('AddTripDialog', () => {
  ...
  it('FE-JRN-ADDTRIP-006: links the picked trip and notifies the parent', async () => {
    ...
    server.use(
      http.post('/api/journeys/4/trips', async ({ request }) => {
  ...
  it('FE-JRN-ADDTRIP-007: shows a busy state on the row while the link request runs', async () => {
    server.use(
      http.post('/api/journeys/4/trips', async () => {
        await delay(40);
        return HttpResponse.json({ ok: true });
      })
    );
  ...
  it('FE-JRN-ADDTRIP-009: closes via the header button and survives a failed trip fetch', async () => {
    ...
    const headerClose = screen.getByRole('heading', { name: 'Link Trip' }).parentElement!.querySelector('button')!;
    act(() => {
      headerClose.click();
    });
