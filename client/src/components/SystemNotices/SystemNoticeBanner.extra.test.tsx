...
describe('BannerRenderer', () => {
  ...
  it('FE-W4SNB-009: a nav CTA routes in-app and dismisses the banner', () => {
    render(
      <BannerRenderer
        notices={[notice({ id: 'a', cta: { kind: 'nav', href: '/settings', labelKey: 'Open settings' } })]}
      />
    );
  ...
  it('FE-W4SNB-010: a nav CTA on a sticky banner does not dismiss it', () => {
    render(
      <BannerRenderer
        notices={[
          notice({ id: 'a', dismissible: false, cta: { kind: 'nav', href: '/settings', labelKey: 'Open settings' } }),
        ]}
      />
    );
  ...
  it('FE-W4SNB-011: a link CTA opens a hardened new tab', () => {
    ...
    render(
      <BannerRenderer
        notices={[notice({ id: 'a', cta: { kind: 'link', href: 'https://trek.dev', labelKey: 'Read more' } })]}
      />
    );
  ...
  it('FE-W4SNB-012: an action CTA runs the registered handler and dismisses by default', () => {
    ...
    render(
      <BannerRenderer
        notices={[notice({ id: 'a', cta: { kind: 'action', actionId: 'open-support', labelKey: 'Support' } })]}
      />
    );
    ...
    render(
      <BannerRenderer
        notices={[
          notice({
            id: 'a',
            cta: { kind: 'action', actionId: 'stay-open', labelKey: 'Support', dismissOnAction: false },
          }),
        ]}
      />
    );
...
describe('ToastRenderer', () => {
  ...
  it('FE-W4SNB-029: an unmapped severity falls back to the info variant', () => {
    render(
      <ToastRenderer
        notices={[notice({ id: 't1', display: 'toast', severity: 'legendary' as SystemNoticeDTO['severity'] })]}
      />
    );
