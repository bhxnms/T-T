      ...
      <div className="flex flex-col justify-center" style={{ flex: '1 1 0' }}>
        ...
        {notice.icon === 'Heart' && !notice.media && (
          <div className="relative overflow-hidden bg-gradient-to-br from-rose-500 via-pink-500 to-indigo-500 px-8 py-6 text-center">
            ...
            />
            <h2 id={titleId} className="relative text-xl font-bold leading-tight text-white">
              {title}
            </h2>
          </div>
        )}
            ...
            <div className="mb-4 flex flex-wrap justify-center gap-2">
              {notice.highlights.map((h, i) => {
                ...
                return (
                  ...
                  >
                    {HIcon ? (
                      <HIcon size={13} className="shrink-0 text-indigo-500 dark:text-indigo-400" />
                    ) : (
          ...
          {ctaLabel && isLastPage ? (
            <div className="flex w-full flex-col gap-2.5 sm:flex-row">
              <button
                ...
                onClick={onCTA}
                className={`inline-flex h-11 flex-1 items-center justify-center gap-2 rounded-lg font-semibold shadow-sm transition active:scale-[0.98] ${
              ...
              >
                {primaryBrand ? (
                  <BrandIcon brand={primaryBrand} size={18} />
                ) : (
                  notice.cta?.kind === 'link' && <Coffee size={17} aria-hidden="true" />
                )}
                {ctaLabel}
              </button>
              {secondaryCtaLabel && (
                ...
                >
                  {secondaryBrand ? (
                    <BrandIcon brand={secondaryBrand} size={18} />
                  ) : (
                    notice.secondaryCta?.kind === 'link' && <Coffee size={17} aria-hidden="true" />
                  )}
                  {secondaryCtaLabel}
                </button>
              )}
