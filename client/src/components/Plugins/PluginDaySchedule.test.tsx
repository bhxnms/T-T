...
describe('dayTintBackground', () => {
  it("FE-W4PDS-015: mixes a tone at the region's own alpha, over the caller's base", () => {
    expect(dayTintBackground({ headerTone: 'warn' }, 'header', '--day-tint-header')).toBe(
      'color-mix(in srgb, #f59e0b var(--day-tint-header), transparent)'
    );
    expect(dayTintBackground({ badgeTone: 'danger' }, 'badge', '--day-tint-badge', 'var(--bg-hover)')).toBe(
      'color-mix(in srgb, #ef4444 var(--day-tint-badge), var(--bg-hover))'
    );
  });
  ...
  it("FE-W4PDS-016: clamps a plugin's own colour into the theme's lightness band", () => {
    ...
    expect(dayTintBackground({ badgeColor: '#00ff00' }, 'badge', '--day-tint-badge')).toBe(
      'color-mix(in srgb, oklch(from #00ff00 clamp(var(--day-tint-l-min), l, var(--day-tint-l-max)) c h) var(--day-tint-badge), transparent)'
    );
  });
  ...
  it('FE-W4PDS-019: falls back to the default hex for an unknown tone', () => {
    expect(dayTintBackground({ headerTone: 'nope' as 'default' }, 'header', '--a')).toBe(
      'color-mix(in srgb, #4F46E5 var(--a), transparent)'
    );
  });
