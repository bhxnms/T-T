...
function readAppearance() {
  ...
  return {
    ...
    reducedMotion:
      el.hasAttribute('data-reduce-motion') || window.matchMedia('(prefers-reduced-motion: reduce)').matches,
  };
}
        ...
        case 'trek:geolocation': {
          ...
          if (action === 'watch') {
            ...
            geoWatchRef.current = navigator.geolocation.watchPosition(
              (p) => {
                if (loadsRef.current <= 1 && stillAllowed())
                  post({ type: 'trek:geolocation:update', position: geoPosition(p) });
              },
              (e) => {
                if (loadsRef.current <= 1 && stillAllowed())
                  post({ type: 'trek:geolocation:update', error: geoErrorCode(e) });
              },
              geoOpts
            );
            break;
          ...
          navigator.geolocation.getCurrentPosition(
            (p) => {
              if (loadsRef.current <= 1) post({ type: 'trek:geolocation:result', requestId, position: geoPosition(p) });
            },
            (e) => {
              if (loadsRef.current <= 1) fail(geoErrorCode(e));
            },
            geoOpts
          );
          break;
        ...
        case 'trek:session:clear': {
          ...
          if (scope === 'trip' && tripId == null) {
            post({
              ...
              message: 'trip session storage requires a trip context',
            });
            break;
          }
        ...
        case 'trek:session:remove': {
          ...
          if (typeof msg.key !== 'string' || msg.key.length === 0 || msg.key.length > PLUGIN_SESSION_MAX_KEY_LENGTH) {
            post({
              type: 'trek:error',
              requestId: msg.requestId,
              code: 'SESSION_INVALID_KEY',
              message: `session key must be 1-${PLUGIN_SESSION_MAX_KEY_LENGTH} characters`,
            });
            break;
          }
          const { scopeKeyPrefix, storageKey } = createPluginSessionStorageKeys(
            userId,
            pluginId,
            scope,
            tripId,
          ...
          try {
            if (msg.type === 'trek:session:get') {
              ...
              post({
                type: 'trek:response',
                requestId: msg.requestId,
                data: storedValue === null ? undefined : JSON.parse(storedValue),
              });
              break;
            }
            ...
            if (msg.type === 'trek:session:set') {
              ...
              if (serializedValue === undefined) {
                post({
                  type: 'trek:error',
                  requestId: msg.requestId,
                  code: 'SESSION_INVALID_VALUE',
                  message: 'session value must be JSON-serialisable',
                });
                break;
              }
              ...
              if (valueBytes > PLUGIN_SESSION_MAX_VALUE_BYTES) {
                post({
                  ...
                  message: `session value exceeds ${PLUGIN_SESSION_MAX_VALUE_BYTES} bytes`,
                });
                break;
              }
              ...
              if (!keys.includes(storageKey) && keys.length >= PLUGIN_SESSION_MAX_KEYS) {
                post({
                  type: 'trek:error',
                  requestId: msg.requestId,
                  code: 'SESSION_KEY_LIMIT',
                  message: `plugin session storage allows at most ${PLUGIN_SESSION_MAX_KEYS} keys`,
                });
                break;
              }
    ...
    <ErrorBoundary boundaryId="plugin-frame" label={pluginLabel} resetKeys={[pluginId]}>
      <>
        ...
        {loadFailed && (
          <div
            role="alert"
            className="bg-surface-2 text-content-2 flex items-center justify-center rounded-xl border border-edge p-6 text-caption"
