...
const viewContributions = vi.fn(async (_view: string, _tripId: string | number) => ({
  contributions: [] as ViewContribution[],
}));
...
import { PluginActions, PluginCardFooter, PluginColumns, usePluginViewContributions } from './PluginContributions';
...
function action(overrides: Partial<Extract<ViewContribution, { kind: 'action' }>> = {}): ViewContribution {
  return {
    ...
    id: 'a1',
    label: 'Recalculate',
    target: { kind: 'route', method: 'POST', sub: 'recalc' },
    ...overrides,
  };
}
...
describe('usePluginViewContributions', () => {
  it('FE-W4PCON-001: buckets contributions by entity id', async () => {
    viewContributions.mockResolvedValue({
      contributions: [
        column({ entityId: 1, id: 'a' }),
        column({ entityId: 2, id: 'b' }),
        column({ entityId: 1, id: 'c' }),
      ],
    });
  ...
  it('FE-W4PCON-006: refetches when the view changes', async () => {
    const { rerender } = renderHook(({ view }: { view: 'places' | 'day' }) => usePluginViewContributions(view, 1), {
...
describe('PluginColumns', () => {
  ...
  it('FE-W4PCON-009: falls back to the muted tone for an unknown tone', () => {
    render(
      <PluginColumns items={[column({ tone: 'weird' as Extract<ViewContribution, { kind: 'column' }>['tone'] })]} />
    );
...
describe('PluginActions', () => {
  ...
  it('FE-W4PCON-015: a frame action opens the sandboxed modal and closes on the X', () => {
    render(
      <PluginActions items={[action({ label: 'Configure', target: { kind: 'frame', sub: 'config' } })]} tripId={9} />
    );
