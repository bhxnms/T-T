    ...
    <div className="flex flex-col gap-1">
      {cols.map((c) => (
        <div key={c.pluginId + c.id} className="flex items-baseline justify-between gap-2 text-xs">
          <span className="shrink-0 font-medium text-content-secondary">{c.label}</span>
          {c.url ? (
            <a
              href={c.url}
              target="_blank"
              rel="noreferrer noopener"
              className="truncate text-right text-accent"
              onClick={(e) => e.stopPropagation()}
            >
              {c.value ?? '↗'}
            </a>
          ) : (
            <span className={`${TONE_CLASS[c.tone] ?? TONE_CLASS.default} truncate text-right`}>{c.value}</span>
          )}
        </div>
      ))}
    </div>
...
export function PluginCardFooter({ items, tripId }: { items: ViewContribution[]; tripId: number }) {
  ...
  return (
    <div
      ...
      style={{ marginTop: 8, paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}
    >
      <PluginColumns items={items} />
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        <PluginActions items={items} tripId={tripId} />
      </div>
    </div>
  );
}
...
export function PluginActions({
  ...
  const run = (a: Action, e: React.MouseEvent) => {
    e.stopPropagation();
    if (a.target.kind === 'frame') {
      setFrame({ pluginId: a.pluginId, label: a.label });
      return;
    }
    void pluginsApi
      .invoke(a.pluginId, a.target.sub, { method: a.target.method })
    ...
    <>
      {actions.map((a) => (
        <button
          ...
          className={
            className ??
            'inline-flex h-7 items-center rounded-lg border border-edge bg-surface-card px-2 text-xs text-content-muted transition-colors hover:border-content-faint hover:text-content'
          }
        >
          {a.label}
        </button>
      ...
      {frame && (
        ...
        >
          <div
            role="presentation"
            className="flex h-[70vh] w-full max-w-lg flex-col overflow-hidden rounded-xl bg-surface-card shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex shrink-0 items-center justify-between border-b border-edge px-4 py-2.5">
              <span className="truncate text-sm font-medium text-content">{frame.label}</span>
              <button type="button" onClick={() => setFrame(null)} className="text-content-muted hover:text-content">
                <X size={16} />
              </button>
            </div>
