...
describe('Spinner', () => {
  ...
  it('FE-W4SPIN-002: keeps caller classes and still spins', () => {
    const { container } = render(<Spinner className="h-10 w-10 border-4 border-accent" />);
    const el = container.firstElementChild as HTMLElement;
...
describe('PageSpinner', () => {
  ...
  it('FE-W4SPIN-004: forwards wrapper classes and style', () => {
    const { container } = render(
      <PageSpinner wrapperClassName="h-screen grid place-items-center" wrapperStyle={{ minHeight: 200 }} />
    );
    const wrapper = container.firstElementChild as HTMLElement;
  ...
  it('FE-W4SPIN-005: forwards the ring className down to the Spinner', () => {
    const { container } = render(<PageSpinner className="h-8 w-8 border-2" />);

    expect((container.querySelector('.animate-spin') as HTMLElement).className).toBe(
