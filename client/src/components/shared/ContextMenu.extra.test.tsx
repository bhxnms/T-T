...
describe('useContextMenu', () => {
  it('FE-W4CTX-001: opening stores the pointer position and the items', () => {
    ...
    act(() => {
      result.current.open(
        { clientX: 120, clientY: 40, preventDefault, stopPropagation } as unknown as React.MouseEvent,
        items
      );
    });
  ...
  it('FE-W4CTX-002: closing clears the menu', () => {
    ...
    act(() => {
      result.current.open(
        { clientX: 0, clientY: 0, preventDefault() {}, stopPropagation() {} } as unknown as React.MouseEvent,
        []
      );
    });
...
describe('ContextMenu', () => {
  ...
  it('FE-W4CTX-004: renders one button per item at the pointer position', () => {
    const { baseElement } = render(
      <ContextMenu
        menu={{ x: 40, y: 20, items: [{ label: 'Open' }, { label: 'Delete', icon: Trash2, danger: true }] }}
        onClose={() => {}}
      />
    );
  ...
  it('FE-W4CTX-005: a danger item is red and carries its icon', () => {
    render(
      <ContextMenu menu={{ x: 0, y: 0, items: [{ label: 'Delete', icon: Trash2, danger: true }] }} onClose={() => {}} />
    );
  ...
  it('FE-W4CTX-006: a divider entry renders a rule instead of a button', () => {
    render(
      <ContextMenu
        menu={{ x: 0, y: 0, items: [{ label: 'Open' }, { divider: true }, { label: 'Delete' }] }}
        onClose={() => {}}
      />
    );
  ...
  it('FE-W4CTX-010: flips the menu back inside the viewport when it would overflow', () => {
    ...
    const { baseElement } = render(
      <ContextMenu menu={{ x: 380, y: 290, items: [{ label: 'Open' }] }} onClose={() => {}} />
    );
