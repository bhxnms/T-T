...
function Harness({
  ...
  return (
    <NumericInput value={v} onValueChange={setV} mode={mode} signToggleLabel={signToggleLabel} aria-label="field" />
  );
}
...
describe('NumericInput', () => {
  ...
  it('FE-COMP-NUMINPUT-008: a caller-supplied onFocus still runs alongside the select', () => {
    const onFocus = vi.fn();
    render(<NumericInput value="7" onValueChange={() => {}} onFocus={onFocus} aria-label="field" />);
    const input = screen.getByLabelText('field') as HTMLInputElement;
