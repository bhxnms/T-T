    ...
    >
      ...
      {avg !== null ? (
        ...
        >
          ...
          <span style={{ fontWeight: 400, color: 'var(--text-faint)' }}>({ratings.length})</span>
        </span>
      ) : (
      ...
      {tip &&
        ...
        createPortal(
          <div
            role="tooltip"
            className="border border-edge-faint bg-surface-card text-content"
            style={{
              position: 'fixed',
              top: tip.top - 8,
              left: tip.left,
          ...
          >
            {ratings.map((r) => (
              <div key={r.user_id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0' }}>
                ...
                >
                  {r.username}
                  {r.user_id === currentUserId ? ` (${t('vacay.you')})` : ''}
                </span>
