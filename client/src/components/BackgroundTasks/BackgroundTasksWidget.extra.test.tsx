...
import { fireEvent, render } from '../../../tests/helpers/render';
import { healthApi, reservationsApi } from '../../api/client';
...
describe('BackgroundTasksWidget — websocket', () => {
  it('FE-W4BGT-008: import:progress updates the running card', () => {
    ...
    act(() => {
      handler({ type: 'import:progress', jobId: 'j1', tripId: 't1', done: 2, total: 4 });
    });
  ...
  it('FE-W4BGT-009: import:done attaches the parsed items', () => {
    ...
    act(() => {
      handler({ type: 'import:done', jobId: 'j1', tripId: 't1', result: { items: [{ id: 1 }], warnings: [] } });
    });
  ...
  it('FE-W4BGT-010: import:error surfaces the message', () => {
    ...
    act(() => {
      handler({ type: 'import:error', jobId: 'j1', tripId: 't1', message: 'boom' });
    });
...
describe('BackgroundTasksWidget — AI retry', () => {
  const withFiles = () =>
    task({
      items: [],
      sourceFiles: [new File(['%PDF'], 'voucher.pdf', { type: 'application/pdf' })],
    });
  ...
  it('FE-W4BGT-018: hides the retry on a job that already ran with force-ai', async () => {
    ...
    useBackgroundTasksStore.setState({
      tasks: [task({ items: [], mode: 'force-ai', sourceFiles: [new File([''], 'a.pdf')] })],
    });
  ...
  it('FE-W4BGT-020: a refused retry surfaces the server error on the original card', async () => {
    vi.mocked(healthApi.features).mockResolvedValue({ aiParsing: true } as never);
    vi.mocked(reservationsApi.importBookingAsync).mockRejectedValue({
      response: { data: { error: 'No model configured' } },
    });
    useBackgroundTasksStore.setState({ tasks: [withFiles()] });
    render(<BackgroundTasksWidget />);
