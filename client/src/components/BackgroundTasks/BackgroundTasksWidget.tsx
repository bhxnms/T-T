...
export default function BackgroundTasksWidget() {
  ...
  useEffect(() => {
    ...
    for (const task of restored) {
      reservationsApi
        ...
        .then((s) => {
          if (s.status === 'done')
            setDone(task.id, task.tripId, (s.result?.items ?? []) as never, s.result?.warnings ?? []);
  ...
  useEffect(() => {
    const pending = tasks.filter(
      (task) => task.status === 'running' || (task.status === 'done' && task.items === undefined)
    );
      ...
      {tasks.map((task) => (
        ...
        >
          <div style={{ flexShrink: 0, marginTop: 1 }}>
            {(task.status === 'running' || (task.status === 'done' && task.items === undefined)) && (
                  ...
                  {aiParsing && task.mode !== 'force-ai' && task.sourceFiles && task.sourceFiles.length > 0 && (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: 'calc(11.5px * var(--fs-scale-caption, 1))',
                        fontWeight: 600,
                        cursor: retrying === task.id ? 'default' : 'pointer',
                        opacity: retrying === task.id ? 0.6 : 1,
                        fontFamily: 'inherit',
                      }}
                    >
