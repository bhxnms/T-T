...
export default function AdminStoragePanel(): React.ReactElement {
  ...
  if (admin.loading) {
    return <p className="p-4 text-sm italic text-content-faint">{t('storage.loading')}</p>;
  }
  if (!admin.state || !admin.draft) {
    return (
      <div role="alert" className="rounded-xl border border-edge bg-surface-card p-4 text-sm text-content">
  ...
  return (
    <div>
      ...
      <Section title={t('storage.health.title')} icon={Activity}>
        ...
        {state.seedFilePresent && <p className="mt-2 text-xs text-content-faint">{t('storage.health.seedFile')}</p>}
      </Section>
        ...
        <div className="space-y-3">
          {rows.map((row) => {
            ...
            return (
              <div
                key={row.name}
                data-testid={`storage-backend-${row.name}`}
                className="rounded-xl border border-edge-secondary p-4"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-content">{row.name}</span>
                  <span className="rounded-full border border-edge px-2 py-0.5 text-xs text-content-secondary">
                    {t(`storage.type.${row.type}`)}
                  </span>
                  <span className="rounded-full border border-edge px-2 py-0.5 text-xs text-content-faint">
                ...
                {row.source === 'env' && (
                  <p className="mt-1 text-xs text-content-faint">{t('storage.backends.envReadOnly')}</p>
                )}
                {result === 'running' ? (
                  <p className="mt-2 text-xs text-content-faint">{t('storage.test.running')}</p>
                ) : (
                  <TestResult
                  ...
                  <div className="mt-2">
                    {backfill?.status === 'running' ? (
                      <>
                        ...
                        <button
                          type="button"
                          className="mt-1 text-xs text-content-secondary underline"
                          style={LINK_BUTTON_STYLE}
                ...
                {syncPrompt === row.name && (
                  <div className="mt-2 rounded-lg border border-edge-secondary p-2">
          ...
          {degenerate.map(({ backend, reason }) => {
            ...
            return (
              <div
                key={backend.name}
                data-testid={`storage-backend-${backend.name}`}
                className="rounded-xl border border-edge-secondary p-4"
              >
                <div className="flex flex-wrap items-center gap-2">
            ...
            {state.migrations.map((m) => (
              ...
              >
                ...
                {m.status === 'running' ? (
                  ...
                  <p className="mt-1 text-xs text-content-faint">{t('storage.migrate.cancelled')}</p>
                )}
              </div>
            ))}
          </div>
            ...
            return (
              <div key={category} data-testid={`storage-category-${category}`}>
                <label className="mb-1 block text-sm font-medium text-content-secondary">
                  ...
                  <span className="ml-2 rounded border border-edge px-1.5 py-0.5 font-mono text-xs font-normal text-content-faint">
                    {category}
                  </span>
                </label>
                <p className="mb-1.5 text-xs text-content-faint">{t(`storage.categoryDesc.${category}`)}</p>
                ...
                />
                {changed && (
                  <p role="alert" className="mt-1 text-xs text-content-faint">
                    {t('storage.categories.reassignWarning')}
                  </p>
                )}
                {viaMirror && CACHE_CATEGORIES.includes(category) && (
                  <p role="note" className="mt-1 text-xs text-content-faint">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          style={{
            padding: '10px 20px',
            borderRadius: 10,
            cursor: admin.dirty && !admin.saving ? 'pointer' : 'default',
            fontFamily: 'inherit',
            fontSize: 'calc(14px * var(--fs-scale-body, 1))',
            fontWeight: 600,
      ...
      {migrationQueue.length > 0 && (
        <p className="mt-2 text-xs text-content-faint">
          {t('storage.migrate.queued', {
            categories: categoryNames(
              t,
              migrationQueue.map((c) => c.category)
            ),
          })}
        </p>
      )}
      {migratePromptOpen && (
        <div className="mt-2 rounded-lg border border-edge bg-surface-secondary px-3 py-2" role="alertdialog">
          <p className="text-sm text-content">{t('storage.migrate.promptTitle')}</p>
          {migrationCandidates.map((c) => (
            <p key={c.category} className="mt-1 text-xs text-content-secondary">
              {c.objects === null
      ...
      {admin.saveError && (
        <div role="alert" className="mt-2 text-sm text-content">
          <p>{admin.saveError}</p>
