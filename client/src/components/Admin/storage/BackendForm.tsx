...
const INPUT_CLASS = 'mt-1.5 w-full px-3 py-2 border rounded-lg text-sm border-edge bg-surface-card text-content';
  ...
  const renderField = (field: StorageBackendFieldDef): React.ReactElement => {
    ...
    return (
      <label key={field.key} className={LABEL_CLASS}>
        ...
        />
        {field.helpKey && <span className="mt-1 block text-xs font-normal text-content-faint">{t(field.helpKey)}</span>}
      </label>
    );
  };
    ...
    <div className="space-y-4 rounded-xl border border-edge bg-surface-card p-4">
      ...
      {mirror && (
        <div>
          ...
          <p className="mb-1 text-xs text-content-faint">{t('storage.mirror.targetsHelp')}</p>
          <div className="space-y-1">
            {mirror.candidates
          ...
          {targets.length > 0 && (
            <p className="mt-1 text-xs text-content-faint" role="note">
