  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h3 className="min-w-0 flex-1 truncate text-caption font-semibold uppercase tracking-[0.06em] text-content">
          {label}
        </h3>
...
const RECOMMENDED_MODELS: { id: string; label: string; note: string; recommended: boolean; vision: boolean }[] = [
  {
    id: 'qwen3:8b',
    label: 'Qwen3 — 8B',
    note: 'Recommended · best extraction quality & speed on CPU (thinking auto-disabled) · Apache-2.0',
    recommended: true,
    vision: false,
  },
];
...
function LlmParsingConfig({ addon }: { addon: Addon }) {
  ...
  const fieldCls =
    'w-full rounded-lg border border-edge-secondary bg-surface px-2.5 py-1.5 text-caption text-content placeholder:text-content-faint transition-colors focus:border-edge focus:outline-none';
  ...
  return (
    <li className="space-y-2.5 py-1.5">
      ...
      {provider === 'anthropic' && (
        <p className="text-caption text-content-faint">
          Anthropic reads PDFs (including scans) natively. Local/OpenAI models receive extracted text — scanned PDFs
          need Anthropic.
        </p>
      )}
      <label className="block">
        ...
        <input
          ...
          placeholder={
            provider === 'anthropic' ? 'claude-opus-4-8' : provider === 'openai' ? 'gpt-4o' : 'select or pull below'
          }
        />
      </label>
        ...
        <div className="space-y-2 rounded-lg border border-edge-secondary bg-surface p-2.5">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-caption text-content-muted underline disabled:opacity-60"
            >
                ...
                return (
                  <div key={m.id} className="min-w-0" title={m.note}>
                    <div className="flex items-center gap-2">
                      ...
                      {m.recommended && (
                        <span className="shrink-0 rounded-md bg-success-soft px-1.5 py-px text-caption font-semibold text-success">
                          Recommended
                        </span>
                      )}
                      {installedHere ? (
                        <button
                          ...
                          disabled={active}
                          className={`shrink-0 rounded-md px-2 py-1 text-caption font-medium transition-colors ${active ? 'bg-surface-tertiary text-content-muted' : 'border border-edge-secondary text-content-secondary hover:border-edge'}`}
                        >
                          {active ? 'Selected' : 'Use'}
                        </button>
                      ) : (
                        <button
                          ...
                          onClick={() => pull(m.id)}
                          disabled={!!pulling}
                          className="shrink-0 rounded-md bg-accent px-2 py-1 text-caption font-medium text-accent-text disabled:opacity-60"
                        >
                    ...
                    {isPulling && (
                      <div className="mt-1">
                        ...
                        <div className="mt-0.5 text-caption text-content-faint">
                          {pullStatus}
                          {pullPct ? ` · ${pullPct}%` : ''}
                        </div>
                      </div>
      ...
      <div className="flex justify-end">
        <button
          ...
          onClick={save}
          disabled={saving}
          className="rounded-lg bg-accent px-3 py-1.5 text-caption font-medium text-accent-text transition-opacity disabled:opacity-60"
        >
          {saving ? 'Saving…' : 'Save'}
        </button>
      </div>
