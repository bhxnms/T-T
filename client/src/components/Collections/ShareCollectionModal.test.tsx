...
describe('ShareCollectionModal', () => {
  ...
  it('FE-COMP-COLSHARE-011: a second remove is ignored while the first is still running', async () => {
    let release!: () => void;
    actions.removeMember.mockReturnValue(
      new Promise<void>((r) => {
        release = r;
      })
    );
  ...
  it('FE-COMP-COLSHARE-021: leaving asks for confirmation first and can be backed out of', () => {
    ...
    expect(
      screen.getByText('Leave this shared list? You will lose access until you are invited again.')
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: 'Cancel' }));
    expect(actions.leave).not.toHaveBeenCalled();
    expect(
      screen.queryByText('Leave this shared list? You will lose access until you are invited again.')
