...
describe('ListEditorModal', () => {
  beforeEach(() => {
    ...
    actions = {
      createCollection: vi.fn<CollectionStore['createCollection']>(async () => ({
        id: 77,
        owner_id: 1,
        name: 'Fresh',
      })),
    ...
    vi.spyOn(tripsApi, 'searchCoverImages').mockResolvedValue({
      photos: [
        {
          id: 'p1',
          url: 'https://img/full1.jpg',
          thumb: 'https://img/thumb1.jpg',
          description: 'A street',
          photographer: 'Rin',
        },
        {
          id: 'p2',
          url: 'https://img/full2.jpg',
          thumb: 'https://img/thumb2.jpg',
          description: null,
          photographer: null,
        },
      ],
  ...
  it('FE-COMP-LISTEDITOR-008: editing patches the existing list and never calls onCreated', async () => {
    ...
    expect(actions.updateCollection).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ name: 'Tokyo 2027', color: '#ec4899' })
    );
  ...
  it('FE-COMP-LISTEDITOR-026: a second Enter while the create is in flight does not make a second list', async () => {
    ...
    actions.createCollection.mockReturnValueOnce(
      new Promise((resolve) => {
        release = resolve;
      })
    );
  ...
  it('FE-COMP-LISTEDITOR-021: a failed cover upload toasts the server message and the retry updates instead of re-creating', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('e.g. Tokyo 2025'), { target: { value: 'Oslo' } });
    fireEvent.change(container.ownerDocument.querySelector('input[type="file"]') as HTMLInputElement, {
      target: { files: [file] },
    });
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Create' }));
    await waitFor(() =>
      expect(actions.updateCollection).toHaveBeenCalledWith(77, expect.objectContaining({ name: 'Oslo' }))
    );
    expect(actions.createCollection).toHaveBeenCalledTimes(1);
  });
