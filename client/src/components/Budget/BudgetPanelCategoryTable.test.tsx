...
import { createEvent, fireEvent, render, screen, within } from '../../../tests/helpers/render';
  ...
  it('FE-W4BCT-012: blanks the per-person columns for a custom member split', () => {
    setup({}, [
      budgetItem({
        ...
        days: 5,
        members: [
          { user_id: 1, username: 'ada', amount: 70 },
...
describe('BudgetCategoryTable — drag and drop', () => {
  ...
  it('FE-W4BCT-035: dims the dragged row and outlines the drop target', () => {
    ...
    expect((over.container.querySelector('tbody tr') as HTMLElement).style.boxShadow).toBe(
  ...
  it('FE-W4BCT-038: moving between a row and its own cells keeps the drop marker', () => {
    const { setDragOverItem, setDragOverCat, container } = setup({
      dragCat: 'Transport',
      dragItem: 2,
      dragItemCat: 'Food',
    });
...
describe('BudgetCategoryTable — remaining cells', () => {
  ...
  it('FE-W4BCT-046: picking a day from the date cell stores the ISO date', () => {
    const { handleUpdateField, container } = setup({}, [
      budgetItem({ expense_date: '2026-06-15' } as Partial<BudgetItem>),
    ]);
  ...
  it('FE-W4BCT-050: an item without persons, days or members dashes out every derived column', () => {
    const { container } = setup({}, [
      budgetItem({ persons: null, days: null, members: undefined } as unknown as Partial<BudgetItem>),
    ]);
...
describe('BudgetCategoryTable — member chips', () => {
  ...
  it('FE-W4BCT-052: picking a member from either chip dropdown sets the item members', () => {
    ...
    const editors = Array.from(container.querySelectorAll('td button')).filter((b) =>
      b.querySelector('.lucide-pencil, .lucide-users')
    );
  ...
  it('FE-W4BCT-053: read-only member chips expose no editing controls', () => {
    setup({ hasMultipleMembers: true, canEdit: false }, [
