...
export default function AddItemRow({ onAdd, t }: AddItemRowProps) {
  ...
  const handleAdd = () => {
    ...
    onAdd({
      name: name.trim(),
      total_price: Number.parseFloat(String(price).replace(',', '.')) || 0,
      persons: Number.parseInt(persons) || null,
      days: Number.parseInt(days) || null,
      note: note.trim() || null,
    ...
    <tr className="bg-surface-secondary">
      ...
      <td style={{ padding: '4px 6px' }}>
        <input
          ...
          onPaste={(e) => {
            e.preventDefault();
            setPrice(normalizePastedAmount(e.clipboardData.getData('text')));
          }}
          placeholder="0,00"
          inputMode="decimal"
      ...
      <td
        ...
        style={{ padding: '4px 6px', fontSize: 'calc(12px * var(--fs-scale-body, 1))', textAlign: 'center' }}
      >
        -
      </td>
      <td
