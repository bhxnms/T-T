    ...
    <div className="vg-card rounded-[22px]" style={{ padding: '15px 16px 14px' }}>
      ...
      <div className="grid grid-cols-7" style={{ gap: 4 }}>
        {weeks.flat().map((day, di) => {
          ...
          const holidayMarkers = Array.isArray(rawHolidayMarkers)
            ? rawHolidayMarkers
            : rawHolidayMarkers
              ? [rawHolidayMarkers]
              : [];
          ...
          else if (dayEntries.length === 0 && publicHoliday)
            background = `color-mix(in srgb, ${publicHoliday.color} 22%, transparent)`;
          ...
          else if (dayEntries.length === 0 && schoolHolidayMarkers.length > 0)
            background = schoolHolidayWash(schoolHolidayMarkers[0].color);
          ...
          const numShadow =
            hasEntries && dayEntries.every((e) => e.kind === 'comp')
          ...
          return (
            <div
              ...
              title={
                publicHoliday
                  ? publicHoliday.label
                    ? `${publicHoliday.label}: ${publicHoliday.localName}`
              ...
              onMouseEnter={(e) => {
                ...
                if (hasEntries || sharedColors.length > 0 || schoolHolidayMarkers.length > 0)
                  onCellHover?.(dateStr, e.currentTarget);
              }}
              onMouseLeave={(e) => {
            ...
            >
              ...
              {dayEntries.length === 2 && (
                <div className="absolute inset-0 overflow-hidden" style={{ borderRadius: 10 }}>
                  <div
                    ...
                    style={{ background: segFill(dayEntries[0]), clipPath: 'polygon(0 0, 100% 0, 0 100%)' }}
                  />
                  <div
                    className="absolute inset-0"
                    style={{ background: segFill(dayEntries[1]), clipPath: 'polygon(100% 0, 100% 100%, 0 100%)' }}
                  />
                </div>
              )}
              ...
              {dayEntries.length === 3 && (
                <div className="absolute inset-0 overflow-hidden" style={{ borderRadius: 10 }}>
                  <div className="absolute left-0 top-0 h-full w-1/2" style={{ background: segFill(dayEntries[0]) }} />
                  <div className="absolute right-0 top-0 h-1/2 w-1/2" style={{ background: segFill(dayEntries[1]) }} />
                  <div
                    className="absolute bottom-0 right-0 h-1/2 w-1/2"
                    style={{ background: segFill(dayEntries[2]) }}
                  />
                </div>
              )}
              {dayEntries.length >= 4 && (
                <div className="absolute inset-0 overflow-hidden" style={{ borderRadius: 10 }}>
                  <div className="absolute left-0 top-0 h-1/2 w-1/2" style={{ background: segFill(dayEntries[0]) }} />
                  <div className="absolute right-0 top-0 h-1/2 w-1/2" style={{ background: segFill(dayEntries[1]) }} />
                  <div
                  ...
                  <div
                    className="absolute bottom-0 right-0 h-1/2 w-1/2"
                    style={{ background: segFill(dayEntries[3]) }}
                  />
                </div>
              )}
              ...
              {tripDates?.has(dateStr) && (
                <span
                  className="absolute right-1 top-1 z-[2] h-[5px] w-[5px] rounded-full bg-[#3b82f6]"
                  style={{ boxShadow: '0 0 0 1.5px var(--vg-surf)' }}
                />
              )}
              ...
              {anyHalf && (
                <span
                  className="absolute bottom-1 right-1 z-[3] h-[5px] w-[5px] rounded-full bg-[#f97316]"
                  style={{ boxShadow: '0 0 0 1.5px var(--vg-surf)' }}
                  aria-hidden
                />
              )}
