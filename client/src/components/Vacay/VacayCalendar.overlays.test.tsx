...
vi.mock('./VacayMonthCard', () => ({
  default: ({
    ...
    >
      <button type="button" onClick={() => onCellClick(`2026-${String(month + 1).padStart(2, '0')}-13`)}>
        sat-{month}
      </button>
      <button type="button" onClick={() => onCellClick(`2026-${String(month + 1).padStart(2, '0')}-15`)}>
        mon-{month}
      </button>
      <button type="button" onClick={() => onCellClick('2026-12-24')}>
        company-{month}
      </button>
      <button type="button" onClick={(e) => onCellHover?.('2026-06-15', e.currentTarget)}>
        hover-{month}
      </button>
      <button type="button" onClick={() => onCellHover?.(null, null)}>
        unhover-{month}
      </button>
    </div>
  ),
