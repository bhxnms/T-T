  ...
  return (
    <div className="vg-card rounded-[22px]" style={{ padding: '14px 18px' }}>
      ...
      <button
        ...
        className={`flex w-full items-start gap-2 text-left ${collapsed ? '' : 'mb-2.5'}`}
      ...
      >
        <span className="min-w-0 flex-1">
          ...
          {windowLabel && (
            <span className="mt-0.5 block" style={{ fontSize: 10.5, color: 'var(--vg-ink3)' }}>
              {windowLabel}
            </span>
          )}
        </span>
      ...
      {!collapsed &&
        (stats.length === 0 ? (
          <p className="py-3 text-center text-[11px]" style={{ color: 'var(--vg-ink3)' }}>
            {t('vacay.noData')}
          </p>
        ) : (
...
function StatCard({ stat: s, isMe, canEdit, selectedYear, isShiftedYear, onSave, t }: StatCardProps) {
  ...
  const rowShifted = s.window_start ? !s.window_start.endsWith('-01-01') : isShiftedYear;
  ...
  return (
    <div style={{ padding: 10, borderRadius: 14, border: '1px solid var(--vg-line)' }}>
      <div className="flex items-center gap-2" style={{ marginBottom: 6 }}>
        ...
        <span className="min-w-0 truncate" style={{ fontSize: 13, fontWeight: 700, color: 'var(--vg-ink)' }}>
          {s.person_name}
        </span>
        {isMe && <VacayBadge label={t('vacay.you')} />}
        <span
          className="ml-auto tabular-nums"
          style={{ fontFamily: 'var(--font-subtext)', fontSize: 10.5, color: 'var(--vg-ink3)' }}
        >
          {fmtDays(s.used)}/{s.total_available}
        </span>
      </div>
      <div
        className="overflow-hidden"
        style={{ height: 6, borderRadius: 99, background: 'var(--vg-surf2)', marginBottom: 7 }}
      >
        <div
          ...
          <div style={{ fontSize: 10, marginBottom: 2, color: 'var(--vg-ink3)', height: 13, lineHeight: '13px' }}>
            ...
            {canEdit && !editing && (
              <Pencil
                ...
                style={{ verticalAlign: 'middle', color: 'var(--vg-ink3)' }}
              />
            )}
          </div>
          {editing ? (
            <NumericInput
              ...
              autoFocus
              className="m-0 w-full bg-transparent p-0 outline-none [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none"
        ...
        <div className="flex flex-wrap items-center gap-1.5" style={{ marginTop: 10 }}>
          {s.carried_over > 0 && (
            <div className="flex items-center gap-1.5 rounded-md border border-[rgba(245,158,11,0.15)] bg-[rgba(245,158,11,0.08)] px-2 py-1">
              <span className="text-[10px] text-[#d97706]">
                +{s.carried_over} {carriedOverLabel}
              </span>
            </div>
          )}
          ...
          {compUsed > 0 && (
            ...
            >
              ...
              <span className="text-[10px]" style={{ color: 'var(--vg-ink2)' }}>
                {t('vacay.compUsedCount', { count: fmtDays(compUsed) })}
              </span>
            </div>
          )}
        </div>
