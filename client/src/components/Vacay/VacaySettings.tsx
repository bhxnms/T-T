...
export default function VacaySettings({ onClose }: VacaySettingsProps) {
  ...
  const publicHolidayCalendars = (plan.holiday_calendars ?? []).filter(
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 items-stretch gap-5 md:grid-cols-2">
        ...
        <div
          ...
          style={{ border: '1px solid var(--vg-line)', background: 'var(--vg-surf2)' }}
        >
          {/* Block weekends */}
          <SettingToggle
            icon={CalendarOff}
            ...
            <div data-testid="weekend-days" style={{ paddingLeft: 36 }}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  const current: number[] = plan.weekend_days
                    ? String(plan.weekend_days).split(',').map(Number)
                    : [0, 6];
            ...
            {plan.company_holidays_enabled && (
              <div className="ml-7 mt-2">
                ...
                >
                  ...
                  <span className="text-[10px]" style={{ color: 'var(--vg-ink3)' }}>
              ...
              <div className="ml-7 mt-2 space-y-2">
                ...
                {showAddForm ? (
                  <AddCalendarForm
                    ...
                    calendarType="public_holiday"
                    onAdd={async (data) => {
                      await addHolidayCalendar({ ...data, type: 'public_holiday' });
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-dashed py-2.5 text-xs font-medium transition-colors"
              ...
              <div className="ml-7 mt-2 space-y-2">
                ...
                {showAddSchoolForm ? (
                  <AddCalendarForm
                    ...
                    defaultColor="#a5f3fc"
                    onAdd={async (data) => {
                      await addHolidayCalendar({ ...data, type: 'school_holiday' });
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p className="mt-0.5 text-xs text-content-faint">{t('vacay.yearTypeHint')}</p>
        </div>
      </div>
      ...
      {type === 'anniversary' && (
        <div style={{ paddingLeft: 36, marginTop: 8 }}>
          ...
          />
          {!yearSettings.hire_date && <p className="mt-1 text-[11px] text-content-faint">{t('vacay.hireDateHint')}</p>}
        </div>
      )}

      <p className="mt-2 text-[11px] text-content-faint" style={{ paddingLeft: 36 }}>
...
function useRegionOptions(country: string, calendarType: 'public_holiday' | 'school_holiday') {
  const [loaded, setLoaded] = useState<{ country: string; options: { value: string; label: string }[] }>({
    country: '',
    options: [],
  });
...
function AddCalendarForm({
  ...
  const { t } = useTranslation();
  // The add form opens pre-set to China: the default audience adds a calendar
  // to get 国庆/春节/中秋, and making them hunt for 中国 in a list of 100+
  // countries misses the point. School-holiday calendars keep no default —
  // their country set is data-driven and CN may not be in it.
  const [region, setRegion] = useState(calendarType === 'public_holiday' ? 'CN' : '');
  ...
  return (
    <div
      className="flex items-start gap-3 rounded-xl border border-dashed p-3"
      style={{ borderColor: 'var(--vg-line2)', background: 'var(--vg-surf)' }}
    >
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <button
          type="button"
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex gap-1.5 pt-0.5">
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1.5 rounded-md bg-content px-2 py-1.5 text-xs font-medium text-surface-card transition-colors disabled:opacity-40"
          >
