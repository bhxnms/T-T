...
export default function VacayCalendar() {
  ...
  const tipSchool = (Array.isArray(tipHolidayRaw) ? tipHolidayRaw : tipHolidayRaw ? [tipHolidayRaw] : []).filter(
    (h) => h.type === 'school_holiday'
  );
  const tipDate = tip
    ? new Intl.DateTimeFormat(locale, { weekday: 'short', day: 'numeric', month: 'long' }).format(
        new Date(tip.date + 'T00:00:00')
      )
    : '';
  ...
  return (
    <div>
      ...
      {tip &&
        ((tipEntries && tipEntries.length > 0) || (tipShared && tipShared.length > 0) || tipSchool.length > 0) && (
          <div
            ...
            <div style={{ padding: '8px 11px', minWidth: 132 }}>
                ...
                return (
                  <div key={i} className="flex items-center gap-2" style={{ marginTop: i ? 4 : 0 }}>
                    <span
                      ...
                      style={{ background: e.person_color || '#6366f1' }}
                    />
                    ...
                    <span
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 700,
                        color: emphasized ? 'var(--vg-ink)' : 'var(--vg-ink3)',
                      }}
                    >
              ...
              {(tipShared ?? []).map((m, i) => (
                <div
                  ...
                  style={{ marginTop: tipEntries?.length || i ? 4 : 0 }}
                >
                  <span className="h-2 w-2 shrink-0 rounded-full" style={{ border: `2px solid ${m.color}` }} />
                ...
                <>
                  ...
                  {tipSchool.map((h, i) => (
                    <div key={`sch${i}`} className="flex items-center gap-2" style={{ marginTop: i ? 3 : 0 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--vg-ink)' }}>
                        {h.label ? `${h.label}: ${h.localName}` : h.localName}
                      </span>
                    </div>
                  ))}
                </>
      ...
      >
        <div className="vg-card flex items-center gap-1.5 rounded-full px-2 py-1.5 sm:gap-2 sm:px-3 sm:py-2">
          <button
            type="button"
            onClick={() => setMode('vacation')}
            className="flex items-center gap-1 rounded-full px-2 py-1.5 text-[11px] font-semibold transition-[background-color,color] duration-150 ease-[cubic-bezier(0.23,1,0.32,1)] sm:gap-1.5 sm:px-3 sm:text-xs"
            style={
              mode === 'vacation'
          ...
          <span className="my-0.5 w-px self-stretch" style={{ background: 'var(--vg-line)' }} aria-hidden />
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-full py-1.5 pl-2 pr-2.5 text-[11px] font-semibold transition-[background-color,color] duration-150 ease-[cubic-bezier(0.23,1,0.32,1)] sm:pl-2.5 sm:pr-3 sm:text-xs"
            style={
              compDay
                ? { background: 'var(--vg-ink)', color: 'var(--vg-bg)' }
          ...
          >
            ...
            <span className="shrink-0 rounded-full bg-[#f97316]" style={{ width: 12, height: 12 }} aria-hidden />
            {t('vacay.modeHalf')}
          </button>
        </div>
      </div>
