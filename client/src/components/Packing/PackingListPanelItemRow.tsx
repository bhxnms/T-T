...
export function ArtikelZeile({
  ...
  return (
    <div
      className="packing-item-row group"
    ...
    >
      ...
      {!isPlaceholder && sharedByMe && (
        <span
          className="packing-row-badge"
          title={recipients.map((r) => r.username).join(', ')}
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: 3,
      ...
      {!isPlaceholder && broughtBy && (
        <span
          className="packing-row-badge"
          title={t('packing.broughtBy', { name: broughtBy })}
          style={{
            display: 'inline-flex',
            alignItems: 'center',
            gap: 3,
        ...
        >
          <Users size={10} /> {broughtBy}
          {contributors.length > 0 ? ` +${contributors.length}` : ''}
        </span>
      )}

      <div
        className="packing-row-inline-actions"
            ...
            <div style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  ...
                  border: item.bag_id
                    ? `2.5px solid ${selectedBag?.color || 'var(--border-primary)'}`
              ...
              {showBagPicker && (
                ...
                >
                  ...
                  {bags.map((b) => (
                    ...
                    >
                      <span
                        style={{ width: 10, height: 10, borderRadius: '50%', background: b.color, flexShrink: 0 }}
                      />
                      {b.name}
                    </button>
                  ))}
                ...
                <>
                  ...
                  >
                      ...
                      <>
                        ...
                        <div style={{ padding: '2px 0' }}>
                          <OverflowMenuItem
                            ...
                            label={selectedBag?.name || t('packing.noBag')}
                            onClick={async () => {
                              if (item.bag_id) {
                                try {
                                  await updatePackingItem(tripId, item.id, { bag_id: null });
                    ...
                    <OverflowMenuItem
                      ...
                      label={t('packing.changeCategory')}
                      onClick={() => setShowMenuCategories((v) => !v)}
                    />
                    {showMenuCategories && (
                      <div style={{ padding: '2px 0 4px 18px' }}>
                        {categories.map((cat) => (
                          <OverflowMenuItem
                            ...
                            label={cat}
                            active={cat === (item.category || t('packing.defaultCategory'))}
                            onClick={() => handleCatChange(cat)}
                          />
                        ))}
                      </div>
                    ...
                    {canShare && onClone && onJoin && onLeave && (
                      ...
                      >
                        <span
                          style={{ fontSize: 'calc(12.5px * var(--fs-scale-body, 1))', color: 'var(--text-secondary)' }}
                        >
                          {t('packing.share')}
                        </span>
                        <PackingShareControl
...
function OverflowMenuItem({ icon, label, onClick, active = false, danger = false }: OverflowMenuItemProps) {
  return (
    <button
      ...
      onMouseEnter={(e) => {
        if (!active) e.currentTarget.style.background = danger ? '#fef2f2' : 'var(--bg-tertiary)';
      }}
      onMouseLeave={(e) => {
    ...
    >
      <span
        style={{ width: 14, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
      >
        {icon}
      </span>
