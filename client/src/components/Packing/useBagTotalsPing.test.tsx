...
vi.mock('../../api/websocket', () => ({
  addListener: (fn: (e: Record<string, unknown>) => void) => {
    listeners.add(fn);
  },
  removeListener: (fn: (e: Record<string, unknown>) => void) => {
    listeners.delete(fn);
  },
}));
...
describe('useBagTotalsPing', () => {
  ...
  it('FE-COMP-BAGPING-002: folds a burst of pings into a single refetch', async () => {
    ...
    act(() => {
      for (let i = 0; i < 12; i++) emit('packing:bag-totals');
    });
    await flushCoalesce();
  ...
  it('FE-COMP-BAGPING-004: ignores every other event type', async () => {
    ...
    act(() => {
      emit('packing:created');
      emit('packing:updated');
      emit('place:created');
    });
    await flushCoalesce();
