...
describe('usePackingList — members & assignees', () => {
  ...
  it('FE-W5HOOK-005: setting assignees stores the returned list for that category', async () => {
    ...
    await act(async () => {
      await result.current.handleSetAssignees('Gear', [2]);
    });
...
describe('usePackingList — item and category CRUD', () => {
  it('FE-W5HOOK-015: adding to a category reuses its placeholder row', async () => {
    ...
    await act(async () => {
      await result.current.handleAddItemToCategory('Gear', 'Tent');
    });
  ...
  it('FE-W5HOOK-018: deleting the last item of a category unchecks it and converts it to a placeholder', async () => {
    ...
    server.use(
      ...
      http.delete('/api/trips/1/packing/11', () => {
        deleted = true;
  ...
  it('FE-W5HOOK-018b: an item with siblings in its category is deleted outright', async () => {
    ...
    let converted = false;
    server.use(
      http.delete('/api/trips/1/packing/15', () => {
        deleted = true;
        return HttpResponse.json({ success: true });
      }),
      http.put('/api/trips/1/packing/15', () => {
        converted = true;
        return HttpResponse.json({ item });
      })
    );
  ...
  it('FE-W5HOOK-019: an uncategorized item is deleted outright', async () => {
    ...
    let deleted = false;
    server.use(
      http.delete('/api/trips/1/packing/12', () => {
        deleted = true;
        return HttpResponse.json({ success: true });
      })
    );
  ...
  it('FE-W5HOOK-021: adding a category without a name does nothing', async () => {
    ...
    await act(async () => {
      await result.current.handleAddNewCategory();
    });
  ...
  it('FE-W5HOOK-024: renaming the default category moves every uncategorized item', async () => {
    const items = [buildPackingItem({ id: 31, category: null }), buildPackingItem({ id: 32, category: 'Gear' })];
    ...
    await act(async () => {
      await result.current.handleRenameCategory('Other', 'Misc');
    });
  ...
  it('FE-W5HOOK-025: a partly failing category delete surfaces a delete error', async () => {
    ...
    await act(async () => {
      await result.current.handleDeleteCategory(items);
    });
  ...
  it('FE-W5HOOK-026: declining the clear-checked confirmation deletes nothing', async () => {
    ...
    let deleted = false;
    server.use(
      http.delete('/api/trips/1/packing/:itemId', () => {
        deleted = true;
        return HttpResponse.json({ success: true });
      })
    );
...
describe('usePackingList — bags', () => {
  ...
  it('FE-W5HOOK-029: creating a bag without a name does nothing', async () => {
    ...
    await act(async () => {
      await result.current.handleCreateBag();
    });
  ...
  it('FE-W5HOOK-030: creating a bag appends it and clears the form', async () => {
    ...
    act(() => {
      result.current.setNewBagName('Duffel');
      result.current.setShowAddBag(true);
    });
    await act(async () => {
      await result.current.handleCreateBag();
    });
  ...
  it('FE-W5HOOK-032: creating a bag by name returns the new bag', async () => {
    ...
    let created: unknown;
    await act(async () => {
      created = await result.current.handleCreateBagByName('Crate');
    });
  ...
  it('FE-W5HOOK-034: deleting a bag drops it from the list', async () => {
    ...
    await act(async () => {
      await result.current.handleDeleteBag(5);
    });
  ...
  it('FE-W5HOOK-036: updating a bag merges the response into that bag only', async () => {
    ...
    await act(async () => {
      await result.current.handleUpdateBag(6, { name: 'Renamed' });
    });
  ...
  it('FE-W5HOOK-037: a failing bag update surfaces a generic error', async () => {
    ...
    await act(async () => {
      await result.current.handleUpdateBag(6, { name: 'X' });
    });
  ...
  it('FE-W5HOOK-038: setting bag members replaces that bag members', async () => {
    ...
    await act(async () => {
      await result.current.handleSetBagMembers(5, [2]);
    });
...
describe('usePackingList — templates, import and signals', () => {
  ...
  it('FE-W5HOOK-041: applying a template appends its items and reports the count', async () => {
    ...
    server.use(
      http.post('/api/trips/1/packing/apply-template/2', () => HttpResponse.json({ items: [fresh], count: 1 }))
    );
    const { result } = renderPanel();

    act(() => result.current.setShowTemplateDropdown(true));
    await act(async () => {
      await result.current.handleApplyTemplate(2);
    });
  ...
  it('FE-W5HOOK-043: saving a template without a name does nothing', async () => {
    let posted = false;
    server.use(
      http.post('/api/trips/1/packing/save-as-template', () => {
        posted = true;
  ...
  it('FE-W5HOOK-044: saving a template reloads the list and clears the form', async () => {
    ...
    act(() => {
      result.current.setSaveTemplateName('Beach');
      result.current.setShowSaveTemplate(true);
    });
    await act(async () => {
      await result.current.handleSaveAsTemplate();
    });
  ...
  it('FE-W5HOOK-046: importing nothing reports an empty import', async () => {
    ...
    await act(async () => {
      await result.current.handleBulkImport();
    });
  ...
  it('FE-W5HOOK-047: a successful import appends the items and closes the modal', async () => {
    ...
    act(() => {
      result.current.setImportText('Gear, Rope');
  ...
  it('FE-W5HOOK-054: a raised clear-checked signal runs the bulk delete', async () => {
    ...
    let deleted = false;
    server.use(
      http.delete('/api/trips/1/packing/81', () => {
        deleted = true;
        return HttpResponse.json({ success: true });
      })
    );
    ...
    await act(async () => {
      rerender({ tripId: 1, items, clearCheckedSignal: 1 });
    });
  ...
  it('FE-W5HOOK-055: a mousedown outside the template dropdown closes it', async () => {
    ...
    act(() => {
      result.current.setShowTemplateDropdown(true);
    });
    act(() => {
      result.current.templateDropdownRef.current = document.createElement('div');
    });
    act(() => {
  ...
  it('FE-W5HOOK-055a: a mousedown inside the template dropdown keeps it open', async () => {
    ...
    act(() => {
      result.current.templateDropdownRef.current = dropdown;
    });
    act(() => {
...
describe('usePackingList — permissions and sharing', () => {
  ...
  it('FE-W5HOOK-058: the sharing handlers delegate to the packing endpoints', async () => {
    ...
    server.use(
      ...
      http.post('/api/trips/1/packing/91/clone', () => {
        hits.push('clone');
        return HttpResponse.json({ item });
      }),
      http.post('/api/trips/1/packing/91/contributors', () => {
        hits.push('join');
        return HttpResponse.json({ item });
      }),
      http.delete('/api/trips/1/packing/91/contributors/2', () => {
        hits.push('leave');
        return HttpResponse.json({ item });
      })
    );
    ...
    await act(async () => {
      await result.current.handleSetSharing(91, 'shared', [2]);
    });
    await act(async () => {
      await result.current.handleCloneItem(91);
    });
    await act(async () => {
      await result.current.handleJoinItem(91);
    });
    await act(async () => {
      await result.current.handleLeaveItem(91, 2);
    });
