...
import { isStale, refreshPatch, routeFor } from './travelRefresh';
        ...
        <Line label={t('journey.studio.text')} wide>
          ...
          */}
          <Swatches
            value={el.color}
            onPick={(c) =>
              set((el.kind === 'badge' ? { color: c, autoColor: false } : { color: c }) as Partial<BookElement>)
            }
          />
        </Line>
        {el.kind === 'badge' && (
          ...
          <Line label={t('journey.studio.style')} wide>
            <Choice
              ...
              options={(['minimal', 'outline', 'paper', 'dark'] as const).map((style) => ({
                value: style,
                label: t(`journey.studio.mapStyle.${style}`),
              }))}
      ...
      */}
      ...
      <Section label={t('journey.studio.mapLayers')}>
        {el.source !== 'tiles' && el.source !== 'static' && (
          <Switch
            label={t('journey.studio.showLand')}
            on={el.showLand}
            onToggle={() => set({ showLand: !el.showLand } as Partial<BookElement>)}
          />
        )}
        <Switch
          label={t('journey.studio.showRoute')}
          on={el.showRoute}
          onToggle={() => set({ showRoute: !el.showRoute } as Partial<BookElement>)}
        />
        <Switch
          label={t('journey.studio.showPins')}
          on={el.showPins}
          onToggle={() => set({ showPins: !el.showPins } as Partial<BookElement>)}
        />
        <Switch
          label={t('journey.studio.showLabels')}
          on={el.showLabels}
          onToggle={() => set({ showLabels: !el.showLabels } as Partial<BookElement>)}
        />
      </Section>
    </>
  );
...
function CountriesProps({ el, set, t }: Props<BookCountriesElement>) {
  return (
    <Section label={t('journey.studio.countries')}>
      ...
      <Line label={t('journey.studio.align')}>
        <Choice
          ...
          options={(['left', 'center', 'right'] as const).map((align) => ({
            value: align,
            label: t(`journey.studio.align.${align}`),
          }))}
      ...
      <div style={{ marginTop: 10 }}>
        <Switch
          label={t('journey.studio.showOutline')}
          on={el.showOutline}
          onToggle={() => set({ showOutline: !el.showOutline } as Partial<BookElement>)}
        />
        <Switch
          label={t('journey.studio.showFlag')}
          on={el.showFlag}
          onToggle={() => set({ showFlag: !el.showFlag } as Partial<BookElement>)}
        />
        <Switch
          label={t('journey.studio.showName')}
          on={el.showName}
          onToggle={() => set({ showName: !el.showName } as Partial<BookElement>)}
        />
      </div>
...
function BadgeProps({ el, set, t }: Props<BookBadgeElement>) {
  ...
  const parts = (icon: boolean, label: boolean) => set({ showIcon: icon, showLabel: label } as Partial<BookElement>);
        ...
        <Line label={t('journey.studio.style')} wide>
          <Choice
            ...
            options={(['plain', 'chip', 'outline', 'stacked'] as const).map((style) => ({
              value: style,
              label: t(`journey.studio.markStyle.${style}`),
            }))}
            onPick={(style) => set({ style } as Partial<BookElement>)}
          />
        </Line>
