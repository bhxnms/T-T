...
export function StudioCanvas({
  ...
  const frameUnder = (x: number, y: number) =>
    spread?.elements.find(
      (el) =>
        el.kind === 'photo' &&
        !el.locked &&
        x >= el.frame.x &&
        x <= el.frame.x + el.frame.w &&
        y >= el.frame.y &&
        y <= el.frame.y + el.frame.h
    ) ?? null;
    ...
    >
      ...
      */}
      ...
      >
        ...
        {cursors && cursors.length > 0 && <PeerCursors cursors={cursors} spreadIndex={spreadIndex} zoom={zoom} />}
        ...
        {spread.elements.map((el) => (
          <div
            key={el.id}
            onPointerDown={(e) => {
              if (editing !== el.id) startMove(e, el);
            }}
            onDoubleClick={() => {
              ...
              >
                ...
                {sel.length === 1 && (
                  <>
                    <button
                      type="button"
                      onClick={() => raise(spreadIndex, sel[0].id, 'front')}
                    ...
                    <button
                      type="button"
                      onClick={() => raise(spreadIndex, sel[0].id, 'back')}
                      ...
                      spreads: d.spreads.map((sp, i) =>
                        i !== spreadIndex
                          ...
                          : {
                              ...sp,
                              elements: sp.elements.map((e) =>
                                selection.includes(e.id) ? { ...e, locked: !locked } : e
                              ),
                            }
                      ),
                    }))
