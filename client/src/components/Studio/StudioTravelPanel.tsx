...
export function StudioTravelPanel({
  ...
  const place = (el: BookElement) => {
    ...
    if (placed.kind === 'map' && placed.points.length > 1) {
      ...
      void fetchRoads(
        placed.points.map((pt) => ({ lat: pt.lat, lng: pt.lng })),
        { signal: job.signal }
      )
        .then((roads) => {
        ...
        .catch(() => {
          /* No roads is a state the map already draws. */
        })
        .finally(() => {
          roadJobs.current.delete(job);
        });
    }
  };
  ...
  if (startDay) {
    marks.push({
      el: badgeEl(
        'date',
        String(startDay.getDate()),
        startDay.toLocaleDateString(locale, { month: 'long' }).toUpperCase(),
        null,
        'stacked'
      ),
  ...
  marks.push({
    el: badgeEl(
      'distance',
      formatDistance(stats.distance, locale),
      t('journey.studio.metric.distance'),
      null,
      'outline'
    ),
                  ...
                  <li key={entryId ?? `place-${i}`} className={`st-stop ${on ? '' : 'is-off'}`}>
                    ...
                    */}
                    ...
                    {canEdit && entryId != null ? (
                      ...
                      <span className={chip} title={state} aria-label={state}>
                        {mark}
                      </span>
                    )}
                  </li>
