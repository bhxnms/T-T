...
import { authApi, shareApi, tripInviteApi, tripsApi } from '../../api/client';
  ...
  return (
    <div>
      ...
      <p
        ...
        style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', marginBottom: 10, lineHeight: 1.5 }}
      >
        {t('share.linkHint')}
      </p>
 ...
 */
function TripInviteLinkSection({
  tripId,
  t,
}: {
  tripId: number;
  ...
  const create = async () => {
    setBusy(true);
    try {
      const d = await tripInviteApi.createLink(tripId);
      setToken(d.token);
    } catch {
      toast.error(t('share.createError'));
    } finally {
  ...
  const remove = async () => {
    setBusy(true);
    try {
      await tripInviteApi.deleteLink(tripId);
      setToken(null);
    } catch {
      toast.error(t('common.error'));
    } finally {
  ...
  return (
    <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid var(--border-faint)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        ...
        <span className="text-content" style={{ fontSize: 'calc(13px * var(--fs-scale-body, 1))', fontWeight: 600 }}>
          {t('trip.invite.linkTitle')}
        </span>
      </div>
      <p
        className="text-content-faint"
        style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', marginBottom: 12, lineHeight: 1.5 }}
      >
        {t('trip.invite.linkHint')}
      </p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {canManageMembers && (
            <div>
              ...
              {availableUsers.length === 0 && allUsers.length > 0 && canManageMembers && (
                <p
                  className="text-content-faint"
                  style={{ fontSize: 'calc(11.5px * var(--fs-scale-caption, 1))', margin: '6px 0 0' }}
                >
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
              ...
              >
                {t('members.access')} ({realMembers.length}{' '}
                {realMembers.length === 1 ? t('members.person') : t('members.persons')})
              </span>
            </div>
                      ...
                      {isCurrentOwner && member.role !== 'owner' && (
                        <button
                          ...
                          style={{
                            ...
                            opacity: transferringId === member.id ? 0.4 : 1,
                          }}
          ...
          {(isCurrentOwner || guests.length > 0) && (
            <div>
              ...
              <p
                className="text-content-faint"
                style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', margin: '0 0 10px', lineHeight: 1.5 }}
              >
                  ...
                  >
                    ...
                    {renamingGuestId === g.id ? (
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') handleRenameGuest(g.id);
                          if (e.key === 'Escape') {
                            renameCommittedRef.current = true;
                            setRenamingGuestId(null);
                          }
                        ...
                        maxLength={50}
                        className="border border-edge bg-surface text-content"
                        style={{
                          flex: 1,
                          minWidth: 0,
                          fontSize: 'calc(13px * var(--fs-scale-body, 1))',
                    ...
                    {isCurrentOwner && renamingGuestId !== g.id && (
                      <>
                        <button
                          ...
                          onClick={() => {
                            renameCommittedRef.current = false;
                            setRenamingGuestId(g.id);
                            setRenameValue(g.username);
                          }}
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: guests.length > 0 ? 8 : 0 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 'calc(13px * var(--fs-scale-body, 1))',
                      fontWeight: 600,
                      cursor: addingGuest || !newGuestName.trim() ? 'default' : 'pointer',
                      fontFamily: 'inherit',
                      opacity: addingGuest || !newGuestName.trim() ? 0.4 : 1,
                      flexShrink: 0,
                    }}
                  >
