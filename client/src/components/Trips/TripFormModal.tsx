...
import { Bell, Calendar, Camera, FileUp, Search, UserPlus, X } from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { authApi, tripsApi, type TrekImportTripSummary } from '../../api/client';
...
interface TripFormModalProps {
  ...
  /** Fired after trips were imported from a TREK backup, so the list refreshes. */
  onImported?: () => void;
}
...
export default function TripFormModal({
  ...
  // ── 从TREK导入 tab (create mode only) ──
  const [createMode, setCreateMode] = useState<'form' | 'trek'>('form');
  const [trekToken, setTrekToken] = useState<string | null>(null);
  const [trekTrips, setTrekTrips] = useState<TrekImportTripSummary[]>([]);
  const [trekSelected, setTrekSelected] = useState<Set<number>>(new Set());
  const [trekBusy, setTrekBusy] = useState(false);
  const [trekError, setTrekError] = useState('');
  const trekFileRef = useRef<HTMLInputElement>(null);

  const resetTrekState = () => {
    setTrekToken(null);
    setTrekTrips([]);
    setTrekSelected(new Set());
    setTrekError('');
    setTrekBusy(false);
  };

  const handleTrekFile = async (file: File | null) => {
    if (!file) return;
    setTrekBusy(true);
    setTrekError('');
    setTrekTrips([]);
    ...
    try {
      ...
      fd.append('file', file);
      const res = await tripsApi.previewTrekImport(fd);
      setTrekToken(res.token);
      setTrekTrips(res.trips ?? []);
      setTrekSelected(new Set((res.trips ?? []).map((tr) => tr.id)));
      if (!res.trips?.length) setTrekError(t('trips.importTrekNone'));
    } catch (e) {
      setTrekError(t('trips.importTrekFailed'));
    } finally {
      setTrekBusy(false);
    }
  };

  const handleTrekImport = async () => {
    if (!trekToken || trekSelected.size === 0) return;
    setTrekBusy(true);
    setTrekError('');
    try {
      const res = await tripsApi.importTrek(trekToken, [...trekSelected]);
      toast.success(t('trips.importTrekSuccess', { count: res.imported?.length ?? 0 }));
      resetTrekState();
      setCreateMode('form');
      onImported?.();
      onClose();
    } catch (e) {
      setTrekError(getApiErrorMessage(e, t('trips.importTrekFailed')));
    } finally {
      setTrekBusy(false);
    }
  };
  ...
  const inputCls =
    'w-full px-3.5 py-2.5 border border-edge rounded-xl bg-surface-input text-content placeholder:text-content-faint focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent text-body transition-shadow';
  const labelCls =
    'flex items-center gap-1 text-caption font-semibold uppercase tracking-[0.14em] text-content-faint mb-2';
  const ghostBtnCls =
  ...
  return (
        ...
        <div className="flex justify-end gap-3">
          {pendingDateShift ? (
            ...
            <>
              ...
              <button
                type="button"
                onClick={handleTrekImport}
                disabled={trekBusy || trekSelected.size === 0}
                className={primaryBtnCls}
              >
                {trekBusy ? (
                  <>
    ...
    >
      ...
      {/* Create mode offers a choice: fill the form by hand, or pull existing
          trips out of a TREK backup zip. Editing stays form-only. */}
      {!isEditing && !pendingDateShift && (
        <div className="flex gap-1 rounded-xl border border-edge bg-surface-secondary p-1" role="tablist">
          {[
            { id: 'form' as const, label: t('dashboard.createTrip') },
            { id: 'trek' as const, label: t('trips.importTrekTab') },
          ].map((tab) => (
            <button
              key={tab.id}
              type="button"
              role="tab"
              aria-selected={createMode === tab.id}
              onClick={() => setCreateMode(tab.id)}
              className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-body font-medium transition-colors ${
                createMode === tab.id
                  ? 'bg-surface-elevated text-content shadow-sm'
                  : 'text-content-muted hover:text-content'
              }`}
            >
              {tab.id === 'trek' && <FileUp size={14} />}
              {tab.label}
            </button>
          ))}
        </div>
      )}
      {!isEditing && createMode === 'trek' && !pendingDateShift && (
        <div className="space-y-3">
          {trekError && (
            <div className="border-danger/30 rounded-xl border bg-danger-soft p-3 text-body text-danger">
              {trekError}
            </div>
          )}
          <p className="text-body text-content-secondary">{t('trips.importTrekIntro')}</p>
          <input
            ref={trekFileRef}
            type="file"
            accept=".zip"
            className="hidden"
            onChange={(e) => {
              void handleTrekFile(e.target.files?.[0] ?? null);
              e.target.value = '';
            }}
          />
          <button
            type="button"
            onClick={() => trekFileRef.current?.click()}
            disabled={trekBusy}
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-edge py-3 text-body font-medium text-content-secondary transition-colors hover:bg-surface-hover disabled:opacity-50"
          >
            {trekBusy ? (
              <div className="border-accent-text/30 h-4 w-4 animate-spin rounded-full border-2 border-t-accent-text" />
            ) : (
            ...
            {trekBusy ? t('trips.importTrekScanning') : t('trips.importTrekPick')}
          </button>
          {trekTrips.length > 0 && (
            <div className="max-h-72 space-y-1.5 overflow-y-auto">
              {trekTrips.map((tr) => {
                const checked = trekSelected.has(tr.id);
                return (
                  <label
                    key={tr.id}
                    className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 transition-colors ${
                      checked ? 'border-accent bg-surface-selected' : 'border-edge hover:bg-surface-hover'
                    }`}
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-body font-medium text-content">
                        {tr.title || t('trips.importTrekUntitled')}
                      </span>
                      <span className="block text-caption text-content-faint">
                        {[tr.start_date, tr.end_date].filter(Boolean).join(' ~ ')}
                        {' · '}
                        {t('trips.importTrekStats', {
                          days: tr.day_count,
                          places: tr.place_count,
                          photos: tr.photo_count,
                          budget: tr.budget_count,
                        })}
                      </span>
                    </span>
                  </label>
      ...
      <form
        ...
        className={pendingDateShift || (!isEditing && createMode === 'trek') ? 'hidden' : 'space-y-4'}
        onPaste={handlePaste}
      >
        {error && (
          <div className="border-danger/30 rounded-xl border bg-danger-soft p-3 text-body text-danger">{error}</div>
              ...
              <div className={panelCls}>
                ...
                {coverPreview ? (
                  <div className="relative h-[130px] overflow-hidden rounded-xl">
                    ...
                    both themes rather than following the surface tokens. */}
                      <button
                        ...
                        className="flex items-center gap-1 rounded-lg bg-black/55 px-2.5 py-[5px] text-caption font-semibold text-white backdrop-blur-sm"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center rounded-lg bg-black/55 px-2 py-[5px] text-white backdrop-blur-sm"
                      >
                        <X size={12} />
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className={`flex h-[130px] w-full items-center justify-center gap-1.5 rounded-xl border-2 border-dashed px-4 text-body transition-colors ${
                      coverDragActive
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  <button
                    ...
                    disabled={searchingCover || (!coverSearchQuery.trim() && !formData.title.trim())}
                    className="flex items-center gap-1.5 whitespace-nowrap rounded-xl border border-edge px-3 py-2 text-body text-content-secondary hover:bg-surface-hover disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    {searchingCover ? (
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-edge border-t-content-muted" />
                    ) : (
                      <Search size={14} />
                    )}
                ...
                {coverSearchError && <p className="mt-1.5 text-caption text-danger">{coverSearchError}</p>}
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    {coverSearchResults.map((photo) => (
                      <button
                        ...
                        className={`relative h-20 overflow-hidden rounded-xl border transition-colors ${coverPreview === photo.url ? 'ring-accent/20 border-accent ring-2' : 'border-edge hover:border-content-faint'}`}
                      >
                        <img
                          src={photo.thumb}
            ...
            the cell past its track and the two columns overlapped (#2267). */}
              ...
              {!formData.start_date && !formData.end_date && (
                <div>
                  ...
                  <NumericInput
                    ...
                    className={inputCls}
                  />
                  <p className="mt-1.5 text-caption text-content-faint">{t('dashboard.dayCountHint')}</p>
                </div>
              )}
              ...
              {(!isEditing || trip?.user_id === currentUser?.id || currentUser?.role === 'admin') && (
                <div className={!tripRemindersEnabled ? 'opacity-50' : ''}>
                  ...
                  {!tripRemindersEnabled ? (
                    <p className="rounded-xl bg-surface p-3 text-caption text-content-faint">
                          ...
                          <button
                            ...
                            className={`rounded-full border px-3 py-1.5 text-caption font-medium transition-colors ${
                              !customReminder && formData.reminder_days === opt.value
                                ...
                                : 'border-edge bg-surface text-content-secondary hover:border-content-faint'
                            }`}
                          >
                            {opt.label}
                          </button>
                      ...
                      {customReminder && (
                        <div className="mt-2 flex items-center gap-2">
                          <NumericInput
                            ...
                            onValueChange={(raw) =>
                              update('reminder_days', Math.max(1, Math.min(30, Number(raw) || 1)))
                            }
                            className="focus:ring-accent/40 w-20 rounded-xl border border-edge bg-surface-input px-3 py-1.5 text-body text-content focus:outline-none focus:ring-2"
                          />
                    ...
                    <div className="mb-2 flex flex-wrap gap-1.5">
                      {existingMembers.map((m) => (
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-full border border-edge bg-surface px-2.5 py-1 text-body font-medium text-content"
                          onClick={async () => {
                      ...
                      {selectedMembers.map((uid) => {
                        ...
                        return (
                          <button
                            ...
                            className="flex cursor-pointer items-center gap-1.5 rounded-full border border-edge bg-surface px-2.5 py-1 text-body font-medium text-content"
                          >
                            {user.username}
