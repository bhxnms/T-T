...
import { Calendar, Camera, Search, X, UserPlus, Bell, FileUp } from 'lucide-react'
import { tripsApi, authApi, type TrekImportTripSummary } from '../../api/client'
...
export default function TripFormModal({ isOpen, onClose, onSave, trip, onCoverUpdate, onImported }: TripFormModalProps) {
  ...
  const resetTrekState = () => {
    setTrekToken(null); setTrekTrips([]); setTrekSelected(new Set()); setTrekError(''); setTrekBusy(false)
  }

  const handleTrekFile = async (file: File | null) => {
    if (!file) return
    setTrekBusy(true); setTrekError(''); setTrekTrips([]); setTrekToken(null)
    try {
  ...
  return (
        ...
        <div className="flex gap-3 justify-end">
          {pendingDateShift ? (
            ...
            <>
              ...
              <button type="button" onClick={handleTrekImport} disabled={trekBusy || trekSelected.size === 0} className={primaryBtnCls}>
                {trekBusy
                  ? <><div className="w-4 h-4 border-2 border-accent-text/30 border-t-accent-text rounded-full animate-spin" />{t('common.saving')}</>
    ...
    >
      ...
      {!isEditing && !pendingDateShift && (
        <div className="flex gap-1 p-1 rounded-xl bg-surface-secondary border border-edge" role="tablist">
          {([
            { id: 'form' as const, label: t('dashboard.createTrip') },
            { id: 'trek' as const, label: t('trips.importTrekTab') },
          ]).map(tab => (
            <button key={tab.id} type="button" role="tab" aria-selected={createMode === tab.id}
              onClick={() => setCreateMode(tab.id)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-body font-medium transition-colors ${
                createMode === tab.id ? 'bg-surface-elevated text-content shadow-sm' : 'text-content-muted hover:text-content'
              }`}>
      ...
      {!isEditing && createMode === 'trek' && !pendingDateShift && (
        <div className="space-y-3">
          {trekError && (
            <div className="p-3 bg-danger-soft border border-danger/30 rounded-xl text-body text-danger">{trekError}</div>
          )}
          <p className="text-body text-content-secondary">{t('trips.importTrekIntro')}</p>
          <input ref={trekFileRef} type="file" accept=".zip" className="hidden"
            onChange={e => { void handleTrekFile(e.target.files?.[0] ?? null); e.target.value = '' }} />
          <button type="button" onClick={() => trekFileRef.current?.click()} disabled={trekBusy}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-dashed border-edge text-body font-medium text-content-secondary hover:bg-surface-hover transition-colors disabled:opacity-50">
            {trekBusy ? <div className="w-4 h-4 border-2 border-accent-text/30 border-t-accent-text rounded-full animate-spin" /> : <FileUp size={15} />}
                ...
                return (
                  <label key={tr.id}
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block text-body font-medium text-content truncate">{tr.title || t('trips.importTrekUntitled')}</span>
                      <span className="block text-caption text-content-faint">
                        {[tr.start_date, tr.end_date].filter(Boolean).join(' ~ ')}
                        {' · '}{t('trips.importTrekStats', { days: tr.day_count, places: tr.place_count, photos: tr.photo_count, budget: tr.budget_count })}
                      </span>
                    </span>
                  </label>
                )
      ...
      <form onSubmit={handleSubmit} className={pendingDateShift || (!isEditing && createMode === 'trek') ? 'hidden' : 'space-y-4'} onPaste={handlePaste}>
        {error && (
          <div className="p-3 bg-danger-soft border border-danger/30 rounded-xl text-body text-danger">{error}</div>
        )}
