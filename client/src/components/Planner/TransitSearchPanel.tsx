...
import type { Accommodation, Day, Place } from '../../types';
...
function fmtTimeInTz(iso: string | null, tz: string, is12h: boolean): string {
  if (!iso) return '';
  try {
    return new Intl.DateTimeFormat(is12h ? 'en-US' : 'en-GB', {
      timeZone: tz,
      hour: '2-digit',
      minute: '2-digit',
      hour12: is12h,
...
function StopPicker({
  ...
  const search = (q: string) => {
    ...
    debounceRef.current = setTimeout(() => {
      transitApi
        .geocode(q, { lang: language, near: near || undefined })
  ...
  return (
    <div ref={rootRef} style={{ position: 'relative', flex: 1, minWidth: 0 }}>
      <label className="mb-[5px] block text-[11px] font-semibold uppercase tracking-[0.03em] text-content-faint">
        {label}
      </label>
      <div
        className="border border-edge bg-surface-input"
        style={{ display: 'flex', alignItems: 'center', gap: 7, borderRadius: 10, padding: '0 10px', height: 38 }}
      >
...
function ItineraryCard({
  ...
  return (
    <div className="border border-edge bg-surface-card" style={{ borderRadius: 14, overflow: 'hidden' }}>
      <button
        type="button"
        onClick={onToggle}
        style={{
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid var(--border-faint)', padding: '10px 14px 12px' }}>
            ...
            */}
            {it.legs.map((leg, i) => {
              ...
              return (
                <div
                  key={i}
                  style={{ display: 'grid', gridTemplateColumns: '44px 18px 1fr', gap: 8, alignItems: 'stretch' }}
                >
                  <div
                    className="text-content-muted"
                    style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    ...
                    <span
                      style={{
                        ...
                        borderRadius: 2,
                        background: color,
                        opacity: leg.mode === 'WALK' ? 0.45 : 1,
                        margin: '2px 0',
                        ...(leg.mode === 'WALK'
                          ? {
                              backgroundImage:
                                'repeating-linear-gradient(to bottom, var(--border-primary) 0 4px, transparent 4px 8px)',
                              background: 'none',
                            }
                          : {}),
                      }}
                        ...
                        <>
                          ...
                          <TransitMetaBadges
                            ...
                            items={[
                              ...
                              {
                                text:
                                  leg.intermediateStops > 0 ? t('transit.stops', { count: leg.intermediateStops }) : '',
                              },
                            ]}
                          />
                        </>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '44px 18px 1fr', gap: 8, alignItems: 'center' }}>
              <div
                className="text-content-muted"
                style={{ fontSize: 'calc(11.5px * var(--fs-scale-caption, 1))', fontWeight: 600, textAlign: 'right' }}
              >
          ...
          {transitLegs[0]?.agency && (
            <div
              className="text-content-faint"
              style={{ marginTop: 8, fontSize: 'calc(11px * var(--fs-scale-caption, 1))' }}
            >
...
export default function TransitSearchPanel({
  day,
  days,
  places,
  accommodations = [],
  ...
  const search = async () => {
    ...
    try {
      ...
      const modes = allModes
        ? undefined
        : MODE_GROUPS.filter((m) => activeModes.has(m.key))
            .map((m) => m.modes)
            .join(',');
      const d = await transitApi.plan({
    ...
    try {
      ...
      transitLegs.slice(0, -1).forEach((leg, i) => {
        ...
        endpoints.push({
          ...
          lat: s.lat,
          lng: s.lng,
          timezone: tzAt(s.lat, s.lng),
          local_date: t2 ? dateYMDInTz(t2, tzAt(s.lat, s.lng)) : null,
          local_time: t2 ? timeHHmmInTz(t2, tzAt(s.lat, s.lng)) : null,
        });
      });
      endpoints.push({
        role: 'to',
        ...
        metadata: {
          transit: {
            ...
            legs: it.legs.map((l) => ({
              ...
              from: {
                name: l.from.name,
                time: stopTime(l.from) ? timeHHmmInTz(stopTime(l.from)!, tzAt(l.from.lat, l.from.lng)) : null,
                track: l.from.track,
              },
              to: {
                name: l.to.name,
                time: stopTime(l.to) ? timeHHmmInTz(stopTime(l.to)!, tzAt(l.to.lat, l.to.lng)) : null,
                track: l.to.track,
              },
  ...
  const segBtn = (active: boolean): React.CSSProperties => ({
    padding: isMobile ? '6px 5px' : '6px 11px',
    fontSize: isMobile ? 'calc(11px * var(--fs-scale-body, 1))' : 'calc(12px * var(--fs-scale-body, 1))',
    borderRadius: 7,
    fontWeight: 500,
    border: 0,
    cursor: 'pointer',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, fontFamily: 'var(--font-system)' }}>
      ...
      <div
        ...
        style={{ borderRadius: 14, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}
      >
        <div
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div className="bg-surface-secondary" style={{ display: 'flex', borderRadius: 9, padding: 3 }}>
              <button type="button" onClick={() => setArriveBy(false)} style={segBtn(!arriveBy)}>
                {t('transit.depart')}
              </button>
              <button type="button" onClick={() => setArriveBy(true)} style={segBtn(arriveBy)}>
                {t('transit.arrive')}
              </button>
            </div>
            <div style={{ width: 110 }}>
          ...
          <div
            ...
            style={{ display: 'flex', borderRadius: 9, padding: 3, width: isMobile ? '100%' : undefined }}
          >
            <button
              type="button"
              onClick={() => setPref('best')}
              style={{ ...segBtn(pref === 'best'), flex: isMobile ? 1 : undefined }}
            >
              {t('transit.pref.best')}
            </button>
            <button
              type="button"
              onClick={() => setPref('transfers')}
              style={{ ...segBtn(pref === 'transfers'), flex: isMobile ? 1 : undefined }}
            >
              {t('transit.pref.transfers')}
            </button>
            <button
              type="button"
              onClick={() => setPref('walking')}
              style={{ ...segBtn(pref === 'walking'), flex: isMobile ? 1 : undefined }}
            >
              {t('transit.pref.walking')}
            </button>
          </div>
        </div>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              fontSize: 'calc(13px * var(--fs-scale-body, 1))',
              cursor: !from || !to || loading ? 'default' : 'pointer',
              fontFamily: 'inherit',
              opacity: !from || !to || loading ? 0.55 : 1,
              flexShrink: 0,
      ...
      {!loading && ranked && ranked.length === 0 && (
        <div
          className="text-content-faint"
          style={{ textAlign: 'center', padding: '24px 0', fontSize: 'calc(13px * var(--fs-scale-body, 1))' }}
        >
          {t('transit.noResults')}
        </div>
      )}
      {!loading && ranked && ranked.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div
            className="text-content-faint"
            style={{ fontSize: 'calc(10.5px * var(--fs-scale-caption, 1))', textAlign: 'center', marginTop: 2 }}
          >
