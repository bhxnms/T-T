import { Check, ChevronDown, FileDown, MapPin, Plus, Search, Star, Upload, X } from 'lucide-react';
  ...
  return (
    <div className="border-b border-edge-faint" style={{ padding: '14px 16px 10px', flexShrink: 0 }}>
      ...
      {(() => {
        ...
        ).filter(Boolean) as Array<{ id: 'all' | 'unplanned' | 'planned' | 'tracks'; label: string }>;
        return (
            ...
            <Tooltip label={t('places.filterByRating')} placement="bottom">
              <button
                ...
                style={{
                  ...
                  background:
                    ratingFilter !== 'all' ? 'color-mix(in srgb, var(--accent) 14%, transparent)' : 'var(--bg-card)',
                  cursor: 'pointer',
                  fontFamily: 'inherit',
                  fontSize: 'calc(12px * var(--fs-scale-body, 1))',
                  fontWeight: 600,
                ...
                {(['all', 5, 4, 3, 2, 1] as const).map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      key={String(opt)}
                      onClick={() => {
                        setRatingFilter(opt as number | 'all');
