...
import { placeWeekdayIndex, resolveOpenNow, resolvePlaceTimeZone } from './placeOpenState';
...
function Overline({ children }: { children: React.ReactNode }): React.ReactElement {
  return <p className="text-caption font-semibold uppercase tracking-[0.14em] text-content-faint">{children}</p>;
}
...
export default function PlaceDetailsColumn({
  ...
  return (
    ...
    <aside className="flex w-full shrink-0 flex-col self-stretch overflow-hidden rounded-xl border border-edge bg-surface-secondary sm:w-80">
      ...
      {selection && state === 'ready' && !data?.disabled && data?.description && (
        <div className="shrink-0 border-t border-edge p-2.5">
          <button
            ...
            className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-accent-subtle px-3 py-2 text-body font-medium text-accent-on transition-colors hover:bg-accent hover:text-accent-text disabled:opacity-50 disabled:hover:bg-accent-subtle disabled:hover:text-accent-on"
          >
...
function PhotoStrip({
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="truncate text-caption leading-tight text-content-faint">
...
function PhotoTile({
  ...
  return (
    <button
      ...
      className={`group relative block aspect-square w-full overflow-hidden rounded-lg transition-shadow ${
        selected
    ...
    >
      ...
      />
      {selected && (
        <span className="absolute right-1 top-1 rounded-full bg-accent p-0.5 shadow-card">
          <Check className="h-2.5 w-2.5 text-accent-on" />
        </span>
      )}
    </button>
  );
...
function NoKeyHint({ t }: { t: TranslationFn }): React.ReactElement {
  return (
    <div className="border-accent/25 rounded-xl border bg-accent-subtle p-3">
    ...
    <div className="flex items-center gap-1.5">
      <span className="flex items-center gap-0.5" aria-hidden>
        {[1, 2, 3, 4, 5].map((step) => (
          <Star
            key={step}
            className={`h-3 w-3 ${step <= filled ? 'fill-current text-accent-on' : 'text-content-faint'}`}
          />
        ))}
      </span>
      ...
      <div className="overflow-hidden rounded-lg border border-edge bg-surface">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-caption font-medium text-content">{todayLabel}</span>
            {openNow !== null && (
              <span
          ...
          {expanded ? (
            <ChevronUp className="h-3.5 w-3.5 shrink-0 text-content-faint" />
          ) : (
            <ChevronDown className="h-3.5 w-3.5 shrink-0 text-content-faint" />
          )}
        </button>
        {expanded && (
          <ul className="space-y-0.5 border-t border-edge-faint px-2.5 py-1.5">
            {lines.map((line, i) => {
              ...
              if (!day) {
                return (
                  <li key={i} className="break-words text-caption text-content-muted">
                    {times}
                  </li>
                );
              }
              return (
                <li
                  ...
                  className={`flex items-baseline justify-between gap-2 text-caption ${
                    i === today ? 'font-semibold text-content' : 'text-content-muted'
                  }`}
                >
      ...
      <div className="flex flex-wrap gap-1.5">
        {facts
          ...
          .map((fact) => {
            ...
            const shared =
              'inline-flex items-center gap-1.5 max-w-full rounded-full border border-edge bg-surface px-2 py-1 text-caption text-content-secondary';
            ...
            return href ? (
              <a
                ...
                title={label}
                className={`${shared} transition-colors hover:border-accent hover:text-content`}
              >
                {body}
              </a>
            ) : (
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-2 rounded-lg border border-edge bg-surface p-2.5">
        ...
        <p className="whitespace-pre-line text-caption leading-relaxed text-content">{description.text}</p>
