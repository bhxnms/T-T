...
import { assignmentsApi, daysApi, reservationsApi } from '../../api/client';
...
import {
  ...
  usePluginDaySchedule,
  usePluginDayTints,
} from '../Plugins/PluginDaySchedule';
...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const [hotelLegs, setHotelLegs] = useState<
    Record<
      number,
      {
        top?: { seg: RouteSegment; name: string; targetId?: number };
        bottom?: { seg: RouteSegment; name: string; targetId?: number };
      }
    >
  >({});
  ...
  const getSpanLabel = (r: Reservation, phase: string): string | null => {
    ...
    if (r.type === 'parking')
      return t(`reservations.span.${phase === 'start' ? 'dropOff' : phase === 'end' ? 'pickup' : 'ongoing'}`);
      ...
      for (const it of merged) {
        if (it.type === 'place' && it.data.place?.lat && it.data.place?.lng) {
          ...
          if (from || to) {
            ...
            const prev = cur[cur.length - 1];
            if (from && (!prev || withinDriveRange(prev, from)))
              cur.push({ id: r.id, lat: from.lat, lng: from.lng, isPlace: false });
            if (cur.length >= 2 && curHasPlace) runs.push(cur);
            cur = [];
            curHasPlace = false;
            ...
            cur.length > 0 &&
            !(r.type === 'car' && getSpanPhase(r, dayId) === 'middle') &&
            !hidesOnMiddleDay(r, dayId)
          ) {
        ...
        if (it.type === 'place' && it.data.place?.lat && it.data.place?.lng) {
          ...
          if (from)
            wayPts.push({
              ...
              isPlace: false,
              time: null,
              carrierEdge: carrier ? 'departure' : null,
            });
          if (to)
            wayPts.push({
              lat: to.lat,
              ...
              isPlace: false,
              time: null,
              carrierEdge: carrier ? 'arrival' : null,
            });
        }
      }
      ...
      for (const dayId of routeDayIds) {
        ...
        if (wantTop) {
          ...
          tasks.push(async () => {
            ...
            if (seg)
              hotel.top = { seg, name: hotelName(startHotel!), targetId: firstWay!.isPlace ? firstWay!.id : undefined };
          });
        }
        if (wantBottom) {
          ...
          tasks.push(async () => {
            ...
            if (seg)
              hotel.bottom = { seg, name: hotelName(endHotel!), targetId: lastWay!.isPlace ? lastWay!.id : undefined };
          });
        }
      }
    ...
    try {
      ...
      if (transportUpdates.length) {
        for (const tu of transportUpdates) {
          ...
          if (r)
            pendingRollback.push({ id: r.id, day_plan_position: r.day_plan_position, day_positions: r.day_positions });
        }
        useTripStore.setState((state) => ({
          reservations: state.reservations.map((r) => {
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const routeProfileOptions = useMemo(() => {
    ...
    // AMap (高德) routing: traffic-aware durations via the server proxy.
    // Offered whenever the instance carries a key; RouteCalculator dispatches
    // on the `amap:` prefix.
  ...
  const setDayDefaultMode = (dayId: number, mode: string) => {
    useTripStore.setState((state) => ({
      days: state.days.map((d) => (d.id === dayId ? { ...d, default_transport_mode: mode } : d)),
    }));
  ...
  const originDepartureTime = (originKey: string, dayId: number): string | null => {
    for (const a of assignments[String(dayId)] ?? []) {
      if (a.place?.lat != null && a.place?.lng != null && `${a.place.lat},${a.place.lng}` === originKey)
        return a.place.place_time ?? null;
    }
    for (const r of reservations) {
  ...
  const buildTransitLeg = (
    ...
    dayId: number
  ): { from: PickedPlace; to: PickedPlace; time: string | null } | null => {
    if (!seg?.from || !seg?.to) return null;
    const pick = (c: [number, number]): PickedPlace => ({
      name: transitNameIndex.get(`${c[0]},${c[1]}`) || '',
      lat: c[0],
      lng: c[1],
    });
  ...
  const transitLegMenuItem = (dayId: number, seg?: RouteSegment) => {
    ...
    return [
      {
        label: t('transit.title'),
        icon: TramFront,
        onClick: () => onPlanTransitLeg({ dayId, from: leg.from, to: leg.to, time: leg.time }),
      },
    ];
  };
  ...
  const openLegModeMenu = (e: React.MouseEvent, assignmentId: number, dayId: number, seg?: RouteSegment) => {
    ctxMenu.open(e, [
      ...routeProfileOptions.map((o) => ({
        label: o.label,
        icon: modeIcon(o.key),
        onClick: () => setLegMode(assignmentId, dayId, o.key),
      })),
      ...
      { divider: true },
      {
        ...
        icon: RotateCcw,
        onClick: () => setLegMode(assignmentId, dayId, null),
      },
    ]);
  };
  ...
  const openIncomingLegModeMenu = (e: React.MouseEvent, assignmentId: number, dayId: number, seg?: RouteSegment) => {
    ctxMenu.open(e, [
      ...routeProfileOptions.map((o) => ({
        label: o.label,
        icon: modeIcon(o.key),
        onClick: () => setIncomingLegMode(assignmentId, dayId, o.key),
      })),
      ...transitLegMenuItem(dayId, seg),
      { divider: true },
      {
        label: t('dayplan.transportMode.useDefault'),
        icon: RotateCcw,
        onClick: () => setIncomingLegMode(assignmentId, dayId, null),
      },
    ]);
  };
      ...
      >
        {days.map((day, index) => {
          ...
          const toggleDaySelection = () => {
            if (isSelected) {
              onSelectDay(null);
              if (onDayDetail) onDayDetail(null);
            } else {
              onSelectDay(day.id);
          ...
          const hasHotelTransfer = !!(
            routeBookends?.morningIsSleptHere &&
            routeBookends?.eveningIsOvernight &&
            transferMorning?.place_lat != null &&
            transferMorning?.place_lng != null &&
            transferEvening?.place_lat != null &&
            transferEvening?.place_lng != null &&
            (transferMorning.place_lat !== transferEvening.place_lat ||
              transferMorning.place_lng !== transferEvening.place_lng)
          );
          ...
          const dayExportStops = (): NamedWaypoint[] => {
            ...
            const first = dayStops[0]
              ? {
                  isPlace: true,
                  time: dayStops[0].place?.place_time ?? null,
                  lat: dayStops[0].place!.lat!,
                  lng: dayStops[0].place!.lng!,
                }
              : undefined;
            ...
            const last = lastAssignment
              ? {
                  isPlace: true,
                  time: lastAssignment.place?.place_time ?? null,
                  lat: lastAssignment.place!.lat!,
                  lng: lastAssignment.place!.lng!,
                }
              : undefined;
            ...
            const dayHasCarrier = (mergedItemsMap[day.id] || []).some(
              (i) => i.type === 'transport' && hasCarrierEndpointOnDay(i.data, day.id)
            );
            ...
            const morning =
              drawMorning && routeBookends?.morning?.place_lat != null && routeBookends?.morning?.place_lng != null
                ? {
                    lat: routeBookends.morning.place_lat,
                    lng: routeBookends.morning.place_lng,
                    name: routeBookends.morning.place_name,
                  }
                : null;
            const evening =
              drawEvening && routeBookends?.evening?.place_lat != null && routeBookends?.evening?.place_lng != null
                ? {
                    lat: routeBookends.evening.place_lat,
                    lng: routeBookends.evening.place_lng,
                    name: routeBookends.evening.place_name,
                  }
                : null;
          ...
          const visibleCount = merged.filter(
            (i) => !(i.type === 'transport' && hidesOnMiddleDay(i.data, day.id))
          ).length;
          ...
          const headerTintHoverBg =
            dayTintBackground(dayTint, 'header', '--day-tint-header-hover') ?? 'var(--bg-tertiary)';
          ...
          return (
            ...
            >
              ...
              <div
                ...
                onMouseEnter={(e) => {
                  if (!isSelected && !isDragTarget) e.currentTarget.style.background = headerTintHoverBg;
                }}
                onMouseLeave={(e) => {
                  if (!isSelected)
                    e.currentTarget.style.background = isDragTarget ? 'rgba(17,24,39,0.07)' : headerTintBg;
                }}
              >
                  ...
                  return (
                    <div
                      style={{
                        ...
                        flexShrink: 0,
                        alignSelf: hasWeather ? 'flex-start' : 'center',
                        ...
                        background: isSelected
                          ? 'var(--accent)'
                          : (dayTintBackground(dayTint, 'badge', '--day-tint-badge', 'var(--bg-hover)') ??
                            'var(--bg-hover)'),
                        color: isSelected
                          ? 'var(--accent-text)'
                          : dayTinted(dayTint, 'badge')
                            ? 'var(--text-secondary)'
                            : 'var(--text-muted)',
                        display: 'flex',
                        flexDirection: 'column',
              ...
              {isExpanded && (
                <div
                  ...
                  style={{
                    background:
                      dayTintBackground(dayTint, 'activity', '--day-tint-activity', 'var(--bg-hover)') ??
                      'var(--bg-hover)',
                    paddingTop: 6,
                  }}
                  onDragOver={(e) => {
                ...
                >
                    ...
                    (() => {
                      ...
                      return canEditDays && targetId != null ? (
                        <div
                          role="button"
                          tabIndex={0}
                          title={t('dayplan.transportMode.change')}
                          onClick={(e) => openIncomingLegModeMenu(e, targetId, day.id, hotelLegs[day.id]!.top!.seg)}
                          onKeyDown={(e) =>
                            openLegMenuByKey(e, (m) =>
                              openIncomingLegModeMenu(m, targetId, day.id, hotelLegs[day.id]!.top!.seg)
                            )
                          }
                  ...
                  {daySchedule.byPosition[day.id]?.start.map((si) => (
                    <PluginDayScheduleRow key={`${si.pluginId}:${si.id}`} item={si} />
                  ))}
                      ...
                      if (item.type === 'place') {
                          ...
                          if (parseTimeToMinutes(placeTime) !== null) {
                            ...
                            if (!isChronological) {
                              ...
                              setTimeConfirm({
                                ...
                                time: timeStr,
                                fromType: 'place',
                                toType: neighbour.type,
                                toId: neighbour.data.id,
                        ...
                        return (
                          <React.Fragment key={`place-${assignment.id}`}>
                              ...
                              onContextMenu={(e) => {
                                ...
                                ctxMenu.open(e, [
                                  ...
                                  safeHttpUrl(place.website) && {
                                    ...
                                    icon: ExternalLink,
                                    onClick: () =>
                                      window.open(safeHttpUrl(place.website)!, '_blank', 'noopener,noreferrer'),
                                  },
                            ...
                            >
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {assignment.notes && (
                                  ...
                                  <div
                                    title={t('places.assignmentNotes')}
                                    style={{
                                      marginTop: 2,
                                      display: 'flex',
                                      alignItems: 'center',
                                      ...
                                      {linked.map((res) => {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            {timeLabel && (
                                              <span className={tint} style={{ ...chip, fontWeight: 500 }}>
                                                {timeLabel}
                                              </span>
                                            )}
                                            {carrierLabel && (
                                            ...
                                            {hasEndpoints && (
                                              <button
                                                ...
                                                className={
                                                  active
                                                    ? 'bg-[#3b82f6] text-[#fff]'
                                                    : 'bg-transparent text-content-faint'
                                                }
                                                style={{
                                                  flexShrink: 0,
                                                  appearance: 'none',
                            ...
                            {daySchedule.byAssignment[day.id]?.[assignment.id]?.map((si) => (
                              <PluginDayScheduleRow key={`${si.pluginId}:${si.id}`} item={si} />
                            ))}
                            {routeLegs[day.id]?.[assignment.id] &&
                              (canEditDays ? (
                                <div
                                  ...
                                  title={t('dayplan.transportMode.change')}
                                  onClick={(e) =>
                                    openLegModeMenu(e, assignment.id, day.id, routeLegs[day.id]![assignment.id])
                                  }
                                  onKeyDown={(e) =>
                                    openLegMenuByKey(e, (m) =>
                                      openLegModeMenu(m, assignment.id, day.id, routeLegs[day.id]![assignment.id])
                                    )
                                  }
                                  style={{ cursor: 'pointer' }}
                                >
                      ...
                      if (item.type === 'transport') {
                        ...
                        return (
                          <React.Fragment key={`transport-${res.id}-${legKey}-${day.id}`}>
                            ...
                            {daySchedule.byReservation[day.id]?.[res.id]?.map((si) => (
                              <PluginDayScheduleRow key={`${si.pluginId}:${si.id}`} item={si} />
                            ))}
                            {routeLegs[day.id]?.[res.id] &&
                              (() => {
                                const nextPlaceId = merged
                                  .slice(idx + 1)
                                  .find((i) => i.type === 'place' && i.data.place?.lat && i.data.place?.lng)?.data.id;
                                const connector = (
                          ...
                          >
                            ...
                            <div
                              style={{
                                ...
                                justifyContent: 'center',
                                borderRadius: '50%',
                                background: noteSkin.iconBackground,
                                overflow: 'hidden',
                              }}
                            >
                  ...
                  {daySchedule.byPosition[day.id]?.end.map((si) => (
                    <PluginDayScheduleRow key={`${si.pluginId}:${si.id}`} item={si} />
                  ))}
                    ...
                    (() => {
                      ...
                      return canEditDays && targetId != null ? (
                        <div
                          ...
                          tabIndex={0}
                          title={t('dayplan.transportMode.change')}
                          onClick={(e) => openLegModeMenu(e, targetId, day.id, hotelLegs[day.id]!.bottom!.seg)}
                          onKeyDown={(e) =>
                            openLegMenuByKey(e, (m) =>
                              openLegModeMenu(m, targetId, day.id, hotelLegs[day.id]!.bottom!.seg)
                            )
                          }
                          style={{ cursor: 'pointer' }}
                        >
                      ...
                      {isSelected && daySchedule.minutesByDay[day.id] ? (
                        ...
                        >
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <Zap size={11} strokeWidth={2} />+{formatScheduleMinutes(daySchedule.minutesByDay[day.id])}
                          </span>
                        </div>
                      ) : null}
                    </div>
