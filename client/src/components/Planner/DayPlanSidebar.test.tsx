...
beforeEach(() => {
  ...
  vi.mocked(calculateRouteWithLegs).mockImplementation((waypoints) =>
    Promise.resolve({
      coordinates: [],
      distance: 0,
      duration: 0,
...
describe('DayPlanSidebar', () => {
  ...
  it('FE-PLANNER-DAYPLAN-005d: route tools show for a two-hotel transfer day with no places (#1297)', () => {
    ...
    const accommodations = [
      ...
      { id: 2, start_day_id: 11, end_day_id: 12, place_lat: 51.5, place_lng: -0.12 },
    ];
    render(
      <DayPlanSidebar
        {...makeDefaultProps({
          days: [dayA, dayTransfer, dayB],
          accommodations: accommodations as any,
          selectedDayId: 11,
        })}
      />
  ...
  it('FE-PLANNER-DAYPLAN-005f: no route tools on a plain arrival day with no places (#1297 guard)', () => {
    ...
    const accommodations = [{ id: 2, start_day_id: 11, end_day_id: 12, place_lat: 51.5, place_lng: -0.12 }];
    render(
      <DayPlanSidebar
        {...makeDefaultProps({
          days: [dayArrive, dayNext],
          accommodations: accommodations as any,
          selectedDayId: 11,
        })}
      />
  ...
  it('FE-PLANNER-DAYPLAN-104: a transit journey renders line chips and opens its itinerary view, not the edit form (#1065)', async () => {
        ...
        transit: {
          ...
          legs: [
            { mode: 'WALK', duration: 240, from: { name: 'Start' }, to: { name: 'Alexanderplatz' } },
            {
              ...
              duration: 1440,
              stops: 6,
              from: { name: 'Alexanderplatz', time: '08:36' },
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], reservations: [res as any], onEditTransport, onOpenTransit })}
      />
    );
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({
          days: [day],
          reservations: [res1 as any],
          visibleConnectionIds: [401],
          onToggleConnection: vi.fn(),
          allConnectionsShown: true,
          onToggleAllConnections,
        })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-108: the toolbar button is absent when the trip has no routable reservation', () => {
    const day = buildDay({ id: 10, date: '2025-06-01', title: 'Day 1' });
    render(
      <DayPlanSidebar {...makeDefaultProps({ days: [day], reservations: [], onToggleAllConnections: vi.fn() })} />
    );
  ...
  describe('jump to today', () => {
    const isoDaysAround = (offsets: number[]) =>
      offsets.map((o, i) => {
        const d = new Date();
  ...
  it('FE-PLANNER-DAYPLAN-064c: a link inside the note body opens the link, not the edit modal', async () => {
    ...
    const day = buildDay({ id: 10, date: '2025-06-01', title: 'Day 1' });
    mockDayNotesState.dayNotes = {
      '10': [buildDayNote({ id: 55, day_id: 10, text: 'My note', time: '[docs](https://example.com)' })],
    };
    render(<DayPlanSidebar {...makeDefaultProps({ days: [day] })} />);
  ...
  it('FE-PLANNER-DAYPLAN-065: the note modal deletes only through the confirmation dialog', async () => {
    ...
    mockDayNotesState.dayNotes = { '10': [buildDayNote({ id: 55, day_id: 10, text: 'My note' })] };
    mockDayNotesState.noteUi = {
      '10': { mode: 'edit', noteId: 55, text: 'My note', time: '', icon: 'FileText', color: null },
    };
    render(<DayPlanSidebar {...makeDefaultProps({ days: [day] })} />);
    ...
    const assigns = {
      '10': [
        buildAssignment({
          ...
          day_id: 10,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'A1', lat: 10.0, lng: 2.0 }),
        }),
        buildAssignment({
          id: 6,
          day_id: 10,
          order_index: 1,
          place: buildPlace({ id: 2, name: 'A2', lat: 10.01, lng: 2.01 }),
        }),
      ],
      '11': [
        buildAssignment({
          ...
          day_id: 11,
          order_index: 0,
          place: buildPlace({ id: 3, name: 'B1', lat: 20.0, lng: 3.0 }),
        }),
        buildAssignment({
          id: 7,
          day_id: 11,
          order_index: 1,
          place: buildPlace({ id: 4, name: 'B2', lat: 20.01, lng: 3.01 }),
        }),
      ],
    };
    render(
    ...
    const assigns = {
      '11': [
        buildAssignment({
          ...
          day_id: 11,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'A', lat: 48.85, lng: 2.35 }),
        }),
        buildAssignment({
          id: 2,
          day_id: 11,
          order_index: 1,
          place: buildPlace({ id: 2, name: 'B', lat: 48.86, lng: 2.36 }),
        }),
      ],
    };
    render(
  ...
  it('FE-PLANNER-DAYPLAN-111: an externally requested booking opens its detail modal and hands editing back', async () => {
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ externalTransportDetail: res, onExternalTransportDetailHandled, onEditTransport })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-112: the list restores its saved scroll offset and reports further scrolling', () => {
    ...
    Object.defineProperty(Element.prototype, 'scrollTop', {
      configurable: true,
      get(this: Element & { _st?: number }) {
        return this._st ?? 0;
      },
      set(this: Element & { _st?: number }, v: number) {
        this._st = v;
      },
    });
    try {
  ...
  it('FE-PLANNER-DAYPLAN-114: bookings without a saved slot get one derived from their time', async () => {
    ...
    const late = buildReservation({
      ...
      type: 'flight',
      title: 'Late flight',
      reservation_time: '2025-06-01T18:00:00',
      day_id: 10,
    });
    const early = buildReservation({
      ...
      type: 'train',
      title: 'Early train',
      reservation_time: '2025-06-01T06:00:00',
      day_id: 10,
    });
  ...
  it('FE-PLANNER-DAYPLAN-116: dragging a span start past its end collapses the booking onto one day', () => {
    ...
    const flight = buildReservation({
      ...
      day_id: 10,
      end_day_id: 12,
      reservation_time: '2025-06-02T10:00:00',
    });
  ...
  it('FE-PLANNER-DAYPLAN-200: a multi-day parking shows on the first and last day only (#1937)', () => {
    ...
    const parking = buildReservation({
      ...
      day_id: 10,
      end_day_id: 12,
      reservation_time: '2025-06-01T05:30:00',
      reservation_end_time: '2025-06-03T19:00:00',
    });
    ...
    const assignments = {
      '11': [
        buildAssignment({
          ...
          day_id: 11,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'Louvre', lat: 48.86, lng: 2.34 }),
        }),
        buildAssignment({
          id: 2,
          day_id: 11,
          order_index: 1,
          place: buildPlace({ id: 2, name: 'Orsay', lat: 48.87, lng: 2.33 }),
        }),
      ],
    };
    render(
  ...
  it('FE-PLANNER-DAYPLAN-121: a located booking splits the day into separate drive runs', async () => {
    ...
    const flight = buildReservation({
      ...
      title: 'Hop over',
      day_id: 10,
      reservation_time: '2025-06-01T12:00:00',
      endpoints: [
        {
          role: 'from',
          sequence: 0,
    ...
    const assignments = {
      '10': [
        buildAssignment({
          ...
          day_id: 10,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'Morning stop', place_time: '09:00', lat: 48.86, lng: 2.34 }),
        }),
        buildAssignment({
          id: 2,
          day_id: 10,
          order_index: 1,
          place: buildPlace({ id: 2, name: 'Evening stop', place_time: '18:00', lat: 43.7, lng: 7.26 }),
        }),
      ],
    };
    render(
  ...
  it('FE-PLANNER-DAYPLAN-205: the legs of a day are fetched together, not one after the other', async () => {
    const { calculateRouteWithLegs } = await import('../Map/RouteCalculator');
    const release: (() => void)[] = [];
    vi.mocked(calculateRouteWithLegs as any).mockImplementation(
      () =>
        new Promise((resolve) => {
          release.push(() =>
            resolve({
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], places, assignments, selectedDayId: 10, routeShown: true })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-123: adding a note to a collapsed day expands it first', async () => {
    const user = userEvent.setup();
    mockDayNotesState.openAddNote.mockImplementation(
      (dayId: number, _getMerged: unknown, expand: (id: number) => void) => expand(dayId)
    );
  ...
  it('FE-PLANNER-DAYPLAN-124: reordering a day persists booking slots, note order and an undo step', async () => {
    ...
    await waitFor(() =>
      expect(vi.mocked(reservationsApi.updatePositions)).toHaveBeenCalledWith(
        1,
        [{ id: 430, day_plan_position: expect.any(Number) }],
        10
      )
    );
  ...
  it('FE-PLANNER-DAYPLAN-126: reordering around a multi-leg flight writes a position per leg', async () => {
    ...
    fireEvent.dragStart(dragRow(screen.getByText('Place B')), { dataTransfer: emptyDataTransfer });
    fireEvent.drop(cardRow(screen.getAllByText('Berlin to New York')[0]), {
      dataTransfer: { getData: vi.fn(() => '') },
    });

    await waitFor(() => expect(updateReservation).toHaveBeenCalled());
    const payload = (updateReservation.mock.calls[0] as unknown as unknown[])[2] as {
  ...
  it('FE-PLANNER-DAYPLAN-128: undoing a lock restores the previous locked set', async () => {
    ...
    const pushUndo = vi.fn();
    render(
      <DayPlanSidebar {...makeDefaultProps({ days: [day], places: [place], assignments: { '10': [a] }, pushUndo })} />
    );
  ...
  it('FE-PLANNER-DAYPLAN-130b: the weather badge anchors to the day bookend hotel, independent of the optimize setting (#2167)', () => {
    ...
    seedStore(useSettingsStore, {
      settings: { time_format: '24h', temperature_unit: 'celsius', optimize_from_accommodation: false },
    } as any);
    const day = buildDay({ id: 10, date: '2025-06-01', title: 'Day 1' });
    const day2 = buildDay({ id: 11, date: '2025-06-02', title: 'Day 2' });
    const accommodations = [
      { id: 1, start_day_id: 10, end_day_id: 11, place_name: 'Hotel Roma', place_lat: 41.9, place_lng: 12.5 },
    ];
  ...
  it('FE-PLANNER-DAYPLAN-146: a sidebar place dropped on a note lands at the note position', () => {
    ...
    render(
      <DayPlanSidebar {...makeDefaultProps({ days: [day], places: [placeA, placeB], assignments, onAssignToDay })} />
    );
  ...
  it('FE-PLANNER-DAYPLAN-151: cross-day and same-day payloads dropped on a place row are handled apart', async () => {
    ...
    const taxi = buildReservation({ id: 501, type: 'taxi', title: 'Airport taxi', day_id: 10 });
    render(
      <DayPlanSidebar {...makeDefaultProps({ days, places: [place, target], assignments, reservations: [taxi] })} />
    );
  ...
  it('FE-PLANNER-DAYPLAN-154: an assignment without a place renders nothing for that row', () => {
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({
          days: [day],
          assignments: { '10': [orphan] } as never,
        })}
      />
    );
    expect(screen.getByText('Day 1')).toBeInTheDocument();
  ...
  it('FE-PLANNER-DAYPLAN-155: below lg place rows lose their drag handle', () => {
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], places: [place], assignments: { '10': [a] }, isMobile: true })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-155b: at desktop width place rows keep grip and drag', () => {
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], places: [place], assignments: { '10': [a] }, isMobile: false })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-159: dragging over a booking row marks it, and dragEnd clears the marker', () => {
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], places: [placeA, placeB], assignments, reservations: [bus] })}
      />
    );
  ...
  it('FE-PLANNER-DAYPLAN-160: cross-day payloads dropped on a booking row move onto its day', () => {
    ...
    render(
      <DayPlanSidebar {...makeDefaultProps({ days, places: [place], assignments, reservations: [taxi, target] })} />
    );
  ...
  it('FE-PLANNER-DAYPLAN-161: same-day payloads dropped on a booking row reorder the day', async () => {
    ...
    const { day, placeA, placeB, bus, assignments } = dayWithBusAndPlaces();
    mockDayNotesState.dayNotes = { '10': [buildDayNote({ id: 70, day_id: 10, text: 'A note', sort_order: -1 })] };
    const secondBus = buildReservation({
      id: 501,
      type: 'bus',
      title: 'Shuttle',
      day_id: 10,
  ...
  it('FE-PLANNER-DAYPLAN-162: a linked flight shows its times, airline and a route toggle', async () => {
    ...
    const linked = withEndpoints(
      buildReservation({
        ...
        assignment_id: 11,
        reservation_time: '2025-06-01T08:30:00',
        reservation_end_time: '2025-06-01T10:05:00',
  ...
  it('FE-PLANNER-DAYPLAN-163b: every booking on the stop gets its own chip line (#2201)', async () => {
    ...
    const parking = buildReservation({
      ...
      assignment_id: 11,
      reservation_time: '2025-06-01T09:00:00',
      reservation_end_time: '2025-06-01T09:30:00',
    } as any);
  ...
  it('FE-PLANNER-DAYPLAN-169: picking a whole-day travel mode persists it and redraws the map', async () => {
    ...
    render(
      <DayPlanSidebar {...makeDefaultProps({ days: [day], assignments, selectedDayId: 10, onSetRouteProfile })} />
    );
  ...
  it('FE-PLANNER-DAYPLAN-171b: a stop-to-stop connector offers public transport pre-filled from the leg', async () => {
    ...
    const assignments = {
      '10': [
        buildAssignment({
          ...
          day_id: 10,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'Louvre', place_time: '10:30', lat: 48.86, lng: 2.34 }),
        }),
        buildAssignment({
          id: 12,
          day_id: 10,
    ...
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days: [day], assignments, selectedDayId: 10, routeShown: true, onPlanTransitLeg })}
      />
    );
    await user.click(await screen.findByTitle('Change travel mode'));
    ...
    const assignments = {
      '10': [
        buildAssignment({
          ...
          day_id: 10,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'Louvre', place_time: '09:00', lat: 48.86, lng: 2.34 }),
        }),
      ],
      '11': [
        buildAssignment({
          ...
          day_id: 11,
          order_index: 0,
          place: buildPlace({ id: 1, name: 'Louvre', place_time: '16:30', lat: 48.86, lng: 2.34 }),
        }),
        buildAssignment({
          id: 22,
          day_id: 11,
          order_index: 1,
          place: buildPlace({ id: 3, name: 'Rodin', lat: 48.855, lng: 2.315 }),
        }),
      ],
    };
    render(
      <DayPlanSidebar
        {...makeDefaultProps({ days, assignments, selectedDayId: 11, routeShown: true, onPlanTransitLeg })}
      />
    );
    await user.click(await screen.findByTitle('Change travel mode'));
    await user.click(contextMenu().getByRole('button', { name: 'Public transit' }));
  ...
  it('FE-PLANNER-DAYPLAN-180: the chronology check spans notes and bookings, not just stops', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /confirm/i }));
    await waitFor(() =>
      expect(vi.mocked(assignmentsApi.updateTime)).toHaveBeenCalledWith(1, 12, { place_time: null, end_time: null })
    );
    await waitFor(() => expect(onReorder).toHaveBeenCalledWith(10, [12, 11]));
  ...
  it('FE-PLANNER-DAYPLAN-182: optimising registers an undo that restores the previous order', async () => {
    ...
    const onReorder = vi.fn(async () => undefined);
    render(
      <DayPlanSidebar {...makeDefaultProps({ days: [day], assignments, selectedDayId: 10, pushUndo, onReorder })} />
    );
    await user.click(screen.getByRole('button', { name: 'Optimize' }));
    await waitFor(() => expect(onReorder).toHaveBeenCalled());
    const undo = pushUndo.mock.calls[0][1] as () => Promise<void>;
  ...
  it('FE-PLANNER-DAYPLAN-184: failed cross-day moves from the day header are reported', async () => {
    stubTripActions({
      moveAssignment: vi.fn(async () => {
        throw new Error('move failed');
      }),
      moveDayNote: vi.fn(async () => {
    ...
    const transit = buildReservation({
      ...
      metadata: JSON.stringify({
        transit: {
          ...
          transfers: 0,
          walk_seconds: 60,
          legs: [{ mode: 'WALK', duration: 300, from: { name: 'A' }, to: { name: 'B' } }],
        },
      }),
    });
  ...
  it('FE-W5DPS-001: a flight without an airline still shows its flight number', async () => {
    renderWith([
      buildReservation({
        ...
        day_id: 10,
        reservation_time: '2025-06-01T09:00:00',
        metadata: JSON.stringify({ flight_number: 'AF900' }),
      }) as Reservation,
    ]);
  ...
  it('FE-W5DPS-003: a booking without usable metadata gets no subtitle', async () => {
    renderWith([
      buildReservation({
        ...
        day_id: 10,
        reservation_time: '2025-06-01T09:00:00',
        metadata: JSON.stringify({ seat: '4B' }),
      }) as Reservation,
    ]);
