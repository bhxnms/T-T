...
export function TransportModal({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (form.type === 'flight') {
        ...
        if (toPick.location)
          endpoints.push(
            endpointFromLocation(toPick.location, 'to', stops.length + 1, endDate, form.arrival_time || null)
          );
      }
      ...
      if (!reservation && prefill && isBudgetEnabled) {
        const pmeta =
          prefill.metadata && typeof prefill.metadata === 'object' ? (prefill.metadata as Record<string, unknown>) : {};
        const price = Number(pmeta.price);
        if (Number.isFinite(price) && price > 0) {
          (payload as Record<string, unknown>).create_budget_entry = {
            total_price: price,
            category: typeToCostCategory(form.type),
          };
        }
      }
      ...
      if (savedId && tripId) {
        ...
        if (changed) {
          try {
            await setReservationTravelers(tripId, savedId, nextIds);
          } catch {
  ...
  const prefillMeta =
    prefill?.metadata && typeof prefill.metadata === 'object' ? (prefill.metadata as Record<string, unknown>) : null;
  const prefillPrice = Number(prefillMeta?.price);
  const pendingExpense =
    !reservation && Number.isFinite(prefillPrice) && prefillPrice > 0
      ? {
          total_price: prefillPrice,
          currency: (prefillMeta?.priceCurrency as string | null) ?? null,
          category: typeToCostCategory(form.type),
        }
      : null;
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={
        automated ? t('transit.title') : reservation ? t('transport.modalTitle.edit') : t('transport.modalTitle.create')
      }
      size="2xl"
      footer={
    ...
    >
      ...
      {!reservation && tripHasDates && (
        <div
          ...
          style={{ display: 'flex', borderRadius: 11, padding: 3, gap: 2, marginBottom: 14 }}
        >
          {(
            [
              ['manual', t('transport.modeManual')],
              ['automated', t('transport.modeAutomated')],
            ] as const
          ).map(([m, label]) => {
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 180 }}>
              <div
                ...
                style={{ fontSize: 'calc(14px * var(--fs-scale-body, 1))', fontWeight: 700, letterSpacing: '-0.01em' }}
              >
                {t('transit.title')}
              </div>
              <div
                className="text-content-faint"
                style={{ fontSize: 'calc(11.5px * var(--fs-scale-caption, 1))', marginTop: 1 }}
              >
                {t('transit.searchHint')}
              </div>
            </div>
            <div
              style={{ width: typeof window !== 'undefined' && window.innerWidth < 768 ? '100%' : 210, flexShrink: 0 }}
            >
              <CustomSelect
          ...
          {(() => {
            ...
            if (!transitDay)
              return (
                <div
                  className="text-content-faint"
                  style={{ fontSize: 'calc(13px * var(--fs-scale-body, 1))', padding: '4px 2px 12px' }}
                >
                  {t('transit.pickDay')}
                </div>
              );
          ...
          {form.type === 'flight' ? (
                    ...
                    >
                      ...
                      {!isLast && (
                        <>
                          ...
                          <div
                            className={`grid grid-cols-1 ${writesFlightLegs ? 'sm:grid-cols-4' : 'sm:grid-cols-3'} gap-3`}
                          >
                            <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {trainWaypoints.map((wp, i) => {
                ...
                const updateWp = (patch: Partial<StationWaypointForm>) =>
                  setTrainWaypoints((prev) => prev.map((w, j) => (j === i ? { ...w, ...patch } : w)));
                const roleLabel = isFirst
                  ? t('reservations.meta.from')
                  : isLast
                    ? t('reservations.meta.to')
                  ...
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        {!isFirst && !isLast && (
                          <button
                            type="button"
                            onClick={() => setTrainWaypoints((prev) => prev.filter((_, j) => j !== i))}
                            aria-label={t('common.delete')}
                            className="text-content-faint"
                            style={{
                              background: 'none',
                      ...
                      {!isLast && (
                        <>
                          ...
                          <div
                            className={`grid grid-cols-1 ${writesTrainLegs ? 'sm:grid-cols-4' : 'sm:grid-cols-3'} gap-3`}
                          >
                            <div>
                              <label className={labelClass}>{t('reservations.meta.trainNumber')}</label>
                              <input
                                type="text"
                                value={wp.train_number}
                                onChange={(e) => updateWp({ train_number: e.target.value })}
                                placeholder="ICE 123"
                                className={inputClass}
                              />
                            </div>
                            <div>
                              ...
                              <input
                                type="text"
                                value={wp.platform}
                                onChange={(e) => updateWp({ platform: e.target.value })}
                                placeholder="12"
                                className={inputClass}
                              />
                            </div>
                    ...
                    {!isLast && (
                      <button
                        type="button"
                        onClick={() =>
                          setTrainWaypoints((prev) => [
                            ...prev.slice(0, i + 1),
                            emptyStationWaypoint(prev[i]?.depDayId || ''),
                            ...prev.slice(i + 1),
                          ])
                        }
                        className="text-content-faint hover:text-content-secondary"
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input
                ref={fileInputRef}
                type="file"
                accept=".pdf,.doc,.docx,.txt,.pkpass,.pkpasses,image/*,application/vnd.apple.pkpass,application/vnd.apple.pkpasses"
                style={{ display: 'none' }}
