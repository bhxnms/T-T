...
function usePlaceFormModal(props: PlaceFormModalProps) {
  ...
  const [mapSearchProvider, setMapSearchProvider] = useState<'amap' | 'native'>(() =>
  ...
  const { hasMapsKey, hasAmapKey, amapSearchEnabled, placesEnrichEnabled } = useAuthStore();
  const can = useCanDo();
...
export default function PlaceFormModal(props: PlaceFormModalProps) {
  ...
  return (
      ...
      footer={
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-edge px-4 py-2 text-sm text-content-secondary hover:bg-surface-hover hover:text-content"
          >
            {t('common.cancel')}
          </button>
          <button
            ...
            className="rounded-lg bg-accent px-6 py-2 text-sm font-medium text-accent-text hover:bg-accent-hover disabled:opacity-60"
          >
            {isSaving
              ? t('common.saving')
              : place
    ...
    >
      <div className={twoColumn || showDetails ? 'flex items-stretch gap-5' : ''}>
        {showDetails && (
          <PlaceDetailsColumn
        ...
        <form
          ...
          className={twoColumn || showDetails ? 'min-w-0 flex-1 space-y-3' : 'space-y-3'}
          onPaste={handlePaste}
        >
          {/* Place Search */}
          <div className="rounded-xl border border-edge bg-surface-secondary p-3">
            {!hasMapsKey && !hasAmapKey && <p className="mb-2 text-xs text-content-faint">{t('places.osmActive')}</p>}
            {hasAmapKey && (
              <div className="mb-2 flex items-center gap-1.5 text-xs text-content-faint">
                <span>{t('places.searchProvider')}</span>
                <button
                  type="button"
                  onClick={() => setMapSearchProvider('amap')}
                  className={`rounded px-2 py-0.5 ${mapSearchProvider === 'amap' ? 'bg-accent text-accent-text' : 'bg-surface-tertiary'}`}
                >
                  {t('places.searchProviderAmap')}
                </button>
                <button
                  type="button"
                  onClick={() => setMapSearchProvider('native')}
                  className={`rounded px-2 py-0.5 ${mapSearchProvider === 'native' ? 'bg-accent text-accent-text' : 'bg-surface-tertiary'}`}
                >
            ...
            <div className="relative">
              <div className="flex gap-2">
                <input
                  ...
                  className="focus:ring-accent/30 flex-1 rounded-lg border border-edge bg-surface-input px-3 py-1.5 text-sm focus:outline-none focus:ring-2"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="rounded-lg bg-accent px-3 py-1.5 text-sm text-accent-text hover:bg-accent-hover disabled:opacity-60"
                >
              ...
              {acSuggestions.length > 0 && (
                <div className="absolute left-0 right-0 z-20 mt-1 overflow-hidden rounded-lg border border-edge bg-surface-card shadow-dropdown">
                  {acSuggestions.map((s, idx) => (
                    ...
                    >
                      ...
                      {s.secondaryText && <div className="truncate text-xs text-content-muted">{s.secondaryText}</div>}
                    </button>
                  ))}
                </div>
              )}
          ...
          {/* Name */}
          <div>
            <label className="mb-1 block text-sm font-medium text-content-secondary">{t('places.formName')} *</label>
            <div className="relative">
              <input
                type="text"
                value={form.name}
              ...
              {isSearchingMaps && (
                ...
                >
                  <Loader2 className="h-4 w-4 animate-spin text-content-faint" aria-hidden="true" />
                </div>
              )}
            </div>
          </div>
          ...
          {/* Address + Coordinates */}
          <div>
            <label className="mb-1 block text-sm font-medium text-content-secondary">{t('places.formAddress')}</label>
            <input
              type="text"
              value={form.address}
          ...
          {/* Website */}
          <div>
            <label className="mb-1 block text-sm font-medium text-content-secondary">{t('places.formWebsite')}</label>
            <input
              type="url"
              value={form.website}
              ...
              {pendingFiles.length > 0 && (
                <div className="space-y-1" data-testid="pending-files">
                  {pendingFiles.map((file, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2 rounded-lg bg-surface-secondary px-2 py-1.5 text-xs"
                    >
                      <Paperclip size={10} className="shrink-0 text-content-faint" />
                      <span className="flex-1 truncate text-content-secondary">{file.name}</span>
                      <button
                        type="button"
                        onClick={() => handleRemoveFile(idx)}
                        className="shrink-0 text-content-faint hover:text-red-500"
                      >
                        <X size={12} />
                      </button>
                    </div>
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="mb-1 block text-sm font-medium text-content-secondary">{t('places.startTime')}</label>
          <CustomTimePicker value={form.place_time} onChange={(v) => handleChange('place_time', v)} />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-content-secondary">{t('places.endTime')}</label>
          <CustomTimePicker value={form.end_time} onChange={(v) => handleChange('end_time', v)} />
        </div>
      </div>
      {hasTimeError && (
        <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-warning-soft px-2.5 py-1.5 text-caption text-warning">
          <AlertTriangle size={13} className="shrink-0" />
          {t('places.endTimeBeforeStart')}
        </div>
      )}
      {collisions.length > 0 && (
        <div className="mt-2 flex items-start gap-1.5 rounded-lg bg-warning-soft px-2.5 py-1.5 text-caption text-warning">
