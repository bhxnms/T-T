...
import { cleanAmountText, formatTime, splitReservationDateTime } from '../../utils/formatters';
...
import { TransitLegChips, TransitMetaBadges, TransitTitle, fmtTransitDuration } from './transitDisplay';
...
function ConfirmationCode({
  ...
  if (!blurred)
    return (
      <div className={`${fieldValueClass} text-center`} style={{ ...codeStyle, cursor: 'default' }}>
        {code}
      </div>
    );
  return (
      ...
      >
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, minWidth: 0, flexWrap: 'wrap' }}>
          ...
          {r.external_source === 'airtrail' ? (
            <span
              className={
                r.sync_enabled || airtrailMultiLeg
              ...
              title={
                r.sync_enabled
                  ? t('reservations.airtrail.syncedHint')
                  : airtrailMultiLeg
                    ? t('reservations.airtrail.layoverHint')
                    : t('reservations.airtrail.notSyncedHint')
              }
            >
              <Plane size={11} />
              {r.sync_enabled || airtrailMultiLeg
                ? t('reservations.airtrail.synced')
                : t('reservations.airtrail.notSynced')}
            </span>
          ) : null}
        </div>
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
        ...
        {hasCode && (
          <div>
            ...
            <ConfirmationCode
              code={r.confirmation_number}
              blurred={!!blurCodes}
              revealed={codeRevealed}
              onReveal={setCodeRevealed}
            />
          </div>
        )}
          ...
          return (
            <div
              style={{
                display: 'grid',
                gap: 10,
                gridTemplateColumns: coded.length > 1 ? `repeat(${Math.min(coded.length, 3)}, 1fr)` : '1fr',
              }}
            >
              {coded.map((l, i) => (
                <div key={i}>
                  <div className={fieldLabelClass}>
                    {[l.from, l.to].filter(Boolean).join(' → ') || t('reservations.confirmationCode')}
                  </div>
                  <ConfirmationCode
                    code={l.confirmation_number}
                    blurred={!!blurCodes}
                    revealed={codeRevealed}
                    onReveal={setCodeRevealed}
                  />
        ...
        {(() => {
          ...
          if (meta.seat)
            cells.push({
              label: t('reservations.meta.seat'),
              value: meta.seat + (meta.class ? ` · ${meta.class}` : ''),
            });
          if (meta.price != null && meta.price !== '')
            cells.push({
              label: t('reservations.price'),
              value: `${cleanAmountText(meta.price)}${meta.priceCurrency ? ' ' + meta.priceCurrency : ''}`,
            });
        ...
        {r.notes && (
          <div>
            ...
            >
              <Markdown remarkPlugins={[remarkGfm, remarkBreaks]} components={markdownLinkComponents}>
                {r.notes}
              </Markdown>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{ fontSize: 'calc(12.5px * var(--fs-scale-body, 1))', lineHeight: 1.5 }}
              >
        ...
        createPortal(
          ...
          >
            <div
              className="bg-surface-card"
              role="presentation"
              style={{ borderRadius: 14, padding: 20, width: 340, boxShadow: '0 16px 48px rgba(0,0,0,0.22)' }}
              onClick={(e) => e.stopPropagation()}
            >
              <div
                className="text-content"
                style={{ fontWeight: 600, fontSize: 'calc(14px * var(--fs-scale-body, 1))', marginBottom: 6 }}
              >
                {t('reservations.confirm.deleteTitle')}
              </div>
              <div
                className="text-content-muted"
                style={{ fontSize: 'calc(12.5px * var(--fs-scale-body, 1))', marginBottom: 14 }}
              >
                {t('reservations.confirm.deleteBody', { name: r.title })}
              </div>
...
export default function ReservationsPanel({
  ...
  const reservationDetailPlugins = usePluginStore((s) => s.plugins).filter(
    (p) => p.type === 'widget' && p.slot === 'reservation-detail'
  );
          ...
          {tripMembers.length > 1 && reservations.some((r) => (r.travelers || []).length > 0) && (
            <>
              <div
                ...
                style={{ width: 1, height: 22, background: 'var(--border-faint)', flexShrink: 0 }}
              />
              <TravelerFilterAvatars
                members={tripMembers}
                active={travelerFilters}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px 80px' }} className="max-md:!px-4 max-md:!pt-4">
        {total === 0 && reservations.length === 0 ? (
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                  ...
                  {onImport && bookingImportAvailable && (
                    <button
                      ...
                      onClick={onImport}
                      className="bg-surface-card text-content"
                      style={{ ...CTA_STYLE, border: '1px solid var(--border-primary)' }}
                    >
                      <Download size={14} strokeWidth={2} />
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            <p className="text-content-faint" style={{ fontSize: 'calc(13px * var(--fs-scale-body, 1))' }}>
              {t('places.noneFound')}
            </p>
          </div>
        ) : (
          <>
            {transitEntries.length > 0 && (
              <Section
                title={t('transit.sectionTitle')}
                count={transitEntries.length}
                accent="gray"
                storageKey={`trek:bookings-transit-open:${tripId}`}
              >
                {transitEntries.map((r) => (
                  <TransitJourneyCard
                    key={r.id}
                    r={r}
                    days={days}
                    ...
                    detailPlugins={reservationDetailPlugins}
                  />
                ))}
              </Section>
            )}
