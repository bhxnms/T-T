...
function NoteDialog({
  ...
  onRequestDelete,
  t,
}: Omit<DayPlanSidebarNoteModalProps, 'noteUi'> & {
  dayId: string;
  ui: NoteModalUi;
}) {
  ...
  return (
    ...
    >
      <div
        className="note-modal bg-surface-card"
        style={{
          width: '100%',
          maxWidth: 720,
          maxHeight: '90vh',
      ...
      >
        ...
        <div
          className="note-modal-cols"
          style={{ display: 'grid', gridTemplateColumns: '210px 1fr', gap: 20, alignItems: 'start' }}
        >
          {/* Left — how it looks. */}
                ...
                {NOTE_ICONS.map(({ id, Icon }) => (
                  ...
                  >
                    <Icon
                      size={16}
                      strokeWidth={1.8}
                      color={ui.icon === id ? (color ?? 'var(--text-primary)') : 'var(--text-muted)'}
                    />
                  </button>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
            <div>
              ...
              <input
                ...
                required
                className="bg-surface-input text-content"
                style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginTop: 4 }}>
                <span className="text-content-faint" style={{ fontSize: 'calc(10.5px * var(--fs-scale-caption, 1))' }}>
                  {t('notes.markdownHint')}
                </span>
                <span
                  className={bodyLen >= BODY_MAX - 100 ? 'text-[#d97706]' : 'text-content-faint'}
            ...
            {ui.time?.trim() && (
              <div
                className="collab-note-md-full text-content-muted"
                style={{
                  borderRadius: 10,
                  border: `1px solid ${surface.border}`,
                  background: surface.background,
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center' }}>
          {ui.mode === 'edit' && ui.noteId != null && (
            <button
              type="button"
              onClick={() => onRequestDelete(Number(dayId), ui.noteId!)}
