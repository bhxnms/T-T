...
export function ReservationModal({
  ...
  useEffect(() => {
    ...
    const matchPlaceId = (name: string | undefined): string | number => {
      ...
      const loose = places.find(
        (p) => p.name && (p.name.toLowerCase().includes(n) || n.includes(p.name.toLowerCase()))
      );
      return loose?.id ?? '';
    };
    ...
    if (reservation) {
      ...
      const meta = (prefill.metadata && typeof prefill.metadata === 'object' ? prefill.metadata : {}) as Record<
        string,
        string
      >;
          ...
          <>
            ...
            {isEndBeforeStart && (
              <div
                ...
                style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', marginTop: -6 }}
              >
