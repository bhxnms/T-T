...
import { inheritedTrackColor, resolveTrackColor } from '../Map/trackColors';
...
function PhotoCredit({ imageUrl }) {
  ...
  useEffect(() => {
    ...
    if (creditCache.has(key)) {
      setCredit(creditCache.get(key));
      return;
    }
    let alive = true;
    mapsApi
      .placePhotoCredit(key)
      .then((data) => {
...
export default function PlaceInspector({
  ...
  // 已打卡 check-in state (client-side, mirrored onto the Atlas map). Re-read per
  // place id so switching places keeps the toggle honest.
  ...
  const placeFiles = filesForPlace(
    ...
    place.id,
    reservations,
    selectedAssignmentId != null ? [selectedAssignmentId] : []
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          {assignmentInDay?.notes && (
            <div
              ...
              style={{ borderRadius: 10, overflow: 'hidden', flexShrink: 0, padding: '8px 12px' }}
            >
              <div
                className="text-content-faint"
                style={{
        ...
        >
          ...
          {/* 已打卡 check-in — feeds the Atlas check-in dots + the 打卡点 sidebar count */}
          {mode === 'trip' && (
            <ActionButton
              ...
              label={
                <span className="hidden sm:inline">
                  {checkedIn ? t('inspector.checkinDone') : t('inspector.checkin')}
                </span>
              }
            />
          )}
  ...
  return (
    <>
      {(() => {
        ...
        return (
          <div
            className={`grid ${linked.length > 0 && showParticipants ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1'} gap-2`}
          >
