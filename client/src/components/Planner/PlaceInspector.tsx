...
import { X, Clock, MapPin, ExternalLink, Phone, Banknote, Edit2, Trash2, Plus, Minus, ChevronDown, ChevronUp, FileText, Upload, File, FileImage, Star, Navigation, Map as MapIcon, Users, Mountain, TrendingUp, Bookmark, BookmarkCheck, Copy, Route, StickyNote, CheckCircle2 } from 'lucide-react'
    ...
    >
      <div className="bg-surface-elevated" style={{
        ...
        <div className="border-t border-edge-faint" style={{ padding: '10px 16px', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', flexShrink: 0 }}>
          ...
          {mode === 'trip' && (
            <ActionButton onClick={handleToggleCheckin} variant={checkedIn ? 'primary' : 'ghost'}
              icon={<CheckCircle2 size={13} />}
              label={<span className="hidden sm:inline">{checkedIn ? t('inspector.checkinDone') : t('inspector.checkin')}</span>} />
          )}
