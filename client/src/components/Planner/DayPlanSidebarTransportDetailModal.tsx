        ...
        {(() => {
          ...
          if (Array.isArray(meta.legs) && meta.legs.length > 1) {
            for (const leg of meta.legs) {
              ...
              detailFields.push({
                label: segment || t('reservations.confirmationCode'),
                value: leg.confirmation_number,
                sensitive: true,
              });
            }
          }
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div
                    ...
                    style={{ fontSize: 'calc(15px * var(--fs-scale-subtitle, 1))', fontWeight: 600 }}
                  >
                    {res.title}
                  </div>
                  <div
                    ...
                    return (
                      <div key={i} className="bg-surface-tertiary" style={{ padding: '8px 10px', borderRadius: 8 }}>
                        ...
                        {shouldBlur ? (
                          ...
                          <div className="text-content" style={valueStyle}>
                            {f.value}
                          </div>
                        )}
                      </div>
                ...
                <>
                  ...
                  >
                    {meta.transit.duration > 0 && (
                      <span>
                        {Math.floor(meta.transit.duration / 3600) > 0
                          ? `${Math.floor(meta.transit.duration / 3600)} h ${Math.round((meta.transit.duration % 3600) / 60)} min`
                          : t('transit.min', { count: Math.round(meta.transit.duration / 60) })}
                      </span>
                    )}
                    <span className="text-content-faint">·</span>
                    <span>
                      {meta.transit.transfers > 0
                        ? t('transit.transfers', { count: meta.transit.transfers })
                        : t('transit.direct')}
                    </span>
                    {meta.transit.walk_seconds > 59 && (
                      <>
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                          <Footprints size={13} />{' '}
                          {t('transit.min', { count: Math.round(meta.transit.walk_seconds / 60) })}
                        </span>
                      </>
                    )}
                  </div>
                          ...
                          return (
                            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div
                                  className="text-content-faint"
                                  style={{ fontSize: 'calc(10px * var(--fs-scale-caption, 1))', marginTop: 1 }}
                                >
                                  {[
                                    leg.from?.time && !isWalk
