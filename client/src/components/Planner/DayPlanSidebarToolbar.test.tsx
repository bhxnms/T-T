...
describe('DayPlanSidebarToolbar', () => {
  ...
  it('FE-PLANNER-DPTOOLBAR-017: the undo tooltip names the last action when there is one', () => {
    render(
      <DayPlanSidebarToolbar
        {...makeProps({ onUndo: vi.fn(), canUndo: true, undoHover: true, lastActionLabel: 'Reorder' })}
      />
    );
  ...
  it('FE-PLANNER-DPTOOLBAR-018: the undo tooltip falls back to the plain label with nothing to undo', () => {
    render(
      <DayPlanSidebarToolbar
        {...makeProps({ onUndo: vi.fn(), canUndo: false, undoHover: true, lastActionLabel: 'Reorder' })}
      />
    );
    // Both the aria-label and the tooltip carry the same text here.
    expect(screen.getAllByText('undo.button').length + screen.getAllByLabelText('undo.button').length).toBeGreaterThan(
      1
    );
  ...
  it('FE-PLANNER-DPTOOLBAR-019: the reorder button toggles the day-reorder popup', async () => {
    ...
    render(
      <DayPlanSidebarToolbar {...makeProps({ days, canEditDays: true, onReorderDays: vi.fn(), onAddDay: vi.fn() })} />
    );
  ...
  it('FE-PLANNER-DPTOOLBAR-021: the reorder button is hidden without edit rights or without days', () => {
    ...
    const { rerender } = render(
      <DayPlanSidebarToolbar {...makeProps({ days, canEditDays: false, onReorderDays: vi.fn(), onAddDay: vi.fn() })} />
    );
    expect(screen.queryByRole('button', { name: 'dayplan.reorderDays' })).not.toBeInTheDocument();
    rerender(
      <DayPlanSidebarToolbar
        {...makeProps({ days: [], canEditDays: true, onReorderDays: vi.fn(), onAddDay: vi.fn() })}
      />
    );
    expect(screen.queryByRole('button', { name: 'dayplan.reorderDays' })).not.toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-DPTOOLBAR-022: the connections toggle appears only for a routable reservation', async () => {
    ...
    const { rerender } = render(
      <DayPlanSidebarToolbar {...makeProps({ onToggleAllConnections, reservations: [buildReservation({ id: 6 })] })} />
    );
    expect(screen.queryByRole('button', { name: 'map.showAllConnections' })).not.toBeInTheDocument();

    rerender(
      <DayPlanSidebarToolbar {...makeProps({ onToggleAllConnections, reservations: [routableReservation()] })} />
    );
    ...
    rerender(
      <DayPlanSidebarToolbar
        {...makeProps({ onToggleAllConnections, reservations: [routableReservation()], allConnectionsShown: true })}
      />
    );
  ...
  it('FE-PLANNER-DPTOOLBAR-023: the icon buttons paint a hover background and clear it again', async () => {
    ...
    render(
      <DayPlanSidebarToolbar
        {...makeProps({ days, onToggleAllConnections: vi.fn(), reservations: [routableReservation()] })}
      />
    );
