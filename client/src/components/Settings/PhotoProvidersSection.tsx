...
export default function PhotoProvidersSection(): React.ReactElement {
    ...
    for (const provider of activePhotoProviders) {
      ...
      if (cfg.settings_get) {
        apiClient
          ...
          .catch((err) => {
            console.error(`Failed to load ${provider.name} photo provider settings:`, err);
          });
      }

      refreshProviderConnection(provider).catch((err) => {
        console.error(`Failed to refresh ${provider.name} photo provider connection:`, err);
      });
    }
  ...
  const handleSaveProvider = async (provider: PhotoProviderAddon) => {
    ...
    try {
      ...
      console.error(`Failed to save ${provider.name} photo provider settings:`, err);
  ...
  const handleTestProvider = async (provider: PhotoProviderAddon) => {
    ...
    try {
      ...
      console.error(`Failed to test ${provider.name} photo provider connection:`, err);
