    ...
    <>
      <Section title={t('settings.apiKeys.title')} icon={KeyRound}>
        ...
        <div className="flex justify-end">
          ...
          >
            <Plus className="h-3.5 w-3.5" /> {t('settings.apiKeys.create')}
          </button>
        </div>
        ...
        {keys.length === 0 ? (
          ...
          <div className="overflow-hidden rounded-lg border border-edge">
            {keys.map((key, i) => (
              <div
                key={key.id}
                className={`flex items-center gap-3 px-4 py-3 ${i < keys.length - 1 ? 'border-b border-edge' : ''}`}
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 font-mono text-xs" style={{ color: 'var(--text-tertiary)' }}>
                    ...
                    <span className="ml-3 font-sans">
                      {t('settings.apiKeys.createdAt')} {new Date(key.created_at).toLocaleDateString(locale)}
                    </span>
                    {key.last_used_at && (
                      <span className="ml-2">
                        · {t('settings.apiKeys.usedAt')} {new Date(key.last_used_at).toLocaleDateString(locale)}
                      </span>
                    )}
                  </p>
                </div>
      ...
      {modalOpen && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget && !created) closeModal();
          }}
        >
              ...
              <>
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-xs" style={{ color: 'var(--text-tertiary)' }}>
                    {t('settings.apiKeys.modal.nameHint')}
                  </p>
                </div>
