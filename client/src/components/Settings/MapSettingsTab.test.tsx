...
describe('MapSettingsTab – GL providers', () => {
  ...
  it('FE-COMP-MAP-030b: selecting AMap opens its key configuration', async () => {
    const user = userEvent.setup();
    render(<MapSettingsTab />);

    const amapButton = screen.getByRole('button', { name: /高德地图/ });
    expect(amapButton).toHaveAttribute('type', 'button');
    await user.click(amapButton);
