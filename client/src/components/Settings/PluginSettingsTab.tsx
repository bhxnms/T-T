...
import { findMissingRequired, seedSettingsValues, settingsPatch } from '../Plugins/settingsForm';
...
function PluginOAuthSection({
  ...
  return (
    <div className="border-border mt-4 flex items-center justify-between rounded-lg border bg-surface-secondary px-3 py-2">
      <span className="flex items-center gap-2 text-sm text-content-secondary">
        {state.connected ? (
          <>
            <CheckCircle className="h-4 w-4 text-success" /> {t('settings.plugins.oauth.connected')}
          </>
        ) : (
...
function PluginSettingsForm({ id, name, icon }: { id: string; name: string; icon: string | null }) {
  ...
  return (
    <div className="border-border rounded-xl border bg-surface p-5">
      <div className="mb-4 flex items-center gap-2">
        <PluginIcon name={icon} className="h-4 w-4 text-content-secondary" />
          ...
          <label key={f.key} className="block">
            ...
            {f.input_type === 'checkbox' ? (
              ...
              <select
                ...
                className="border-border w-full rounded-lg border bg-surface-secondary px-3 py-2 text-sm text-content"
              >
                <option value="">—</option>
                {f.options.map((o) => (
            ...
            {actions.map((a) => {
              ...
              return (
                <div key={a.key} className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {running === a.key && <Loader2 className="h-4 w-4 animate-spin" />}
                    {a.label}
                  </button>
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-content-faint">{t('settings.plugins.compatibility')}</p>
      </div>
      {plugins.length === 0 ? (
