...
export default function NotificationsTab(): React.ReactElement {
  ...
  const renderContent = () => {
    if (!matrix)
      return (
        <p
          style={{ fontSize: 'calc(12px * var(--fs-scale-body, 1))', color: 'var(--text-faint)', fontStyle: 'italic' }}
        >
          {t('common.loading')}
        </p>
      );
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
        {saving && (
          <p
            style={{ fontSize: 'calc(11px * var(--fs-scale-caption, 1))', color: 'var(--text-faint)', marginBottom: 8 }}
          >
            {t('common.saving')}
          </p>
        )}
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid var(--border-primary)',
                  borderRadius: 6,
                  cursor: !ch.configured || channelTesting === ch.id ? 'not-allowed' : 'pointer',
                  opacity: !ch.configured || channelTesting === ch.id ? 0.5 : 1,
                }}
              >
        ...
        >
          ...
          {visibleChannels.map((ch) => (
            <span
              key={ch.id}
              title={!ch.configured ? t('settings.notificationPreferences.notConfigured') : undefined}
              style={{
                fontSize: 'calc(11px * var(--fs-scale-caption, 1))',
                fontWeight: 600,
                color: ch.configured ? 'var(--text-faint)' : 'var(--color-warning, #d69e2e)',
                textAlign: 'center',
