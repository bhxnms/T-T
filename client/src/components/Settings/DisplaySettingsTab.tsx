import { Check, ChevronDown, Languages, Map, Rocket } from 'lucide-react';
...
export default function DisplaySettingsTab(): React.ReactElement {
  ...
  const [distanceUnit, setDistanceUnit] = useState<DistanceUnit>(
    settings.distance_unit || DEFAULT_SETTINGS.distance_unit
  );
  ...
  return (
    <>
      <Section title={t('settings.general.startup')} icon={Rocket}>
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-content-secondary">{t('settings.startPage')}</label>
          <div className="flex gap-3">
            {(
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    await updateSetting('start_page', opt.value);
                  } catch (e: unknown) {
          ...
          <p className="mt-1 text-xs text-content-faint">{t('settings.startPageHint')}</p>
        </div>
        ...
        {startPage === 'active_trip' && (
          <div>
            <label className="mb-2 block text-sm font-medium text-content-secondary">
              {t('settings.startTripTab')}
            </label>
            <CustomSelect
              value={startTripTab}
              onChange={async (v) => {
                try {
                  await updateSetting('start_trip_tab', String(v));
                } catch (e: unknown) {
                  toast.error(e instanceof Error ? e.message : t('common.error'));
                }
              }}
            ...
            />
            <p className="mt-2 text-xs text-content-faint">{t('settings.startTripTabHint')}</p>
          </div>
        )}
      </Section>
      ...
      <Section title={t('settings.general.travelMap')} icon={Map}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  border:
                    (settings.map_booking_labels === true) === opt.value
                      ? '2px solid var(--text-primary)'
                      : '2px solid var(--border-primary)',
                  background:
                    (settings.map_booking_labels === true) === opt.value ? 'var(--bg-hover)' : 'var(--bg-card)',
                  color: 'var(--text-primary)',
                  transition: 'all 0.15s',
                }}
              >
        ...
        <div>
          ...
          <div className="flex gap-3">
            {[
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    await updateSetting('map_always_show_routes', opt.value);
                  } catch (e: unknown) {
                    toast.error(e instanceof Error ? e.message : t('common.error'));
                  }
                }}
                style={{
                  ...
                  fontWeight: 500,
                  border:
                    (settings.map_always_show_routes === true) === opt.value
                      ? '2px solid var(--text-primary)'
                      : '2px solid var(--border-primary)',
                  background:
                    (settings.map_always_show_routes === true) === opt.value ? 'var(--bg-hover)' : 'var(--bg-card)',
                  color: 'var(--text-primary)',
                  transition: 'all 0.15s',
                }}
              >
          ...
          <p className="mt-1 text-xs text-content-faint">{t('settings.alwaysShowRoutesHint')}</p>
        </div>
