...
const es: NotificationLocale = {
  email: {
    footer: 'Recibiste esto porque tienes las notificaciones activadas en Tourism-Team.',
    manage: 'Gestionar preferencias',
    madeWith: 'Made with',
    openTrek: 'Abrir Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} te invitó a fusionar planes de vacaciones. Abre Tourism-Team para aceptar o rechazar.`,
    }),
    vacay_share: (p) => ({
      title: 'Calendario Vacay compartido',
      body: `${p.actor} compartió su calendario de vacaciones contigo. Abre Tourism-Team para verlo.`,
    }),
    collection_invite: (p) => ({
      title: 'Invitación a colección',
      body: `${p.actor} te invitó a compartir una colección. Abre Tourism-Team para aceptar o rechazar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nueva versión de Tourism-Team disponible',
      body: `Tourism-Team ${p.version} ya está disponible. Visita el panel de administración para actualizar.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Hola',
    body: 'Recibimos una solicitud para restablecer la contraseña de tu cuenta de Tourism-Team. Haz clic en el botón de abajo para establecer una nueva contraseña.',
