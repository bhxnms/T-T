...
const tr: NotificationLocale = {
  email: {
    footer: "Tourism-Team'te bildirimleri etkinleştirdiğiniz için bunu aldınız.",
    manage: 'Ayarlarda tercihleri yönetin',
    madeWith: 'Made with',
    openTrek: "Tourism-Team'i aç",
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için Tourism-Team'i açın.`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay Takvimi Paylaşıldı',
      body: `${p.actor} tatil takvimini sizinle paylaştı. Görüntülemek için Tourism-Team'i açın.`,
    }),
    collection_invite: (p) => ({
      title: 'Koleksiyon daveti',
      body: `${p.actor} sizi bir koleksiyonu paylaşmaya davet etti. Kabul etmek veya reddetmek için Tourism-Team’i açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni Tourism-Team sürümü mevcut',
      body: `Tourism-Team ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'Tourism-Team hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
