...
const fr: NotificationLocale = {
  email: {
    footer: 'Vous recevez cet e-mail car les notifications sont activées dans Tourism-Team.',
    manage: 'Gérer les préférences',
    madeWith: 'Made with',
    openTrek: 'Ouvrir Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vous invite à fusionner les plans de vacances. Ouvrez Tourism-Team pour accepter ou refuser.`,
    }),
    vacay_share: (p) => ({
      title: 'Calendrier Vacay partagé',
      body: `${p.actor} a partagé son calendrier de vacances avec vous. Ouvrez Tourism-Team pour le consulter.`,
    }),
    collection_invite: (p) => ({
      title: 'Invitation à une collection',
      body: `${p.actor} vous invite à partager une collection. Ouvrez Tourism-Team pour accepter ou refuser.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nouvelle version Tourism-Team disponible',
      body: `Tourism-Team ${p.version} est maintenant disponible. Rendez-vous dans le panneau d'administration pour mettre à jour.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Bonjour',
    body: 'Nous avons reçu une demande de réinitialisation du mot de passe de votre compte Tourism-Team. Cliquez sur le bouton ci-dessous pour définir un nouveau mot de passe.',
