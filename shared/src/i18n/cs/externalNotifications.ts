...
const cs: NotificationLocale = {
  email: {
    footer: 'Toto jsi obdržel/a, protože máš povoleny upozornění v Tourism-Team.',
    manage: 'Spravovat předvolby v nastavení',
    madeWith: 'Made with',
    openTrek: 'Otevřít Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete Tourism-Team pro přijetí nebo odmítnutí.`,
    }),
    vacay_share: (p) => ({
      title: 'Kalendář Vacay sdílen',
      body: `${p.actor} s vámi sdílel svůj kalendář dovolených. Otevřete Tourism-Team pro zobrazení.`,
    }),
    collection_invite: (p) => ({
      title: 'Pozvánka do sbírky',
      body: `${p.actor} vás pozval ke sdílení sbírky. Otevřete Tourism-Team pro přijetí nebo odmítnutí.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nová verze Tourism-Team dostupná',
      body: `Tourism-Team ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Ahoj',
    body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu Tourism-Team. Klikni na tlačítko níže a nastav nové heslo.',
