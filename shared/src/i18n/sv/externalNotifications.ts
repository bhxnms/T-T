...
const en: NotificationLocale = {
  email: {
    footer: 'Du har fått detta eftersom du har aktiverat aviseringar i Tourism-Team.',
    manage: 'Hantera egenskaper under Inställningar',
    madeWith: 'Gjorde med',
    openTrek: 'Öppna Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} bjöd in dig att slå samman semesterplaner. Öppna Tourism-Team för att acceptera eller avvisa.`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay kalender delad',
      body: `${p.actor} delade sin semesterkalender med dig. Öppna Tourism-Team för att se den.`,
    }),
    collection_invite: (p) => ({
      title: 'Inbjudan till samling',
      body: `${p.actor} bjöd in dig att dela en samling. Öppna Tourism-Team för att acceptera eller avvisa.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Ny Tourism-Team version tillgänglig',
      body: `Tourism-Team ${p.version} är nu tillgänglig. Gå till adminpanelen för att uppdatera.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'Hej',
    body: 'Vi har fått en begäran om att återställa lösenordet till ditt Tourism-Team konto. Klicka på knappen nedan för att ange ett nytt lösenord.',
