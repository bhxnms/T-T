...
const ko: NotificationLocale = {
  email: {
    footer: 'Tourism-Team에서 알림을 활성화했기 때문에 이 이메일을 받으셨습니다.',
    manage: '설정에서 환경설정 관리',
    madeWith: 'Made with',
    openTrek: 'Tourism-Team 열기',
  },
  events: {
    trip_invite: (p) => ({
      title: `"${p.trip}" 여행 초대`,
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}이(가) 휴가 계획을 합치도록 초대했습니다. Tourism-Team을 열어 수락하거나 거절하세요.`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay 캘린더 공유됨',
      body: `${p.actor}이(가) 휴가 캘린더를 공유했습니다. Tourism-Team을 열어 확인하세요.`,
    }),
    collection_invite: (p) => ({
      title: '컬렉션 초대',
      body: `${p.actor}이(가) 컬렉션 공유에 초대했습니다. Tourism-Team을 열어 수락하거나 거절하세요.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '새 Tourism-Team 버전 사용 가능',
      body: `Tourism-Team ${p.version}을 사용할 수 있습니다. 관리자 패널에서 업데이트하세요.`,
    }),
    replica_failure: (p) => ({
      title: '스토리지 복제본 오류',
      body:
  ...
  passwordReset: {
    subject: '비밀번호 재설정',
    greeting: '안녕하세요',
    body: 'Tourism-Team 계정 비밀번호 재설정 요청을 받았습니다. 아래 버튼을 클릭하여 새 비밀번호를 설정하세요.',
    ctaIntro: '비밀번호 재설정',
