...
const zh: NotificationLocale = {
  email: {
    footer: '您收到此邮件是因为您在 Tourism-Team 中启用了通知。',
    manage: '管理偏好设置',
    madeWith: 'Made with',
    openTrek: '打开 Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
      title: `邀请加入"${p.trip}"`,
    ...
    vacay_invite: (p) => ({
      title: '假期合并邀请',
      body: `${p.actor} 邀请你合并假期计划。打开 Tourism-Team 接受或拒绝。`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay 日历共享',
      body: `${p.actor} 与你共享了假期日历。打开 Tourism-Team 查看。`,
    }),
    collection_invite: (p) => ({
      title: '收藏邀请',
      body: `${p.actor} 邀请你共享收藏。打开 Tourism-Team 接受或拒绝。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      ...
      body: `Tourism-Team ${p.version} 现已可用。请前往管理面板进行更新。`,
    }),
    replica_failure: (p) => ({
      title: '存储副本故障',
      body:
  ...
  passwordReset: {
    subject: '重置您的密码',
    greeting: '您好',
    body: '我们收到了重置您的 Tourism-Team 账户密码的请求。点击下方按钮设置新密码。',
    ctaIntro: '重置密码',
