...
const zhTW: NotificationLocale = {
  email: {
    footer: '您收到這封郵件是因為您在 Tourism-Team 中啟用了通知。',
    manage: '管理偏好設定',
    madeWith: 'Made with',
    openTrek: '開啟 Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
      title: `邀請加入「${p.trip}」`,
    ...
    vacay_invite: (p) => ({
      title: 'Vacay 融合邀請',
      body: `${p.actor} 邀請您合併假期計畫。開啟 Tourism-Team 以接受或拒絕。`,
    }),
    vacay_share: (p) => ({
      title: 'Vacay 日曆已共享',
      body: `${p.actor} 與您共享了假期日曆。開啟 Tourism-Team 以檢視。`,
    }),
    collection_invite: (p) => ({
      title: '收藏邀請',
      body: `${p.actor} 邀請您共享收藏。開啟 Tourism-Team 以接受或拒絕。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新版 Tourism-Team 可用',
      body: `Tourism-Team ${p.version} 現已可用。請前往管理面板進行更新。`,
    }),
    replica_failure: (p) => ({
      title: '儲存複本故障',
      body:
  ...
  passwordReset: {
    subject: '重設您的密碼',
    greeting: '您好',
    body: '我們收到了重設您 Tourism-Team 帳號密碼的請求。點擊下方按鈕以設定新密碼。',
    ctaIntro: '重設密碼',
