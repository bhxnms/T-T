...
const en: NotificationLocale = {
  email: {
    footer: 'Bạn nhận được thông báo này vì bạn đã bật thông báo trong Tourism-Team.',
    manage: 'Quản lý tùy chọn trong Cài đặt',
    madeWith: 'Được làm bằng',
    openTrek: 'Mở Tourism-Team',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} đã mời bạn kết hợp kế hoạch kỳ nghỉ. Mở Tourism-Team để chấp nhận hoặc từ chối.`,
    }),
    vacay_share: (p) => ({
      title: 'Đã chia sẻ lịch Vacay',
      body: `${p.actor} đã chia sẻ lịch nghỉ phép của họ với bạn. Mở Tourism-Team để xem.`,
    }),
    collection_invite: (p) => ({
      title: 'Lời mời bộ sưu tập',
      body: `${p.actor} đã mời bạn chia sẻ một bộ sưu tập. Mở Tourism-Team để chấp nhận hoặc từ chối.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Đã có phiên bản Tourism-Team mới',
      body: `Tourism-Team ${p.version} có bản mới. Vui lòng truy cập bảng điều khiển quản trị để cập nhật.`,
    }),
    replica_failure: (p) => ({
  ...
  passwordReset: {
    ...
    greeting: 'CHÀO',
    body: 'Chúng tôi đã nhận được yêu cầu đặt lại mật khẩu cho tài khoản Tourism-Team của bạn. Nhấp vào nút bên dưới để đặt mật khẩu mới.',
