...
/** TREK-backup import: which trips of a previewed snapshot to copy into TT. */
export const trekImportRequestSchema = z.object({
  token: z.string().min(1),
  trip_ids: z.array(z.number().int()).min(1).max(200),
});
export type TrekImportRequest = z.infer<typeof trekImportRequestSchema>;
