...
export const mapsSearchRequestSchema = z.object({
  ...
  // Provider override for this one request: 'amap' forces the AMap (高德)
  // adapter, 'native' forces TREK's own stack (Google when a key resolves,
  // else Nominatim), and an omitted value follows the instance setting.
  // 'amap' without a configured/allowed key degrades to the instance default
  // rather than failing, so a stale client toggle never breaks the search.
  provider: z.enum(['amap', 'native']).optional(),
});
...
export const mapsAutocompleteRequestSchema = z.object({
  ...
  /** Per-request provider override, same semantics as the search request. */
  provider: z.enum(['amap', 'native']).optional(),
  /**
...
/**
 * AMap route request. Coordinates are WGS-84 (TREK's storage frame); the server
 * adapter converts to GCJ-02 for AMap and back for the returned polyline, so the
 * client never has to know which frame a provider speaks.
 */
export const mapsAmapRouteRequestSchema = z.object({
  profile: z.enum(['driving', 'walking', 'cycling']).optional(),
  // 50 is the practical ceiling for one day's stops; driving's own multi-point
  // limit is lower and the adapter walks leg-by-leg anyway.
  waypoints: z.array(latLng).min(2).max(50),
});
export type MapsAmapRouteRequest = z.infer<typeof mapsAmapRouteRequestSchema>;
