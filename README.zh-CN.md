...
- **演示账号改用 `@tt.local`。** 更早的版本曾初始化为 `demo@trek.app` / `admin@trek.app`（再早是 `demo@nomad.app`）；这些地址现在仍会被识别，因此就地升级的实例无需重置即可继续工作。另外，初始化程序与每小时重置程序的管理员默认地址此前不一致，导致未设置 `DEMO_ADMIN_EMAIL` 时每次重置都会静默丢失管理员的密码和 API 密钥——现在两者共用同一份候选列表。
