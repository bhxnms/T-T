...
Install Tourism-Team on Proxmox VE as an LXC container using the [Proxmox VE Community Scripts](https://community-scripts.org/scripts/trek).

> A big thank you to the members of [community-scripts](https://github.com/community-scripts) for adding Tourism-Team to their collection and maintaining the install and update scripts.
...
> **Tip:** Always verify the latest command on the [community-scripts Tourism-Team page](https://community-scripts.org/scripts/trek) before running — the script URL may change between releases.
...
The container is unprivileged. Tourism-Team is installed at `/opt/trek`.
...
On first boot, Tourism-Team automatically creates an admin account. The credentials are printed to the container log — check them with:
...
Tourism-Team runs as a systemd service named `trek` inside the LXC. To view logs from within the container:
...
If your Proxmox host has multiple network interfaces and you want Tourism-Team to listen on only one of them, set the `HOST` variable in `/opt/trek/server/.env`:
...
> **Tip:** Always check the [community-scripts Tourism-Team page](https://community-scripts.org/scripts/trek) to confirm the latest command before running.
...
- [Reverse-Proxy](Reverse-Proxy) — put Tourism-Team behind Nginx or Caddy
